...
**María Antonella Pereira & Nathalia Maya - Marzo 2021 
	...
	*afro_ci*
	*********
sgen byte afro_ci = . 	  // se queda como missing (.) si no existe la pregunta
...
replace subemp_ci = 1 if horab < 30 & d05 == 5 & d01 == 1
...
* c12 - Cuántas otras ocupaciones tuvo o tiene en últimos 7 días? // 9 NR
* c13- Horas que habitualmente trabaja en ocupación (otras)  // 999 NR
gen byte horao = c13 if c12 == 1
replace horao = 0 if (c12 == 1 & c12 == 9) | c13 == 999
...
/*Variable de inactividad: ra06ya09
           ...
           4 demasiado joven
           5 enfermo
           6 anciano o discapacitado
           7 jubilado o pensionado
           8 rentista
           9 motivos familiares
          10 otra situacion */		   
gen categoinac_ci = .
...
replace ynlm_ci=. if e01dde1==. & e01ede1==. & e01fde1==. & e01hde1==. & e01ide1==. & e01jde1==. & e01kde1==. & e01lde ==. & e01gde1==.

drop e01dde1 e01ede1 e01fde1 e01hde1 e01ide1 e01jde1 e01kde1 e01lde1 e01gde1
...
***eduui_ci***
**************
gen eduui_ci = (inrange(ed54, 1801, 2206) & ed06c == 14)
...
*aguafconsumo_ch*
*****************
/* v09 - Agua para beber en el hogar. El agua que más beben en el hogar proviene de…
	 ...
	 12 Aguatero
	 13 Agua superficial (río, represa, lago, estanque, arroyo, canal, canales de riego)
	 14 Otro (especificar)
	 99 NR

v10. El agua que beben llega a su vivienda a través de….
	  1 Cañería dentro del terreno pero fuera de la vivienda
	  2 Cañería dentro de la vivienda
	  3 Canilla pública
	  4 Vecino
	  5 Aguatero
	  6 Agua embotellada (mineral)
	  7 Otros medios (especificar)
	  9 NR */

gen byte aguafconsumo_ch = .
replace aguafconsumo_ch = 1  if inlist(v09, 1, 2, 3, 4) & inlist(v10, 1, 2)  // red, cañería privada/terreno
replace aguafconsumo_ch = 2  if inlist(v09, 1, 2, 3, 4) & v10 == 3 		// llave pública
replace aguafconsumo_ch = 3  if v09 == 11 | v10 == 6 					//agua embotellada
replace aguafconsumo_ch = 4  if inlist(v09, 5, 6) 						// pozo protegido (dentro del terreno)
replace aguafconsumo_ch = 5  if v09 == 10  								// lluvia
replace aguafconsumo_ch = 6  if v09 == 12 | v10 == 6 					// camión cisterna (aguatero)
replace aguafconsumo_ch = 7  if (v09 == 8 | ((inlist(v09, 1, 2, 3, 4) & (inlist(v10, 4, 7)))))  // otra mejorada
replace aguafconsumo_ch = 8  if v09 == 13 								// agua superficial
replace aguafconsumo_ch = 9  if inlist(v09, 7, 9) 						// pozo/manantial sin protección
replace aguafconsumo_ch = 10 if (inlist(v09, 14, 99) | v10 == 9) 		// otros, no clasificable, NR
...
v10. El agua que beben llega a su vivienda a través de….
	  ...
	  5 Vecino
	  6 Aguatero
	  7 Agua embotellada (mineral)
	  8 Otros medios (especificar)
	  9 NR	*/

gen byte aguafuente_ch = .
replace aguafuente_ch = 1  if inlist(v06, 1, 2, 3, 4)  & inlist(v10, 1, 2) 	//red (ESSAP/SENASA/comunitaria/privada)
replace aguafuente_ch = 2  if inlist(v06, 1, 2, 3, 4) & v10 == 3  		// llave pública
...
replace aguafuente_ch = 6  if (inlist(v06, 1, 2, 3, 4) & v10 == 6)		// camion cisterna (aguatero)
replace aguafuente_ch = 7  if (inlist(v06, 1, 2, 3, 4) & inlist(v10, 4, 5, 8))  // otra mejorada
replace aguafuente_ch = 8  if v06 == 9 									 // agua superficial (tajamar/río)
replace aguafuente_ch = 10 if (inlist(v06, 7, 8, 11, 99) | (inlist(v06, 1, 2, 3, 4) & inlist(v10, 7, 8, 9)))  // pozo, manantial, otras no clasificable, NR
...
replace bano_ch=6 if (v15==8 | v15==3 |v15 ==9) | (v14==.& jefe_ci!=.)
...
gen telef_ch=.
replace telef_ch=1 if v13a==1 //Sí cuentan con línea fija
replace telef_ch=0 if v13a==6 //No cuentan con línea fija
...
gen compu_ch=.
replace compu_ch=1 if v26a==1				//Sí tienen computadora
replace compu_ch=0 if v26a==6				//No tienen computadora
