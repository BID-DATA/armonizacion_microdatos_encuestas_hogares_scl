...
gen lp_ci =.
replace lp_ci= 311654  if dominio==1 | dominio==2   /*area metropolitana*/
replace lp_ci= 186594 if dominio==3 | dominio==5   /*rural*/
replace lp_ci= 219642 if dominio==4  /*resto urbano*/
...
gen lpe_ci =.
replace lpe_ci= 160217  if dominio==1 | dominio==2   /*area metropolitana*/
replace lpe_ci= 113773 if dominio==3 | dominio==5   /*rural*/
replace lpe_ci= 122953 if dominio==4  /*resto urbano*/
...
/* d01: En los últimos 7 días ¿estuvo disponible para trabajar más horas? 
 ...
 d05: ¿Cuál es la razón principal por al que desea mejorar o cambiar o adicionar su empleo actual?:
...
* c01 - Ocupación (activ. secundaria) // 0 NA
* c03 - N° horas trabajadas últimos 7 días (activ. secundaria) // 9 NR
* c04 - Es normal el número de horas trabajadas (activ. secundaria) // 9 NR
* c05 - Horas que trabaja normalmente (otras)  // 999 NR
gen byte horac = c03 if c01 != 0
replace horac = c05 if c04 == 6

* c09 - Cuántas otras ocupaciones tuvo o tiene en últimos 7 días? // 9 NR
* c10 - Horas que habitualmente trabaja en ocupación (otras)  // 999 NR
gen byte horao = c10 if c09 == 1
replace horao = 0 if (c09 == 1 & c09 == 9) | c10 == 999

egen horabco = rsum(horab horac horao), missing
...
----------------------+-----------
0-na                   |     25,090
1empleado público      |      1,236
2empleado privado      |      1,387
3obrero público        |        191
4obrero privado        |      3,263
5empleador o patrón    |        764
6cuenta propia         |      7,415
7famil. no remunerado  |      2,524
8empleado doméstico    |      1,291
----------------------------------
*/
gen categopri_ci=.
...
replace firmapeq_ci=. if b09>8
*/

*****************
***spublico_ci***
*****************
/* cate - categoria de ocupacion:
           0 na                   
           1 empleado público     
           2 empleado privado     
           3 obrero público       
...
replace ynlm_ci=. if e01dde1==. & e01ede1==. & e01fde1==. & e01hde1==. & e01ide1==. & e01jde1==. & e01gde1==.
...
gen rentaimp_ch=v14ede
replace rentaimp_ch=. if v14ede>=15000000
...
replace asiste_ci=1 if ed11>=2 & ed11<=11
replace asiste_ci=0 if ed11==12
label variable asiste_ci "Asiste actualmente a la escuela"

/*
-----------------------------------------
instit. enseÑanza formal a   |
que asiste                   |      Freq.
-----------------------------+-----------
      na                     |     11,761
2sí, educ. esc. básica       |      9,935
3sí, educ. media/bachillerato|      1,894
4sí, formación docente       |        222
5sí, sup. no universitaria   |        108
6sí, sup. universitaria      |        786
7sí, educ. especial          |         10
8sí, educ. básica adultos    |         88
9sí, doctorado               |          4
10sí, maestría               |          7
11sí, especialización        |         59
12      no asiste            |     18,254
      nr                     |         33
-----------------------------------------
*/
...
g       pqnoasis1_ci = 1 if ed16==1 | ed16==4
replace pqnoasis1_ci = 2 if ed16==2
replace pqnoasis1_ci = 3 if ed16==17 | ed16==20 | ed16==23
replace pqnoasis1_ci = 4 if ed16==9  | ed16==18
replace pqnoasis1_ci = 5 if ed16==3  | ed16==21 | ed16==22
replace pqnoasis1_ci = 6 if ed16==8
replace pqnoasis1_ci = 7 if ed16==7
replace pqnoasis1_ci = 8 if ed16==5  | ed16==6  | ed16==10 | ed16==12 | ed16==13 | ed16==14 | ed16==15 | ed16==16
replace pqnoasis1_ci = 9 if ed16==19 | ed16==24 | ed16==25
...
gen edupub_ci=1 if ed12==1
replace edupub_ci=0 if ed12>=2
...
/* v06 -  Agua utilizada en la vivienda. El agua que más utiliza el hogar proviene de…
           ...
           5 Tajamar - río   
           6 Aljibe          
           7 Otra fuente (especificar)            
          99 NR  
	
v07. El agua llega a la vivienda a través de….
...
replace aguafuente_ch = 1  if inlist(v06, 1, 4)  & inlist(v07, 1, 2, 3) 	//red (ESSAP/SENASA/comunitaria/privada)
replace aguafuente_ch = 2  if inlist(v06, 1, 4) & v07 == 4  		// llave pública
replace aguafuente_ch = 4  if v06 == 3		 						// pozo protegido (artesiano/con bomba)
replace aguafuente_ch = 6  if (v06 == 6 | (inlist(v06, 1, 4) & v07 == 6))		// camion cisterna (aguatero)
replace aguafuente_ch = 7  if (inlist(v06, 1, 4) & inlist(v07, 5, 7))  // otra mejorada
replace aguafuente_ch = 8  if v06 == 5 									 // agua superficial (tajamar/río)
replace aguafuente_ch = 10 if inlist(v06, 2, 7, 99) 			   // pozo, manantial, otras no clasificable, NR
...
*/
gen des1_ch=.
replace des1_ch=0 if v09==6
replace des1_ch=1 if v10==1 | v10==3
replace des1_ch=2 if v10==4 | v10==2
replace des1_ch=3 if v10==5
...
replace resid_ch=2 if basura>=3 & basura<=6
replace resid_ch=3 if basura==7 
drop basura
...
gen cocina_ch=.
replace cocina_ch=1 if v1803==3
replace cocina_ch=0 if v1803==0
...
gen telef_ch=.
replace telef_ch=1 if v1804==4 //Sí cuentan con línea fija
replace telef_ch=0 if v1804==0 //No cuentan con línea fija
...
gen automovil=v1810
gen auto_ch=.
replace auto_ch=1 if automovil==10
...
gen compu_ch=.
replace compu_ch=1 if v1815==15				//Sí tienen computadora
replace compu_ch=0 if v1815==0				//No tienen computadora
...
gen internet_ch=v1816
replace internet_ch=1 if v1816==16
...
gen cel_ch=.
replace cel_ch=1 if v1805==5
replace cel_ch=0 if v1805==0
...
gen viviprop_ch=.
replace viviprop_ch=0 if v13==3 | v13==4 
replace viviprop_ch=1 if v13==1
replace viviprop_ch=2 if v13==2
replace viviprop_ch=3 if v13==5 | v13==6 
...
gen vivialqimp_ch=v14ede
replace vivialqimp_ch=. if v14ede>=15000000
