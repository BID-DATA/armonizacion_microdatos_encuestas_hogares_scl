...
replace subemp_ci = 1 if horab < 30 & d04 == 6 & d01 == 1
...
* c10 - Cuántas otras ocupaciones tuvo o tiene en últimos 7 días? // 9 NR
* c11 - Horas que habitualmente trabaja en ocupación (otras)  // 999 NR
gen byte horao = c11 if c10 == 1
replace horao = 0 if (c10 == 1 & c10 == 9) | c11 == 999
...
a05: Por que no trabajo en ult. 7 dias
           ...
           3 demasiado joven       
           4 labores del hogar     
           5 espera comenz. trabajo
           6 inclemencia del tiempo
           7 trabajo temporal      
           8 no consigue trabajo   
           9 estudiante            
          10 enfermo               
          11 anciano, discapacitado
          12 rentista              
          13 jubilado o pensionado 
          14 motivos familiares    
          15 otra situaci�n        
          99 nr  */		  
gen categoinac_ci = .
replace categoinac_ci = 1 if (a05==13 & condocup_ci==3)
...
drop e01dde1 e01ede1 e01fde1 e01hde1 e01ide1 e01jde1 e01gde1
...
Educ. Básica para Adultas    							 10
Superior no Universitario          						 11
Formación Docente                                        12
Form. Militar/Policial                                   13
...
replace aedu_ci = 0 if (nivgra == 0 | (nivgra >= 200 & nivgra <= 299)) /// 
					| (nivgra>=1000 & nivgra<=1099) // sin instruccion, Educ. Inicial, educacion adultos. 
...
replace aedu_ci = aedu_temp + 9 if (nivgra >= 800 & nivgra <= 899) // Media científica
replace aedu_ci = aedu_temp + 9 if (nivgra >= 900 & nivgra <= 999) // Media técnica
...
replace aedu_ci = aedu_temp + 12 if (nivgra >= 1100 & nivgra <= 1199) // Superior no Universitario
replace aedu_ci = aedu_temp + 12 if (nivgra >= 1200 & nivgra <= 1299) // Formación Docente
replace aedu_ci = aedu_temp + 12 if (nivgra >= 1300 & nivgra <= 1399) // Form. Militar Policial
replace aedu_ci = aedu_temp + 12 if (nivgra >= 1400 & nivgra <= 1499) // Superior Universitario

* Post-grado
replace aedu_ci = aedu_temp + 12 + 5 + 2 if ed06 == 2 // Doctorado
replace aedu_ci = aedu_temp + 12 + 5 if ed06 == 3 // Maestría 
replace aedu_ci = aedu_temp + 12 + 5 if ed06 == 4 // Especialización
...
***eduui_ci***
**************
gen eduui_ci = (inrange(ed54, 1101, 1406) & ed06 == 11)
...
***eduuc_ci***
**************
gen eduuc_ci = (inrange(ed06, 1, 10))
...
**************
gen eduac_ci = .
replace eduac_ci = 1 if inrange(ed54, 1201, 1406)
replace eduac_ci = 0 if inrange(ed54, 1101, 1104)
...
replace edupub_ci = 0 if (ed09 == 2 | ed09 == 3 | ed09 == 4)
...
replace aguafuente_ch = 1  if inlist(v06, 1, 5)  & inlist(v08, 1, 2, 3) 	//red (ESSAP/SENASA/comunitaria/privada)
replace aguafuente_ch = 2  if inlist(v06, 1, 5) & v08 == 4  		// llave pública
replace aguafuente_ch = 4  if inlist(v06, 2, 4) 						// pozo protegido (artesiano/con bomba)
replace aguafuente_ch = 6  if (v06 == 7 | (inlist(v06, 1, 5) & v08 == 6))		// camion cisterna (aguatero)
replace aguafuente_ch = 7  if (inlist(v06, 1, 5) & inlist(v08, 5, 7))  // otra mejorada
replace aguafuente_ch = 8  if v06 == 6 									 // agua superficial (tajamar/río)
replace aguafuente_ch = 10 if inlist(v06, 3, 8, 99) 			   // pozo, manantial, otras no clasificable, NR
...
gen telef_ch=.
replace telef_ch=1 if v11l==1 //Sí cuentan con línea fija
replace telef_ch=0 if v11l==6 //No cuentan con línea fija
...
gen compu_ch=.
replace compu_ch=1 if v2411==11				//Sí tienen computadora
replace compu_ch=0 if v2411==0				//No tienen computadora
