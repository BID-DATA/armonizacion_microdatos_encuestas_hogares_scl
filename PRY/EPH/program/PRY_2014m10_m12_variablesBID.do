...
* Se mantiene desde PRY 2011 cambia el 28 de feb de 2014: http://www.tusalario.org/paraguay/Portada/salario/salario-minimo-1/archive/0
...
replace ocupa_ci=7 if (ocup_pea==8 | ocup_pea==7) &  emp_ci==1
replace ocupa_ci=8 if ocup_pea==10 &  emp_ci==1
replace ocupa_ci=9 if ocup_pea==9 &  emp_ci==1
...
servicios comunales, sociales y persona  8
nr	
...
replace rama_ci= 4 if rama_pea==3 & emp_ci==1
replace rama_ci= 5 if rama_pea==4 & emp_ci==1
replace rama_ci= 6 if rama_pea==5 & emp_ci==1
replace rama_ci= 7 if rama_pea==6 & emp_ci==1
replace rama_ci= 8 if rama_pea==7 & emp_ci==1
replace rama_ci= 9 if rama_pea==8 & emp_ci==1
*/
****************
***durades_ci***
****************
...
/* e02tde: ingreso por ayuda familiar del exterior > Esta variable es a nivel de hogar.
   Las variables a nivel individual son e02l1bde e02l2bde */
egen double e02bde = rsum(e02l1bde e02l2bde)
...
drop e01dde1 e01ede1 e01fde1 e01gde1 e01hde1 e01ide1 e01jde1 e01kde1 e02bde1
...
replace aedu_ci = 0 if (nivgra == 0 | (nivgra >= 200 & nivgra <= 299)) /// 
					| (nivgra>=1200 & nivgra<=1299) | (nivgra>=1300 & nivgra<=1399) ///
					| (nivgra>=1400 & nivgra<=1499) | (nivgra>=1500 & nivgra<=1599) ///
					| (nivgra>=1600 & nivgra<=1699) | (nivgra>=1800 & nivgra<=1899) // sin instruccion, Educ. Inicial, educacion adultos.
...
replace aedu_ci = aedu_temp + 6 if (nivgra >= 1700 & nivgra <= 1799) // Formación Profesional no Bachillerato de la Media 
replace aedu_ci = aedu_temp + 12 if (nivgra >= 2000 & nivgra <= 2099) // Técnica Superior  
...
replace aedu_ci = aedu_temp + 12 if (nivgra >= 2300 & nivgra <= 2399) // Form. Militar/Policial 
...
v07a. El agua que utiliza en su vivienda llega a través de …. 
	 1 Cañería dentro del terreno pero fuera de la vivienda
	 2 Cañería dentro de la vivienda
	 3 Canilla pública
	 4 Pozo dentro del terreno
...
replace bano_ch=6 if (v13==8 | v13==3) 
