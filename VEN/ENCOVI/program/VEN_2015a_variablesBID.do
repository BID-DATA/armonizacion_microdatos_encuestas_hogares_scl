...
*****************
gen relacion_ci=.
replace relacion_ci=1 if mhp24==1
replace relacion_ci=2 if mhp24==2
replace relacion_ci=3 if mhp24==3
replace relacion_ci=4 if mhp24>=4 & mhp24<=10 /* Otros familiares */
replace relacion_ci=5 if mhp24==11  
replace relacion_ci=6 if mhp24==12
...
*****************
gen byte civil_ci=.
replace civil_ci=1 if mhp27==8
replace civil_ci=2 if (mhp27>=1  & mhp27<=4)
replace civil_ci=3 if mhp27==5 | mhp27 ==6
replace civil_ci=4 if mhp27==7
...
gen categopri_ci=.
replace categopri_ci=1 if tp49==5 & condocup_ci==1
replace categopri_ci=2 if (tp49==6 | tp49==7 | tp49==9)  & condocup_ci==1
replace categopri_ci=3 if (tp49>=1 & tp49<=4)   & condocup_ci==1
replace categopri_ci=4 if tp49==8 & condocup_ci==1
...
*tamemp_ci***
*************
gen tamemp_ci=1 if tp48>=1 & tp48<=3
label var  tamemp_ci "Tamaño de Empresa" 
*Empresas medianas
replace tamemp_ci=2 if tp48>=4 & tp48<=5
*Empresas grandes
replace tamemp_ci=3 if tp48>=6 & tp48<=7
label define tamaño 1"Pequeña" 2"Mediana" 3"Grande"
label values tamemp_ci tamaño
tab tamemp_ci [iw=factor_ci]

gen categoinac_ci = .
replace categoinac_ci = 1 if ((tp45==7) & condocup_ci==3)
replace categoinac_ci = 2 if ((tp45==6) & condocup_ci==3)
replace categoinac_ci = 3 if ((tp45==5) & condocup_ci==3)
...
*************
gen pension_ci=. 
replace pension_ci = 1 if pp61==1 | (pp63ss==1 | pp63ep==2 | pp63pr==3 | pp63ot==4)
replace pension_ci = 0 if pp61==2 |  (pp63ss==0 & pp63ep==0 & pp63pr==0 & pp63ot==0)
label var pension_ci "1=Recibe pension contributiva"

*************
*ypen_ci*
*************
**DZ Octubre 2017- Se agregan las variables pp63pm pp63om a la suma*
recode pp63sm (98=.) (99=.)
recode pp63em (98=.) (99=.)
recode pp63pm (98=.) (99=.)
recode pp63om (98=.) (99=.)

egen ypen_ci= rsum(pp63sm pp63em pp63pm pp63om) if pension_ci==1, missing
...
gen pensionsub_ci=( psp67m1==14 |  psp67m2==14 | psp67m3==14)
replace pensionsub_ci=. if (( psp67m1>=98 & psp67m1<=99) & ( psp67m2>=98 & psp67m2<=99) & ( psp67m3>=98 & psp67m3<=99))
...
replace cesante_ci=1 if (tp45==4) & condocup_ci==2
...
**salmm_ci***
*************
gen salmm_ci=8163.85 /*promedio 2 do semestre*/
label var salmm_ci "Salario minimo legal"

*************
***tecnica_ci**
*************
gen tecnica_ci=(ep36n==5)
...
replace spublico_ci=1 if emp_ci==1 & (tp49==1 | tp49==2) 
replace spublico_ci=0 if emp_ci==1 & (tp49>2 & tp49<=9) 
label var spublico "Personas que trabajan en el sector publico"

****************
***ylmpri_ci ***
****************
recode tp50m tp51m (99=.) (98=.)
...
recode pp63pm  pp63om (98=.) (99=.)
egen pensiones = rsum(pp63sm pp63em  pp63pm  pp63om), missing
...
g aux = tp51m-pensiones

replace pensiones = pensiones - tp51m if aux== 0 // se hace este ajuste para evitar duplicar las cifras.

egen ynlm_ci= rsum(pensiones tp51m), missing
...
***************
gen byte asiste_ci=.
replace asiste_ci=1 if ep38 ==1 
replace asiste_ci=0 if ep38 ==2
replace asiste_ci=0 if ep38 ==3
...
**************
gen eduno_ci=.
replace eduno=1 if ep36n==1
replace eduno=0 if ep36n>1 & ep36n<=7
...
***************
gen edupre_ci=.
replace edupre=1 if ep36n==2
replace edupre=0 if ep36n>2 | ep36n==1
...
***************
g asispre_ci=.
replace asispre_ci=1 if asiste_ci==1 & (ep36n==2)
replace asispre_ci=1 if asiste_ci==1 & (ep36n==1) & (edad_ci<=5)
recode asispre_ci (.=0)
...
***************
gen edupub_ci=.
replace edupub_ci = 1 if ep41 == 2
replace edupub_ci = 0 if ep41 == 1
...
replace pqnoasis1_ci = 7 if pqnoasis_ci ==6
...
*************
gen bano_ch=.
replace bano_ch = 1 if vp10>=1 & vp10<=3
replace bano_ch = 0 if vp10==4
...
gen des1_ch=.
replace des1_ch=0 if vp10==4
replace des1_ch=1 if vp10==1 | vp10==2 
replace des1_ch=2 if vp10==3
...
gen des2_ch=.
replace des2_ch=0 if vp10==4
replace des2_ch=1 if vp10==1 | vp10==2 | vp10==3
...
replace pared_ch=1 if vp2>=1 & vp2<7
...
**************
gen resid_ch=.
replace resid_ch = 0 if vp11 ==1
replace resid_ch = 1 if vp11 ==5
replace resid_ch = 2 if vp11 ==4
replace resid_ch = 3 if vp11 ==2 | vp11 ==3 | vp11 ==6
...
gen refrig_ch=.
replace refrig_ch=1 if hp20n==1
replace refrig_ch=0 if hp20n==2
...
gen compu_ch=.
replace compu_ch=1 if hp20c==1
replace compu_ch=0 if hp20c==2
...
gen internet_ch=.
replace internet_ch=1 if hp20i==1
replace internet_ch=0 if hp20i==2
...
************
gen cel_ch=.
replace cel_ch=1 if mhp28 ==1
replace cel_ch=0 if mhp28 ==2
...
*****************
gen viviprop_ch=.
replace viviprop_ch=0 if hp22==3 
replace viviprop_ch=1 if hp22==1
replace viviprop_ch=2 if hp22==2
replace viviprop_ch=3 if hp22>=4 & hp22  <=6
...
/*_____________________________________________________________________________________________________*/
  ...
  afro_ci ind_ci noafroind_ci afroind_ci afro_ch ind_ch noafroind_ch afroind_ch dis_ci disWG_ci dis_ch VEN_dis_ci /// Diversidad
