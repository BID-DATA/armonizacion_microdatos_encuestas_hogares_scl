...
replace horastot_ci=tmhp45 if tmhp45>=0 & tmhp45 <168
...
gen categopri_ci=.
replace categopri_ci=1 if tmhp41  ==5 & condocup_ci==1
replace categopri_ci=2 if (tmhp41  ==6 | tmhp41  ==7 | tmhp41  ==9)  & condocup_ci==1
replace categopri_ci=3 if (tmhp41  >=1 & tmhp41  <=4)   & condocup_ci==1
replace categopri_ci=4 if tmhp41  ==8 & condocup_ci==1
...
gen tipocontrato_ci=.
replace tipocontrato_ci =1 if tmhp43 ==1
replace tipocontrato_ci =2 if tmhp43 ==2
replace tipocontrato_ci =3 if tmhp43 ==3 | tmhp43==4
...
*tamemp_ci***
*************
gen tamemp_ci=1 if tmhp40>=1 & tmhp40<=3
label var  tamemp_ci "Tamaño de Empresa" 
*Empresas medianas
replace tamemp_ci=2 if tmhp40>=4 & tmhp40<=5
*Empresas grandes
replace tamemp_ci=3 if tmhp40>=6 & tmhp40<=7
label define tamaño 1"Pequeña" 2"Mediana" 3"Grande"
label values tamemp_ci tamaño
tab tamemp_ci [iw=factor_ci]

gen categoinac_ci = .
replace categoinac_ci = 1 if ((tmhp34==9) & condocup_ci==3)
replace categoinac_ci = 2 if ((tmhp34==8) & condocup_ci==3)
replace categoinac_ci = 3 if ((tmhp34==5) & condocup_ci==3)
...
gen pension_ci=.
replace pension_ci=1 if ((pmhp53j ==1 | pmhp53p==2) | (pmhp55ss==1 | pmhp55ep==2 | pmhp55ip==3 | pmhp55ot==4))
replace pension_ci=0 if (( pmhp53njp ==3) | (pmhp55ss==0 & pmhp55ep==0 & pmhp55ip==0 & pmhp55ot==0))
...
gen ypen_ci= pmhp55bs if pension_ci==1
replace ypen_ci =. if pmhp55bs==98| pmhp55bs==99
...
gen pensionsub_ci=( mmhp58m1==14 |  mmhp58m1==14 | mmhp58m1==14)
replace pensionsub_ci=. if ((mmhp58m1 >=98 & mmhp58m1<=99) & ( mmhp58m2>=98 & mmhp58m2<=99) & ( mmhp58m3>=98 & mmhp58m3<=99))
...
replace cesante_ci=1 if ( tmhp34==4) & condocup_ci==2
...
**salmm_ci***
*************
/*La encuesta toma como periodo de referencia el periodo junio 2018(Fue levantada entre julio y septiembre)
Se toma el salario de Julio, http://www.sistematemis.com.ve/Salarios-Minimos-Venezuela?action=  */
gen salmm_ci=3000000.00
...
replace horaspri_ci=tmhp48  if tmhp45>=0 & tmhp45 <168
...
replace desalent_ci = 1 if (inlist(tmhp37, 1, 3) & condocup_ci == 3)
...
replace afiliado_ci = 1 if ((tmhp44ss==4 | smhp24a ==1 | smhp24b ==2 | pmhp55ss==1) & emp_ci==1)
...
replace cotizando_ci = 1 if ((pmhp56sv ==1 | pmhp56si==2 | pmhp56se==3 | pmhp56so==4) & emp_ci==1)
...
replace spublico_ci=1 if emp_ci==1 & (tmhp41  ==1 | tmhp41  ==2) 
replace spublico_ci=0 if emp_ci==1 & (tmhp41  >2 & tmhp41  <=9) 
label var spublico "Personas que trabajan en el sector publico"

****************
***ylmpri_ci ***
****************
recode tmhp42bs (99=.) (98=.)
gen ylmpri_ci=tmhp42bs
...
egen ynlm_ci= rsum(pensiones tmhp52bs ), missing
...
***eduui_ci***
**************
gen eduui_ci = inlist(emhp27n, 5, 6) & emhp28 == 1 | inlist(emhp27n, 5, 6) &  emhp30 != 1
...
***eduuc_ci***
***************
gen byte eduuc_ci = (inlist(emhp27n, 5, 6) &  emhp30 == 1) | emhp27n == 7
...
**************
gen eduac_ci=.
replace eduac_ci = 1 if inlist(emhp27n, 6, 7)
replace eduac_ci = 0 if emhp27n == 5
...
************
gen sinbano_ch = 3
replace sinbano_ch = 0 if vsp8!=5
...
gen refrig_ch=.
replace refrig_ch=1 if dhp14n ==1
replace refrig_ch=0 if dhp14n ==2
...
gen compu_ch=.
replace compu_ch=1 if dhp14c ==1
replace compu_ch=0 if dhp14c ==2
...
gen internet_ch=.
replace internet_ch=1 if dhp14i==1
replace internet_ch=0 if dhp14i==2
...
gen viviprop_ch=.
replace viviprop_ch=0 if dhp15==3 
replace viviprop_ch=1 if dhp15==1
replace viviprop_ch=2 if dhp15==2
replace viviprop_ch=3 if dhp15>=4 & dhp15  <=7
