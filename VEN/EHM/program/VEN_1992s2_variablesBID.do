...
gen byte civil_ci=.
replace civil_ci=1 if estcivi==0 | estcivi==1
replace civil_ci=2 if estcivi==2 | estcivi==3 | estcivi==4 | estcivi==5
replace civil_ci=3 if estcivi==7
replace civil_ci=4 if estcivi==6
...
/*
egen cc=group( entrevis region efederal locali nrotrh zona tipoviv nrocuart agua excretas alumb tenenviv tiphogar nrolinea parentco sexo edad estcivi casado nrohijos alfabet anoeduc nivel asist lugarnac trabaja tipresid otrotrab otrotra tieneotr pqnotra pagoenef masdeuno hrstot diastrab hrsnort rztramen bustrab pqbustra hechodil cualdil realidil tpojorxi ocupbus pqnobus bus3mes cesante tpoultra ocup tamest rama categ tiping ingprime especies condact factorex paren actv ocupa ocupr ram ramar cat ingprim suesalef gananef ingprief factorh dom persindo ncony nhijos notrop notro nigno nocup ndesocup suesaleh gananefh ingprieh sexoj edadj condactj ocupj ocuprj ramaj ramarj categj nconyuges nhijos_ch notropari notronopari nempdom clasehog nmiembros_ch2 nmayor21 nmenor21 nmayor65 nmenor6 nmenor1)
drop nmiembros_ch2
by cc, sort: gen dup=_n
sort cc dup idh relacio edad sexo
tab dup [w=factor_ci]
by region, sort: tab dup [w=factor_ci]
*/
************************************
...
*/
/*Sintaxis de consultor requirió modificaciones
...
label var  tamemp_ci "Tamaño de Empresa" 
*Empresas medianas
replace tamemp_ci=2 if tamest==2 | tamest==3
*Empresas grandes
replace tamemp_ci=3 if tamest==4 | tamest==5
label define tamaño 1"Pequeña" 2"Mediana" 3"Grande"
label values tamemp_ci tamaño
*/
...
notes categoinac_ci: no se puede identificar a los jubilados
...
*********
gen lp_ci=.
replace lp_ci=2817.8 if zona_c==1
replace lp_ci=2254.2 if zona_c==0
...
gen salmm_ci=.
replace salmm_ci=9000/1000 if zona_c==1 /*en Bs: me parecen q estan muy altos! */
replace salmm_ci=7000/1000 if zona_c==0 
label var salmm_ci "Salario minimo legal"

*Y.L. -> divido al salario minimo entre 1000 para hacerlo comparable a lo largo del tiempo.
...
gen ocupa_ci=.
replace ocupa_ci=1 if ocup>=0 & ocup<=9
replace ocupa_ci=2 if ocup>=10 & ocup<=19
replace ocupa_ci=3 if ocup>=20 & ocup<=23
replace ocupa_ci=4 if ocup>=25 & ocup<=29
replace ocupa_ci=6 if ocup>=30 & ocup<=35
replace ocupa_ci=7 if ocup>=40 & ocup<=79
replace ocupa_ci=5 if ocup>=80 & ocup<=89
replace ocupa_ci=8 if ocup>=90 & ocup<=91
replace ocupa_ci=9 if ocup>=99
...
gen byte rama_ci=.
replace rama=1 if rama>=111 & rama<=130
replace rama=2 if rama>=210 & rama<=290
replace rama=3 if rama>=311 & rama<=390
replace rama=4 if rama>=410 & rama<=420
replace rama=5 if rama==500
replace rama=6 if rama>=610 & rama<=632
replace rama=7 if rama>=711 & rama<=720
replace rama=8 if rama>=810 & rama<=833
replace rama=9 if rama>=910 & rama<=960
...
replace horastot_ci=hrstot if hrstot<=110 & hrstot>0
...
gen durades0_ci=.
replace durades0_ci=tpoultra if tpoultra>0 & tpoultra<9
...
replace desalent_ci=1 if (trabaja>3 & trabaja<=6) & tieneotr==4 & hechodil==2 & pqnobus==1 
...
gen tiempoparc_ci=.
replace tiempoparc_ci=1 if hrsnort>=0 & hrsnort<=30 & rztramen==1
replace tiempoparc_ci=. if (hrsnort==. | rztramen==. | hrsnort<0 | rztramen<0)
label var tiempoparc_ci "Trabajadores a medio tiempo"

* Modificacion MGD 07/14/2014: Condicionado a que esten ocupados.
gen categopri_ci=.
replace categopri_ci=1 if categ==4 & condocup_ci==1
replace categopri_ci=2 if categ==3 & condocup_ci==1
replace categopri_ci=3 if (categ==1 | categ==2)  & condocup_ci==1
replace categopri_ci=4 if categ==5 & condocup_ci==1
...
replace tamfirma_ci=1 if emp==1 & (tamest==5)
replace tamfirma_ci=0 if emp==1 & (tamest>=1 & tamest<=4)
...
replace spublico=1 if emp==1 & categ==1 
replace spublico=0 if emp==1 & categ>=2 & categ<=5
...
gen ylmpri_ci=.
replace ylmpri_ci=ingprime*10 if ingprime<9999 & tiping==2
replace ylmpri_ci=ingprime*10*4.3 if ingprime<9999 & tiping==1
...
gen ylm0_ci=.
replace ylm0_ci=ingprime*10 if ingprime<9999 & tiping==2
replace ylm0_ci=ingprime*10*4.3 if ingprime<9999 & tiping==1
...
gen nrylmpri0_ci=.
replace nrylmpri0_ci=0 if (ingprime>0 & ingprime<9999)
replace nrylmpri0_ci=1 if (ingprime==9999) 
...
sort idh
egen nrylmpri0_ch=sum(nrylmpri0_ci) if miembro==1, by(idh) 
...
gen GRADO=anoeduc if anoeduc>0 & anoeduc<9
...
label var asiste "1: Personas que actualmente asisten a centros de enseñanza"
...
gen eduac_ci=.
replace eduac=0 if eduui_ci==1 | eduuc-ci==1
...
gen luz_ch=.
replace luz_ch=1 if alumb==1
replace luz_ch=0 if alumb==2
...
gen cuartos_ch=.
replace cuartos_ch=nrocuart if nrocuart>=0 & nrocuart<9
...
replace vivi1_ch=2 if tipoviv==2 | tipoviv==3 | tipoviv==4
replace vivi1_ch=3 if tipoviv>=5 & tipoviv<=8
...
gen viviprop_ch=.
replace viviprop_ch=0 if tenenviv>=3 & tenenviv<=7
replace viviprop_ch=1 if tenenviv==1
replace viviprop_ch=2 if tenenviv==2
replace viviprop_ch=3 if tenenviv==9
