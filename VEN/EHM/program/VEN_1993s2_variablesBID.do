...
/*replace mes= 7  if  sem_lev>=1  & sem_lev<=2
...
gen byte civil_ci=.
replace civil_ci=1 if ecivil==0 | ecivil==1
replace civil_ci=2 if ecivil==2 | ecivil==3 | ecivil==4 | ecivil==5
replace civil_ci=3 if ecivil==7
...
*egen cc=group(entrev region entidad ncontrol localida trh area rotacion fecha vivtip cuartos agua excretas alumbrad tenencia hogmiem hogtip linea parent sexo edad ecivil casado hijos alf grado nivel aesc lnac tiempres p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 p12 p13 p14 p15 p16 p17 p18 p19 p20 p21 p22 p23 p24 p26 p27 ingtip ingmon p30 p31 codviv codsum p25b codmat factor filler5d hombres mujeres miem02 miem36 miem79 miem1014 miem1564 miem65o ocup1014 des1014 ocupados desocupa area_res parentes pp13 niveledu escolar cat_ocup jornada salariom salarioh ingf_s1 nconyuges nhijo notropari notronopari nempdom clasehog nmiembros_ch2 nmayor21 nmenor21 nmayor65 nmenor6 nmenor1)
/*
drop nmiembros_ch2
...
*********
gen lp_ci=.
replace lp_ci=3830.1 if zona_c==1
replace lp_ci=3064.1 if zona_c==0
...
gen salmm_ci=.
replace salmm_ci=9000/1000 if zona_c==1 
...
***************
gen byte rama_ci=.
replace rama_ci=1 if (p26>=111 & p26<=141) & emp_ci==1
replace rama_ci=2 if (p26>=210 & p26<=290) & emp_ci==1
replace rama_ci=3 if (p26>=311 & p26<=390) & emp_ci==1
replace rama_ci=4 if (p26>=410 & p26<=420) & emp_ci==1
replace rama_ci=5 if p26==500 & emp_ci==1
replace rama_ci=6 if (p26>=610 & p26<=632) & emp_ci==1
replace rama_ci=7 if (p26>=711 & p26<=720) & emp_ci==1
replace rama_ci=8 if (p26>=810 & p26<=833) & emp_ci==1
replace rama_ci=9 if (p26>=910 & p26<=960) & emp_ci==1
...
gen subemp_ci=.
*replace subemp_ci=1 if p11>=0 & p11<=30 & pp32==1 
*replace subemp_ci=. if (p11==. | pp32==. | p11<0 | pp32<0)
...
replace spublico=1 if emp==1 & p27==1 
replace spublico=0 if emp==1 & p27>=2 & p27<=5
...
** LAS PREGUNTAS DE EDUCACION SE HACEN A PARTIR DE LOS 10 AÑOS DE EDAD, NO COMPARABLE CON LOS AÑOS POSTERIORES EN DONDE SE REALIZAN A PARTIR DE LOS 3 AÑOS ** 
