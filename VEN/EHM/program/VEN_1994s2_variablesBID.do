...
label variable mes_c "Mes de la Encuesta: Segundo Semestre de 1994"  
label define mes_c 7 "JUL" 8 "AUG" 9 "SEP" 10 "OCT" 11 "NOV" 12 "DEC" 
label values mes_c mes_c

gen zona_c=.
replace zona_c=1 if area_res==1
replace zona_c=0 if area_res==2
...
gen relacion_ci=.
replace relacion_ci=1 if parentes==1
replace relacion_ci=2 if parentes==2
replace relacion_ci=3 if parentes==3
replace relacion_ci=4 if parentes>=4 & parentes<=14 /* Otros familiares */
replace relacion_ci=5 if parentes==15  
replace relacion_ci=6 if parentes==16 | parentes==17 /*Es el sevicio domestico, Incluye a familiares del Serv. Domestico en parentes==17 */
...
/* Sintaxis del consultor requirió modificació.
...
*  ypen_ci  *
*************
*11/3/2015 MGD: se lo divide para 1000 para hacerlo comparable
...
*********
gen lp_ci=.
replace lp_ci=6567.6 if zona_c==1
replace lp_ci=5254.1 if zona_c==0
...
replace ocupa_ci=9 if pp44a>=99
...
gen YOCUPAPM1=pp56d
gen YOCUPAM1=pp57
replace pp56d=. if pp56d==999998
replace pp57=. if pp57==999998
replace pp59g=0 if pp59g==999999
replace pp60h=0 if pp60h==9999
...
*Y.L. -> Divido al ingreso entre 1000 para hacerlo comparable en el tiempo
...
gen nrylmpri_ci=.
replace nrylmpri_ci=0 if (pp56d!=-1 & pp56d!=-2 & pp56d!=-3 & YOCUPAPM1!=999998)
replace nrylmpri_ci=1 if (pp56d==-2 | pp56d==-3 | YOCUPAPM1==999998) 
...
gen NIVEL=pp18a if pp18a<9
gen GRADO=pp18b
gen ULTSEM=pp18c if pp18c<99
gen ASIST=pp20 if pp20<3
gen EDAD=edad
...
gen byte aedu_ci=.
replace aedu=0 if NIVEL==0 | NIVEL==1
replace aedu=GRADO if NIVEL==2 & GRADO>0
replace aedu=GRADO+9 if NIVEL==3 & GRADO>0 & GRADO<=2
replace aedu=11 if NIVEL==3 & GRADO>2
replace aedu=GRADO+11 if (NIVEL==4 | NIVEL==5) & GRADO>0 
replace aedu=int(ULTSEM/2)+11 if (NIVEL==4 | NIVEL==5) & ULTSEM>0 
...
gen eduno_ci=.
replace eduno=1 if NIVEL==0
replace eduno=0 if NIVEL>0 & NIVEL<=6
label var eduno_ci "1 = personas sin educacion (excluye preescolar)"

gen edupre_ci=.
replace edupre=1 if NIVEL==1
replace edupre=0 if NIVEL>1 | NIVEL==0
...
OLD CODE:
gen eduui_ci=.
replace eduui=1 if aedu>11 & ((aedu<14 & NIVEL==4) | (aedu<16 & NIVEL==5))
replace eduui=0 if (aedu>=0 & aedu<=11) | (aedu>=16 & aedu!=. & NIVEL==5) | (aedu>=14 & aedu!=. & NIVEL==4) | (NIVEL==3 & GRADO==3 & aedu==12)
label var eduui_ci "1 = personas que no han completado el nivel universitario o superior"

gen eduuc_ci=.
replace eduuc=1 if (aedu>=16 & aedu!=. & NIVEL==5) | (aedu>=14 & aedu!=. & NIVEL==4)
replace eduuc=0 if aedu>=0 & ((aedu<14) | (aedu<16 & NIVEL==5))
...
replace edus1i=1 if edusi==1 & (NIVEL==2 & (GRADO==7 | GRADO==8))
...
replace edus1c=1 if edusi==1 & (NIVEL==2 & GRADO==9)
...
replace edus2i=1 if edusi==1 & (NIVEL==3 & GRADO<2) 
