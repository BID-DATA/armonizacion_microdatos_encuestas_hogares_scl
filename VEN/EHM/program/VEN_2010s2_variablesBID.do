...
/* Y.L = Definición de indicadores:
http://www.ine.gov.ve/index.php?option=com_content&id=333&Itemid=103

Población Ocupada:Personas de 15 años y más de edad, de uno u otro sexo,
quienes declararon que estaban trabajando o tenían un empleo, durante la 
semana anterior al día de la entrevista.

Población Desocupada: Personas de 15 años y más, de uno u otro sexo, quienes
declararon que durante la semana anterior al día de la entrevista no estaban 
...
gen salmm_ci=.
/*
replace salmm_ci=1064.25 if mes_c>=6 & mes_c<=8
replace salmm_ci=1223.89 if mes_c>=9 & mes_c<=12
no hay la variable mes_c*/
* 2015 MGD: promedio del semestre y segun zona urbana y rural

replace salmm_ci=1223.89
...
replace desalent_ci = 1 if (inlist(pp43, 1, 2, 3) & condocup_ci == 3)
...
* no esta la variable pp42 en la base
gen subemp_ci=.
...
***eduui_ci***
**************
gen eduui_ci = inlist(pp25a, 5, 6) & pp26 == 2
...
***eduuc_ci***
***************
gen byte eduuc_ci = inlist(pp25a, 5, 6) & pp26 == 1
...
**************
gen eduac_ci=.
replace eduac_ci = 1 if pp25a == 6
replace eduac_ci = 0 if pp25a == 5
...
gen dorm_ch=ph12 /* en caso de detectar un solo hogar esta pregunta no se hace y se copia de la variable pv6*/
*replace dorm_ch=pv6
...
clonevar codindustria=pp49
clonevar codocupa=pp48
