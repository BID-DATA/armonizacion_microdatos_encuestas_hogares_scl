...
*categoinac_ci***
*****************
*Genera la variable para clasificar a los inactivos Jubilados, pensionados e incapacitados
gen categoinac_ci = .
...
replace desalent_ci = 1 if (inlist(pp39, 1, 2) & condocup_ci == 3)
...
replace cotizando_ci = 1 if (pp56c==1 & emp_ci==1)
...
* Ha hecho algo para trabajar horas adicionales pp38(2005), pp35c(2004)y--(2003-1995)
gen subemp_ci=0
...
replace aguafuente_ch= 10 if pv7==4   //otras fuentes
...
01	Platabanda  MP
02	Teja MP
03	Láminas asfálticas (solo a partir de 2003) MP
--  Fibrocemento, cemento, ligero y similares
04	Láminas métalicass (Zinc y similares) MP
05	Asbesto y Similares (solo a partir de 2003)  MP? PREGUNTAR
06	Otros (Palmas, tabla y similares) MNP (para la encuesta 2021 esto es desecho)
*/
gen techo_ch=.
...
*Alquiler ph16b
gen vivialq_ch=.
destring ph16b, replace
replace vivialq_ch=ph16b if ph16b>=0 &viviprop_ch==0 //solo para los que declararon que alquilan
