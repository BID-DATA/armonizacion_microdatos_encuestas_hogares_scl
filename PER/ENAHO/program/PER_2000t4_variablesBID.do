...
gen mes_c=real(substr(fecent01,1,2))
...
replace afiliado_ci = 1 if (p558a==1 & emp_ci==1)
...
gen remesas_ci= d556t2/3 if (p5565a==1 & d556t2 != 0)
...
gen edad = p208a
gen sexo = p207
gen factorex=round(factor,1)

/*
ESTRATO
1 Ciudades Capitales (Urbano)
2 Grandes Ciudades (Urbano)
3 CCPP de m s de 2000 Habitantes (Urbano)
4 CCPP DE 500 a 2000 Habitantes (Rural) 
5 Area Rural
*/

gen area=1 if     (estrato>=1 & estrato<=4)
replace area=2 if (estrato>=4 & estrato<=5)
...
1 Sin nivel				---	---
2 Inicial				---	---
3 Primaria incompleta			0-5	0-5
4 Primaria completa			5-6	6
5 Secundaria incomplet			1-4	---
6 Secundaria completa			5	---
7 Bachillerato				1	---
8 Sup. no Univ.Incomp.			1-4	---
9 Sup.no Univ. Comp.			3-5	---
10 Sup. Univ. Incompl.			1-5	---
11 Sup. Univ. Completa			4-7	---	
12 Post-Grado Universitario		1-3 	---	
*/
...
replace anoest=12 if	(p301a>=7 & p301a<=11) & p301b==1
replace anoest=13 if	(p301a>=7 & p301a<=11) & p301b==2
replace anoest=14 if	(p301a>=7 & p301a<=11) & p301b==3
replace anoest=15 if	(p301a>=7 & p301a<=11) & p301b==4
replace anoest=16 if	(p301a>=7 & p301a<=11) & p301b==5
replace anoest=17 if	(p301a>=7 & p301a<=11) & p301b==6 | (p301a==12) & p301b==1
replace anoest=18 if	(p301a>=7 & p301a<=11) & p301b==7 | (p301a==12) & p301b==2
replace anoest=19 if 	(p301a==12) & p301b==3 
replace anoest=20 if 	(p301a==12) & p301b==4 
...
/*
p204
204. ¿ES MIEMBRO DEL HOGAR FAMILIAR?
 1. Si
 2. No
...
6 Secundaria completa			5	---	1-2 ==> 303
7 Bachillerato				1	---	1-2 ==> 303
8 Sup. no Univ.Incomp.			1-4	---	1-2 ==> 303
9 Sup.no Univ. Comp.			3-5	---	1-2 ==> 303
10 Sup. Univ. Incompl.			1-5	---	1-2 ==> 303
11 Sup. Univ. Completa			4-7	---	1-2 ==> 303	
12 Post-Grado Universitario		1-3 	---	1-2 ==> 303	

p302 
302. ¿Sabe leer y escribir?

p303
303. ACTUALMENTE, ¿ASISTE A ALGUN CENTRO O PROGRAMA DE ENSEÑANZA REGULAR?

p304a 					p304b	p304c	p304d Centro de estudios
304. Nivel educativo al que asiste	Año	Grado	1. Estatal 2.No Estatal
1 Educación Inicial			---	---	1-2
2 Primaria				0,1-5	1-6	1-2
3 Secundaria				1-5	---	1-2
4 Bachillerato				1-2
5 Superior No Universitaria		1-5	---	1-2
6 Superior Universitaria		1-6	---	1-2
7 Post-grado Universitario, 		
Doctorado, Maestría			1-2	---	1-2
*/
...
gen NERP=0 if     p204==1 & (edad>=6 & edad<=11) & (p303==1 | p303==2)
replace NERP=1 if p204==1 & (edad>=6 & edad<=11) & (p303==1) & p304a==2

** Target 3, Additional Indicator: Net Attendance Ratio in Secondary
* ISCED 2 & 3

gen NERS=0 if     p204==1 & (edad>=12 & edad<=16) & (p303==1 | p303==2)
replace NERS=1 if p204==1 & (edad>=12 & edad<=16) & (p303==1) & (p304a==3 | p304a==4)
...
gen leeYesc=0 if (p301a>=1 & p301a<=12)
...
gen prim=1 if (p303==1 & p304a==2) & p204==1
gen sec=1 if  (p303==1 & (p304a==3 | p304a==4)) & p204==1
gen ter=1 if  (p303==1 & (p304a==5 | p304a==6)) & p204==1
...
gen MARW=1 if ((p302==1 | (p301a>=4 & p301a<=12)) & (edad>=15 & edad<=24) & (sexo==2)) 
replace MARW=0 if MARW==.
gen HARW=1 if ((p302==1 | (p301a>=4 & p301a<=12)) & (edad>=15 & edad<=24) & (sexo==1)) 
replace HARW=0 if HARW==.

gen RATIOLITRW=0 if ((p302==1 | (p301a>=4 & p301a<=12)) & (edad>=15 & edad<=24) & (sexo==2)) 
replace RATIOLITRW=1 if ((p302==1 | (p301a>=4 & p301a<=12)) & (edad>=15 & edad<=24) & (sexo==1)) 
...
gen WENAS=0 if     (edad>=15 & edad<=64) & (p507==3 | p507==4) & (p506>=1010 & p506<=9500)
replace WENAS=1 if (edad>=15 & edad<=64) & (p507==3 | p507==4) & (p506>=1010 & p506<=9500) & (sexo==2)

** Target 4, Indicator: Share of women in wage employment in the non-agricultural sector (%)
* With domestic servants

gen WENASD=0 if     (edad>=15 & edad<=64) & (p507==3 | p507==4 | p507==6) & (p506>=1010 & p506<=9500)
replace WENASD=1 if (edad>=15 & edad<=64) & (p507==3 | p507==4 | p507==6) & (p506>=1010 & p506<=9500) & (sexo==2)
...
replace ELEC=1 if p204==1 & (p1121==1 | p1125==4)
...
replace SFUELS=1 if p204==1 & (solidfuels==1 & otherfuel==0)
...
* 11-33 => 1st Digit => Household number, 
*	=> 2nd Digit => Number of households in the dwelling

gen str2 hogar_s=string(hogar)
gen str1 hog_no=substr(hogar_s,1,1)
gen str1 numhog=substr(hogar_s,2,1)
...
replace SECTEN=0 if p204==1 & (((tenencia==5 | tenencia==6 | tenencia==7) | (tipoviv==5 | tipoviv==6 | tipoviv==7)) | ((paredes==4 | paredes==5 | paredes==7 | paredes==8)| (piso==6 | piso==7)) | (persroom>=3) | (SANITATION==0 | WATER==0))
...
gen TELEPHONE=0 if     p203!=0 
replace TELEPHONE=1 if p203!=0 & (p1141==1|p1142==1)
...
p5586 En su centro de trabajo Ud. tiene acceso a Internet?
