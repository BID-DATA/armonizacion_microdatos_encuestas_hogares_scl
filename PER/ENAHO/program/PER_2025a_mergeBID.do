...
clear all
set more off
global ruta = "${surveysFolder}\survey\PER\ENAHO\2025\a\data_orig"
global out ="${surveysFolder}\survey\PER\ENAHO\2025\a\data_merge"

*Equipamiento del hogar
use "$ruta\1031-Modulo18\1031-Modulo18\enaho01-2025-612.dta" , clear
...
label var p6127 "Su hogar tiene: computadora/laptop"
label var p6128 "Su hogar tiene: plancha electrica"
...
label var p61213 "Su hogar tiene: lavadora de ropa"
...
saveold "$ruta\1031-Modulo18\1031-Modulo18\enaho01-2025-612_1.dta", replace

* Merge de bases de datos

clear
use "$ruta\1031-Modulo01\1031-Modulo01\enaho01-2025-100.dta", clear // Características de la Vivienda y del Hogar 
keep if result==1 | result==2 /* Nos quedamos solo con Encuestas completas e incompletas*/
	sort conglome vivienda hogar
	merge 1:m conglome vivienda hogar using "$ruta\1031-Modulo04\1031-Modulo04\enaho01a-2025-400.dta" // Salud 
	tab _merge
	tab result, m
	drop _merge

	sort conglome vivienda hogar codperso
	merge 1:1 conglome vivienda hogar codperso using "$ruta\1031-Modulo02\1031-Modulo02\enaho01-2025-200.dta" //  	Características de los Miembros del Hogar 
	tab _merge
	tab result, m
	tab _merge result, m
	keep if result==1 | result==2 /* Nos quedamos solo con Encuestas completas e incompletas*/
	...
	sort conglome vivienda hogar codperso
	merge 1:1 conglome vivienda hogar codperso using "$ruta\1031-Modulo03\1031-Modulo03\enaho01a-2025-300.dta" //  	Educación - este módulo no le pregunta a personas con menos de 3 años - por lo que tiene menos observaciones
	tab _merge
	tab result, m
	keep if result==1 | result==2 /* Nos quedamos solo con Encuestas completas e incompletas*/
	drop _merge
	...
	sort conglome vivienda hogar codperso
	merge 1:1 conglome vivienda hogar codperso using "$ruta\1031-Modulo05\1031-Modulo05\enaho01a-2025-500.dta" //  	Empleo e Ingresos - este módulo no le pregunta a personas con menos de 14 años - por lo que tiene menos observaciones
	tab _merge
	tab result, m
	keep if result==1 | result==2 /* Nos quedamos solo con Encuestas completas e incompletas*/
	drop _merge
	...
	sort conglome vivienda hogar
	merge m:1 conglome vivienda hogar  using "$ruta\1031-Modulo34\1031-Modulo34\sumaria-2025.dta" //Sumarias ( Variables Calculadas ) 
	tab _merge
	tab result, m 
	keep if result==1 | result==2 /* Nos quedamos solo con Encuestas completas e incompletas*/
	drop _merge
	...
	sort conglome vivienda hogar
	merge m:1 conglome vivienda hogar using "$ruta\1031-Modulo18\1031-Modulo18\enaho01-2025-612_1.dta" //  	Equipamiento del Hogar 
	tab _merge
	tab result, m 
	keep if result==1 | result==2 /* Nos quedamos solo con Encuestas completas e incompletas*/
	drop _merge
