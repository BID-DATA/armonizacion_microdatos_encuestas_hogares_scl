...
replace relacion_ci=4 if  rel_head==4
...
gen civil_ci=.
replace civil_ci=1 if marital_status==1
replace civil_ci=2 if marital_status==2 | marital_status==3
...
egen nmiembros_ch=rowtotal(aux ninos ninas)
...
gen cesante_ci=1 if ever_worked==1 & condocup_ci==2
replace cesante_ci=0 if ever_worked==2 & condocup_ci==2
...
gen nempleos_ci=.
replace nempleos_ci=1 if more_than_1_job==2
replace nempleos_ci=2 if more_than_1_job==3
...
***spublico_ci***
*****************
gen spublico_ci=1 if ((emp_stat==1 | emp_stat==2) & condocup_ci==1)
...
replace ocupa_ci=2 if ((occupation>=1110 & occupation<=1439)) & emp_ci==1
replace ocupa_ci=3 if ((occupation>=4110 & occupation<=4419))& emp_ci==1
replace ocupa_ci=4 if ((occupation>=5200 & occupation<=5249) | (occupation>=9110 & occupation<=9113)) & emp_ci==1
replace ocupa_ci=5 if ((occupation>=5110 & occupation<=5169) | (occupation>=5310 & occupation<=5419) | (occupation>=9120 & occupation<=9162) | (occupation>=9500 & occupation<=9711)) & emp_ci==1
replace ocupa_ci=6 if ((occupation>=6110 & occupation<=6349) | (occupation>=9211 & occupation<=9214)) & emp_ci==1
replace ocupa_ci=7 if ((occupation>=7111 & occupation<=8350) | (occupation>=9310 & occupation<=9414))& emp_ci==1
...
gen rama_ci=.
replace rama_ci=1 if (industry>=111 & industry<=399) & emp_ci==1
replace rama_ci=2 if industry>=500 & industry<=999 & emp_ci==1
replace rama_ci=3 if industry>=1000 & industry<=3499 & emp_ci==1
replace rama_ci=4 if industry>=3500 & industry<=3999 & emp_ci==1
replace rama_ci=5 if industry>=4010 & industry<=4399 & emp_ci==1
...
* Variable categorica
gen durades_ci=.
replace durades= 0.5  if available_since  ==1
replace durades= 2    if available_since  ==2
replace durades= 4.5  if available_since  ==3
replace durades= 9    if available_since  ==4
replace durades= 12   if available_since  ==5
...
***ylmpri_ci***
***************
recode empl_income (9999999=.) 
g ylmpri_ci=empl_income/12
...
***ylmsec_ci***
***************
recode second_income (9999999=.) 
g ylmsec_ci=second_income/12
...
***ylmotros_ci***
*****************
recode  othr_income ( 9999999 =.) (9989999=.)
gen ylmotros_ci=  othr_income/12
...
/*NO es posible saber cuantos años de educación tienen los individuos, ni tampoco definir cual es 
...
replace aguared_ch=0 if (water>=2 & water<=4)
...
gen aguamala_ch=.
replace aguamala_ch=1 if water==4
replace aguamala_ch=0 if (water>=1 & water<=3)
...
gen aguamejorada_ch=.
replace aguamejorada_ch=1 if water>=1 & water<=3
replace aguamejorada_ch=0 if water==4
...
/*_____________________________________________________________________________________________________*/
  ...
  aedu_ci eduui_ci eduuc_ci edupre_ci eduac_ci asiste_ci razonesnoasis_ci /// Educación
