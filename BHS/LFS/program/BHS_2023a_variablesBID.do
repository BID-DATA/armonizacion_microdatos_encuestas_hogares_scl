...
**********************************
	...
	* Ocupado: trabajó ≥1h o ausente con vínculo
	replace condocup_ci = 1 if (anywork==1) ///
		| (temporarily_abse==1 & (expect_return==1| date_of_return==1))
	* Desocupado: no ocupado + buscó (acciones) + disponible
	replace condocup_ci = 2 if condocup_ci==. & edad_ci>=15 ///
		& ( look_for_work_in==1 ///
			| most_recent_act_1==1 | most_recent_act_2==1 | most_recent_act_3==1 ///
			| most_recent_act_4==1  | most_recent_act_5==1| most_recent_act_6==1 | most_recent_act_7==1) ///
		& ( offered_job==1 | available_since<. )
	* Inactivo
	...
	* 1 Jubilado/Pensionado | 2 Estudiante | 3 Quehaceres domésticos | 4 Otros inactivos
	replace categoinac_ci = 1 if condocup_ci==3 & major_activity==6   // Retired
	replace categoinac_ci = 2 if condocup_ci==3 & major_activity==5   // At school
	replace categoinac_ci = 3 if condocup_ci==3 & major_activity==4   // At home
	replace categoinac_ci = 4 if condocup_ci==3 & inlist(major_activity,3,7) // Unable / Other
	...
	*****************
	...
	replace durades_ci = available_since*52/12 if condocup_ci==2 & available_since<.
	...
	****************
	...
	replace desalent_ci = (discouraged==1) if discouraged<.
...
****************************
	...
	**********
	...
	rename what_is_your_highest_certi highest_certi
	gen aedu_ci = .
