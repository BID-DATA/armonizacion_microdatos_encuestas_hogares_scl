...
global ruta = "${surveysFolder}\\survey\\BHS\LFS\\2019\\a\\data_orig"
...
use  "$ruta\Bahamas_LFS_`ANO'_housing.dta", clear
replace island = 18 if island == 4
tab island
tab island, nolab
sort island hhno  
...
use "$ruta\Bahamas_LFS_`ANO'_individual.dta", clear
tab island
tab island, nolab
...
br if dup>=1
sort island hhno  
count if missing(island) | missing(hhno)    // before merge (individuals)

merge  m:1 island hhno using "$ruta\housing.dta"
tab island if _merge==1   // after merge, what islands failed on the individual side
tab island if _merge==2   // which islands exist only in housing
