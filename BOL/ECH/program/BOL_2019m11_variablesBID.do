...
replace relacion_ci=4 if  s02a_05>=4 &  s02a_05<=10
replace relacion_ci=5 if  s02a_05==11 |  s02a_05==13
replace relacion_ci=6 if  s02a_05==12
...
gen tamemp_ci=.
replace tamemp_ci=1 if s06b_21a>=1 & s06b_21a<=5
replace tamemp_ci=2 if s06b_21a>=6 & s06b_21a<=49
replace tamemp_ci=3 if s06b_21a>49 & s06b_21a!=.
...
gen tamemp=.
replace tamemp=1 if s06b_21a>=1 & s06b_21a<=4
replace tamemp=2 if s06b_21a>=5 & s06b_21a<=14
replace tamemp=3 if s06b_21a>=15 & s06b_21a<=40
replace tamemp=4 if s06b_21a>=41 & s06b_21a!=.
...
egen double aux_pc = rowtotal(s07a_01a s07a_01b s07a_01c s07a_01d), missing
...
*pensionsub_ci*
***************
gen pensionsub_ci = (s07a_01e == 1)  if !missing(s07a_01e)
...
replace desalent_ci = 1 if (s06a_05 == 2 & inlist(s06a_10, 3, 4) & condocup_ci == 3)
...
replace cotizando_ci = 1 if (s06g_55 == 1 & emp_ci==1)
...
replace afiliado_ci = 1 if (s06g_54 == 1 & emp_ci==1)
...
gen subemp_ci=0
replace subemp_ci=1 if (s06g_47==1 & s06g_48==1)  & horaspri_ci <= 30 & emp_ci==1
...
replace nempleos_ci=2 if emp_ci==1 & s06d_34==1
...
***antiguedad_ci***
*******************
**Modificado: Lina Arias 06/25/2021
** Se incluye la antiguedad de ser cuenta propia, empleador, ... la cual se preguntó en 2019
gen antiguedad_ci=.	
replace antiguedad_ci=s06b_15aa/52.14  	if s06b_15ab==2 & emp_ci==1
replace antiguedad_ci=s06b_15aa/12   	if s06b_15ab==4 & emp_ci==1
replace antiguedad_ci=s06b_15aa	   		if s06b_15ab==8 & emp_ci==1

replace antiguedad_ci=s06b_15ca/52.14  	if s06b_15cb==2 & emp_ci==1
replace antiguedad_ci=s06b_15ca/12   	if s06b_15cb==4 & emp_ci==1
replace antiguedad_ci=s06b_15ca	   		if s06b_15cb==8 & emp_ci==1
...
gen categoinac_ci = .
replace categoinac_ci = 1 if (s06a_09==3 & condocup_ci==3) // Jubilados o pensionados
replace categoinac_ci = 2 if (s06a_09==1 & condocup_ci==3) // Estudiantes
replace categoinac_ci = 3 if (s06a_09==2 & condocup_ci==3) // Quehaceres domésticos
...
gen yliquido2 = .
replace yliquido2= s06f_42a*30		if s06f_42b==1
replace yliquido2= s06f_42a*4.3		if s06f_42b==2
replace yliquido2= s06f_42a*2		if s06f_42b==3
replace yliquido2= s06f_42a			if s06f_42b==4
replace yliquido2= s06f_42a/2		if s06f_42b==5
replace yliquido2= s06f_42a/3		if s06f_42b==6
replace yliquido2= s06f_42a/6		if s06f_42b==7
replace yliquido2= s06f_42a/12		if s06f_42b==8
...
gen yhrsextr2 = .
replace yhrsextr2=s06f_43a1/12 if s06f_43a==1
...
gen yremesas_m = .
replace yremesas_m = rem*4.3		if s07c_07==2
replace yremesas_m = rem*2		    if s07c_07==3
replace yremesas_m = rem			if s07c_07==4
replace yremesas_m = rem/2			if s07c_07==5
replace yremesas_m = rem/3			if s07c_07==6
replace yremesas_m = rem/6			if s07c_07==7
replace yremesas_m = rem/12			if s07c_07==8
...
gen yremesas_nm = .
replace yremesas_nm = rem_nm*4.3		if s07c_07==2
replace yremesas_nm = rem_nm*2		    if s07c_07==3
replace yremesas_nm = rem_nm 			if s07c_07==4
replace yremesas_nm = rem_nm/2			if s07c_07==5
replace yremesas_nm = rem_nm/3			if s07c_07==6
replace yremesas_nm = rem_nm/6			if s07c_07==7
replace yremesas_nm = rem_nm/12			if s07c_07==8
...
egen double ylmpri_ci = rowtotal(yliquido ycomisio yhrsextr yprima yaguina yactpri), missing
...
egen double ylmsec_ci = rowtotal(yliquido2 yhrsextr2), missing
...
egen double ylnmsec_ci = rowtotal(yalimen2  yvivien2), missing
...
* PNC - Pensiones sociales no contributivas:
		* 1 Renta Dignidad s07a_01e== 1  $ s07a_01e0
* PTMC - Programas de transferencias monetarias condicionadas:
		* 2 Bono Juancito Pinto s05a_08== 1  $monto imputado > 0
		* 3 Bono Juana Azurduy s04b_20a1==1 |s04b_20b==1 |s04d_27a==1 $ s04b_20a2 s04b_20a1 > 0
* POTROT - Programas de otras transferencias monetarias no condicionadas
		* 4 Bono de Indigencia, Bono Personas con Discapacidad, Renta Solidaria s04a_09== 1 $ s04a_09a/12
		* 5 Bono de Natalidad s06c_28b== 1 $monto imputado > 0

*** Beneficiarios a nivel individual:
	
	// PNC
	gen byte pnc_ci = (s07a_01e == 1) if !missing(s07a_01e)

	// PTMC
	gen byte juancito_ci = (s05a_08 == 1) if !missing(s05a_08)
	gen byte juana_ci = (s04b_20a1 == 1 | s04b_20b == 1 | s04d_27a == 1)
	replace juana_ci = . if s04b_20a1 == . & s04b_20b == . & s04d_27a == .
	
	gen byte ptmc_ci = (juancito_ci == 1 | juana_ci == 1)
	replace ptmc_ci = . if juancito_ci == . & juana_ci == .
	
	// POTROT
	gen byte discap_ci = (s04a_09 == 1) if !missing(s04a_09)
	gen byte natal_ci = (s06c_28b == 1) if !missing(s06c_28b)
...
*** Montos de transferencias a nivel individual:
	...
	destring s04a_09a, gen(ing_dis)
	gen double ydiscap_ci = ing_dis/12
...
*** ylmnr_ch ***
****************
by idh_ch, sort: egen ylmnr_ch = total(ylm_ci) if miembros_ci==1, missing
...
***eduui_ci***
**************
gen byte eduui_ci = (inlist(s05a_06a, 71, 72, 76, 78, 79) | (s05a_04 == 2 & inlist(s05a_02a,  71, 72, 76, 78, 79) &  s05a_05 != 1)) 
...
gen byte eduuc_ci = ((s05a_04 == 2 & ((inlist(s05a_02a,  71, 72, 76, 78, 79) &  s05a_05 == 1) | inlist(s05a_02a, 73, 74, 75))) | inlist(s05a_06a, 73, 74, 75))
...
gen eduac_ci = . 
replace eduac_ci = 1 if (inlist(s05a_02a, 71, 72, 73, 74, 75, 76, 78, 79) | inlist(s05a_06a, 71, 72, 73, 74, 75, 76, 78, 79))
replace eduac_ci = 0 if (s05a_06a == 76) | (s05a_02a == 76)
