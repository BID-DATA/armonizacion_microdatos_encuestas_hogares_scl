...
global in = "${surveysFolder}\survey\BOL\ECH\2023\m11_m12\data_orig\"
global out = "${surveysFolder}\survey\BOL\ECH\2023\m11_m12\data_merge\"
...
Versión 2022: Carolina Rivas
Versión 2023: Jillie Chang
...
import spss using "$in\EH2023_Equipamiento.sav", case(lower)
...
compress
save "$in\eh2023_gastos_equipamiento_reshape.dta", replace


*** Gastos Alimentarios ***

clear
import spss using "$in\EH2023_Gastos_Alimentarios.sav", case(lower)
...
clear
import spss using "$in\EH2023_Persona.sav", case(lower)
*Modificación Cesar Lins - Feb 2021, data was updated by INE and some variable names changed to UPPERCASE
rename *, lower
sort folio nro
...
compress
save "$in\eh2023_persona_reshape.dta", replace


*** Gastos no Alimentarios ***
* En 2022 y 2023 está disponible este módulo
/*
clear
import spss using "$in\EH2022_Gastos no Alimentarios.sav", case(lower)
*Modificación Cesar Lins - Feb 2021, data was updated by INE and some variable names changed to UPPERCASE
rename *, lower
duplicates report folio
sort folio 
...
save "$in\eh2022_gastos_no_alimentarios_reshape.dta", replace
*/

*** Vivienda ***

clear
import spss using "$in\EH2023_Vivienda.sav", case(lower)
*Modificación Cesar Lins - Feb 2021, data was updated by INE and some variable names changed to UPPERCASE
rename *, lower
duplicates report folio
sort folio 
...
compress
save "$in\eh2023_vivienda_reshape.dta", replace
...
clear
use "$in\eh2023_persona_reshape.dta", clear

merge m:1 folio using "$in\eh2023_gastos_equipamiento_reshape.dta", force
drop _merge

merge m:1 folio using "$in\eh2023_gastos_alimentarios_reshape.dta", force
drop _merge

*merge m:1 folio using "$in\eh2023_gastos_no_alimentarios_reshape.dta", force
*drop _merge

merge m:1 folio using "$in\eh2023_vivienda_reshape.dta", force
drop _merge

compress
save "$out\BOL_2023m11_m12.dta", replace