...
*******************************************************	
*Este año no está la variable con las etiquetas de cada categoría de etnicidad, se pueden acceder acá: https://anda.ine.gob.bo/index.php/catalog/38/data-dictionary/F6?file_name=EH2014_Persona
	...
	*tab s3a_02b, m
	...
	gen byte afro_ci = .
	replace afro_ci = 1 if s3a_02b == "001"
	replace afro_ci = 0 if s3a_02b != "001"
	
	*********
	*ind_ci*
	*********
	gen byte ind_ci = .
	replace ind_ci = 1 if s3a_02b != "001" & s3a_02a == 1
	replace ind_ci = 0 if s3a_02b == "001" | s3a_02a == 2 | s3a_02a == 3
...
gen tipopen_ci=.
replace tipopen_ci=1 if s7a_01a>0 & s7a_01a!=.
replace tipopen_ci=2 if s7a_01d>0 & s7a_01d!=.
replace tipopen_ci=3 if s7a_01b>0 & s7a_01b!=.
replace tipopen_ci=4 if s7a_01c>0 & s7a_01c!=. 
replace tipopen_ci=12 if (s7a_01a>0 & s7a_01d>0) & (s7a_01a!=. & s7a_01d!=.)
replace tipopen_ci=13 if (s7a_01a>0 & s7a_01b>0) & (s7a_01a!=. & s7a_01b!=.)
replace tipopen_ci=23 if (s7a_01d>0 & s7a_01b>0) & (s7a_01d!=. & s7a_01b!=.)
replace tipopen_ci=123 if (s7a_01a>0 & s7a_01d>0 & s7a_01b>0) & (s7a_01a!=. & s7a_01d!=. & s7a_01b!=.)
...
* 1. OCUPADOS
replace condocup_ci = 1 if s6a_01 == 1                       ///
    | inrange(s6a_02,1,6)                                    ///
    | inrange(s6a_03,1,7)
...
gen tamemp_ci=.
replace tamemp_ci=1 if s6b_20>=1 & s6b_20<=5
replace tamemp_ci=2 if s6b_20>=6 & s6b_20<=49
replace tamemp_ci=3 if s6b_20>49 & s6b_20!=.
...
gen tamemp=.
replace tamemp=1 if s6b_20>=1 & s6b_20<=4
replace tamemp=2 if s6b_20>=5 & s6b_20<=14
replace tamemp=3 if s6b_20>=15 & s6b_20<=40
replace tamemp=4 if s6b_20>=41 & s6b_20!=.
...
egen aux_p=rsum(s7a_01a s7a_01b s7a_01c s7a_01d), missing
...
gen  ypensub_ci=s7a_01e_b if s7a_01e_b>1 & s7a_01e_b!=. 
...
gen aux_min= s6b_23m/60
egen horas_min= rsum(s6b_23h aux_min), m
gen horaspri_ci = horas_min*s6b_22
replace horaspri_ci=. if s6b_22==. | s6b_23h==. | s6b_23m==.
...
gen aux_min2=s6e_39bm/60
egen horas_min2=rsum(s6e_39bh aux_min2)
gen horassec_ci= horas_min2*s6e_39a
replace horassec_ci=. if s6e_39a==. | s6e_39bh==. | s6e_39bm==.
...
* 1. Patrón / empleador (incluye cooperativa)
replace categopri_ci = 1 if inlist(s6b_16, 4, 5, 6)

* 2. Cuenta propia
replace categopri_ci = 2 if s6b_16 == 3

* 3. Empleado (incluye empleada/o del hogar)
replace categopri_ci = 3 if inlist(s6b_16, 1, 2, 8)

* 4. No remunerado (trabajador familiar / aprendiz)
replace categopri_ci = 4 if s6b_16 == 7

* Solo se aplica a ocupados
replace categopri_ci = . if emp_ci != 1

label define categopri_ci 1 "Patrón" 2 "Cuenta propia" 3 "Empleado" 4 "No remunerado"
...
* 1. Patrón / empleador (incluye cooperativista)
replace categosec_ci = 1 if inlist(s6e_36, 4, 5, 6)

* 2. Cuenta propia
replace categosec_ci = 2 if s6e_36 == 3

* 3. Empleado (incluye empleada/o del hogar)
replace categosec_ci = 3 if inlist(s6e_36, 1, 2, 8)

* 4. No remunerado
replace categosec_ci = 4 if s6e_36 == 7

label define categosec_ci 1 "Patrón" 2 "Cuenta propia" 3 "Empleado" 4 "No remunerado"
label value categosec_ci categosec_ci
label var categosec_ci "Categoría ocupacional trabajo secundario"
...
gen tipocontrato_ci=.
replace tipocontrato_ci=1 if s6b_21==3 & categopri_ci==3
replace tipocontrato_ci=2 if s6b_21==1 & categopri_ci==3
*replace tipocontrato_ci=3 if ((s6_21==2 | s6_21==4) | tipocontrato_ci==.) & categopri_ci==3
*Mod. MLO 2015,10
replace tipocontrato_ci=3 if ((s6b_21==2 | s6b_21==4 | s6b_21==5) | tipocontrato_ci==.) & categopri_ci==3
...
gen categoinac_ci =1 if (s6a_09==3 & condocup_ci==3)
replace categoinac_ci = 2 if  (s6a_09==1 & condocup_ci==3)
replace categoinac_ci = 3 if  (s6a_09==2 & condocup_ci==3)
...
gen ycomisio = .
replace ycomisio= s6c_27a_1*30	if s6c_27a_2==1
replace ycomisio= s6c_27a_1*4.3	if s6c_27a_2==2
replace ycomisio= s6c_27a_1*2	if s6c_27a_2==3
replace ycomisio= s6c_27a_1		if s6c_27a_2==4
replace ycomisio= s6c_27a_1/2	if s6c_27a_2==5
replace ycomisio= s6c_27a_1/3	if s6c_27a_2==6
replace ycomisio= s6c_27a_1/6 	if s6c_27a_2==7
replace ycomisio= s6c_27a_1/12 	if s6c_27a_2==8

****************
* horas extras *
****************
gen yhrsextr= .
replace yhrsextr= s6c_27b_1 *30	    if s6c_27b_2==1
replace yhrsextr= s6c_27b_1 *4.3	if s6c_27b_2==2
replace yhrsextr= s6c_27b_1 *2		if s6c_27b_2==3
replace yhrsextr= s6c_27b_1 		if s6c_27b_2==4
replace yhrsextr= s6c_27b_1 /2		if s6c_27b_2==5
replace yhrsextr= s6c_27b_1 /3		if s6c_27b_2==6
replace yhrsextr= s6c_27b_1 /6	    if s6c_27b_2==7
replace yhrsextr= s6c_27b_1 /12	    if s6c_27b_2==8
...
*************
gen yalimen = .
replace yalimen= s6c_30a_3*30		if s6c_30a_2==1 & s6c_30a_1==1
replace yalimen= s6c_30a_3*4.3	if s6c_30a_2==2 & s6c_30a_1==1
replace yalimen= s6c_30a_3*2		if s6c_30a_2==3 & s6c_30a_1==1
replace yalimen= s6c_30a_3		if s6c_30a_2==4 & s6c_30a_1==1
replace yalimen= s6c_30a_3/2		if s6c_30a_2==5 & s6c_30a_1==1
replace yalimen= s6c_30a_3/3		if s6c_30a_2==6 & s6c_30a_1==1
replace yalimen= s6c_30a_3/6		if s6c_30a_2==7 & s6c_30a_1==1
replace yalimen= s6c_30a_3/12		if s6c_30a_2==8 & s6c_30a_1==1
...
gen ytranspo = .
replace ytranspo= s6c_30b_3*30	if s6c_30b_2==1 & s6c_30b_1==1
replace ytranspo= s6c_30b_3*4.3	if s6c_30b_2==2 & s6c_30b_1==1
replace ytranspo= s6c_30b_3*2		if s6c_30b_2==3 & s6c_30b_1==1
replace ytranspo= s6c_30b_3		if s6c_30b_2==4 & s6c_30b_1==1
replace ytranspo= s6c_30b_3/2		if s6c_30b_2==5 & s6c_30b_1==1
replace ytranspo= s6c_30b_3/3		if s6c_30b_2==6 & s6c_30b_1==1
replace ytranspo= s6c_30b_3/6		if s6c_30b_2==7 & s6c_30b_1==1
replace ytranspo= s6c_30b_3/12	if s6c_30b_2==8 & s6c_30b_1==1

**************
* vestimenta *
**************
recode s6c_30c_3 (777777=.)
gen yvesti = .
replace yvesti= s6c_30c_3*30		if s6c_30c_2==1 & s6c_30c_1==1
replace yvesti= s6c_30c_3*4.3		if s6c_30c_2==2 & s6c_30c_1==1
replace yvesti= s6c_30c_3*2		if s6c_30c_2==3 & s6c_30c_1==1
replace yvesti= s6c_30c_3			if s6c_30c_2==4 & s6c_30c_1==1
replace yvesti= s6c_30c_3/2		if s6c_30c_2==5 & s6c_30c_1==1
replace yvesti= s6c_30c_3/3		if s6c_30c_2==6 & s6c_30c_1==1
replace yvesti= s6c_30c_3/6		if s6c_30c_2==7 & s6c_30c_1==1
replace yvesti= s6c_30c_3/12		if s6c_30c_2==8 & s6c_30c_1==1
...
gen yvivien = .
replace yvivien= s6c_30d_3*30		if s6c_30d_2==1 & s6c_30d_1==1
replace yvivien= s6c_30d_3*4.3	    if s6c_30d_2==2 & s6c_30d_1==1
replace yvivien= s6c_30d_3*2		if s6c_30d_2==3 & s6c_30d_1==1
replace yvivien= s6c_30d_3		    if s6c_30d_2==4 & s6c_30d_1==1
replace yvivien= s6c_30d_3/2		if s6c_30d_2==5 & s6c_30d_1==1
replace yvivien= s6c_30d_3/3		if s6c_30d_2==6 & s6c_30d_1==1
replace yvivien= s6c_30d_3/6		if s6c_30d_2==7 & s6c_30d_1==1
replace yvivien= s6c_30d_3/12		if s6c_30d_2==8 & s6c_30d_1==1
...
gen yotros = .
replace yotros= s6c_30e_3*30		if s6c_30e_2==1 & s6c_30e_1==1
replace yotros= s6c_30e_3*4.3	    if s6c_30e_2==2 & s6c_30e_1==1
replace yotros= s6c_30e_3*2	    if s6c_30e_2==3 & s6c_30e_1==1
replace yotros= s6c_30e_3		    if s6c_30e_2==4 & s6c_30e_1==1
replace yotros= s6c_30e_3/2	    if s6c_30e_2==5 & s6c_30e_1==1
replace yotros= s6c_30e_3/3		if s6c_30e_2==6 & s6c_30e_1==1
replace yotros= s6c_30e_3/6		if s6c_30e_2==7 & s6c_30e_1==1
replace yotros= s6c_30e_3/12		if s6c_30e_2==8 & s6c_30e_1==1
...
gen yhrsextr2 = .
replace yhrsextr2=s6f_42a_2/12 if s6f_42a_1==1
...
gen yalimen2 = .
replace yalimen2= s6f_42b_2/12	if s6f_42b_1==1
...
gen yvivien2= .
replace yvivien2= s6f_42c_2/12	if s6f_42c_1==1
...
/*
  ...
  8  anual
*/
* No hay la categoria de diario en s7b_05a_2
gen yasistfam = .
replace yasistfam= s7b_05a_1*4.3	if s7b_05a_2==2
replace yasistfam= s7b_05a_1*2		if s7b_05a_2==3
replace yasistfam= s7b_05a_1		if s7b_05a_2==4
replace yasistfam= s7b_05a_1/2		if s7b_05a_2==5
replace yasistfam= s7b_05a_1/3		if s7b_05a_2==6
replace yasistfam= s7b_05a_1/6		if s7b_05a_2==7
replace yasistfam= s7b_05a_1/12		if s7b_05a_2==8


*********************
* Trans. monetarias *
*********************
* No hay la categoria de diario en s7b_05b_2

gen ytransmon = .
replace ytransmon= s7b_05b_1*4.3	if s7b_05b_2==2
replace ytransmon= s7b_05b_1*2		if s7b_05b_2==3
replace ytransmon= s7b_05b_1		if s7b_05b_2==4
replace ytransmon= s7b_05b_1/2		if s7b_05b_2==5
replace ytransmon= s7b_05b_1/3		if s7b_05b_2==6
replace ytransmon= s7b_05b_1/6		if s7b_05b_2==7
replace ytransmon= s7b_05b_1/12		if s7b_05b_2==8
...
replace s6_112 =  s7c_08a*9.45281   if s7c_08b== "B" /*euro*/
replace s6_112 =  s7c_08a*6.86		if s7c_08b== "C" /*dolar*/
replace s6_112 =  s7c_08a*1.05220   if s7c_08b== "D" /*peso argentino*/
replace s6_112 =  s7c_08a*2.90407   if s7c_08b== "E" /*real*/
replace s6_112 =  s7c_08a*0.01306	if s7c_08b== "F" /*peso chileno*/
replace s6_112 =  s7c_08a*2.45289   if s7c_08b== "G" /*soles*/
...
replace aedu_ci = 0 if s5a_02==11 | s5a_02==12 | s5a_02==13 | s5a_02==81

* Primaria & Secundaria
* Sistema escolar antiguo
replace aedu_ci = s5a_02a if s5a_02==21
replace aedu_ci = s5a_02a+5 if s5a_02==22
replace aedu_ci = s5a_02a+8 if s5a_02==23
* Sistema escolar anterior
replace aedu_ci = s5a_02a if s5a_02==31
replace aedu_ci = s5a_02a+8 if s5a_02==32
* Sistema escolar actual
replace aedu_ci = s5a_02a if s5a_02==41
replace aedu_ci = s5a_02a+6 if s5a_02==42

* Superior
replace aedu_ci = s5a_02a+12 if (s5a_02>=71 & s5a_02<=73) | (s5a_02>=77 & s5a_02<=79)
replace aedu_ci = s5a_02a+17 if (s5a_02>=74 & s5a_02<=76)
...
* Ninguno o preescolar
replace aedu_ci = 0 if s5a_02==11 | s5a_02==12 | s5a_02==13 

*Primaria & Secundaria
replace aedu_ci = 1 if s5a_02a==1 & (s5a_02==21 | s5a_02==31 | s5a_02==41)
replace aedu_ci = 2 if s5a_02a==2 & (s5a_02==21 | s5a_02==31 | s5a_02==41)
replace aedu_ci = 3 if s5a_02a==3 & (s5a_02==21 | s5a_02==31 | s5a_02==41)
replace aedu_ci = 4 if s5a_02a==4 & (s5a_02==21 | s5a_02==31 | s5a_02==41)
replace aedu_ci = 5 if s5a_02a==5 & (s5a_02==21 | s5a_02==31 | s5a_02==41)
replace aedu_ci = 6 if (s5a_02a==6 & (s5a_02==31 | s5a_02==41)) |  (s5a_02a==1 & s5a_02==22)
replace aedu_ci = 7 if (s5a_02a==7 & s5a_02==31) |  (s5a_02a==2 & s5a_02==22) | (s5a_02a==1 & s5a_02==42) 
replace aedu_ci = 8 if (s5a_02a==8 & s5a_02==31) |  (s5a_02a==3 & s5a_02==22) | (s5a_02a==2 & s5a_02==42)
replace aedu_ci = 9 if (s5a_02a==1 & s5a_02==23) |  (s5a_02a==1 & s5a_02==32) | (s5a_02a==3 & s5a_02==42)
replace aedu_ci = 10 if (s5a_02a==2 & s5a_02==23) |  (s5a_02a==2 & s5a_02==32) | (s5a_02a==4 & s5a_02==42)
...
*pqnoasis1_ci*
**************
gen pqnoasis1_ci = 1 if s5b_11==2
replace pqnoasis1_ci = 2 if s5b_11==3
replace pqnoasis1_ci = 3 if s5b_11==4 
replace pqnoasis1_ci = 4 if s5b_11==8
replace pqnoasis1_ci = 5 if s5b_11==9 
replace pqnoasis1_ci = 6 if s5b_11==6 
replace pqnoasis1_ci = 7 if s5b_11==7  
replace pqnoasis1_ci = 8 if s5b_11==5
...
****************
  gen aguared_ch = 0
  replace aguared_ch = 1 if (s1a_09==1 | s1a_09==2)
  replace aguared = . if s1a_09==.
  ...
  *****************
  ...
  replace aguafconsumo_ch = 1 if s1a_09==1|s1a_09==2
  replace aguafconsumo_ch = 2 if s1a_09==3
  replace aguafconsumo_ch = 3 if s1a_09==9
  replace aguafconsumo_ch = 4 if (s1a_09==4 | s1a_09==5)
  replace aguafconsumo_ch = 6 if s1a_09==10
  replace aguafconsumo_ch = 7 if s1a_09 == 7
  replace aguafconsumo_ch = 8 if s1a_09==8
  replace aguafconsumo_ch = 9 if s1a_09==6
  replace aguafconsumo_ch = 10 if s1a_09== 11 
  
  *****************
  *aguafuente_ch*
  *****************
  gen aguafuente_ch = 0
  replace aguafuente_ch = 1 if s1a_09==1|s1a_09==2
  replace aguafuente_ch = 2 if s1a_09==3
  replace aguafuente_ch = 3 if s1a_09==9
  replace aguafuente_ch = 4 if (s1a_09==4 | s1a_09==5)
  replace aguafuente_ch = 6 if s1a_09==10
  replace aguafuente_ch = 7 if s1a_09 == 7
  replace aguafuente_ch = 8 if s1a_09==8
  replace aguafuente_ch = 9 if s1a_09==6
  replace aguafuente_ch = 10 if s1a_09== 11 
  ...
  *************
  gen aguadist_ch=0
  replace aguadist_ch=1 if s1a_09==1
  replace aguadist_ch=2 if s1a_09==2
  replace aguadist_ch=3 if (s1a_09==3)
...
replace aguamejorada_ch = 1 if aguafuente_ch<=7
  ...
  *****************
    gen bano_ch=6
  replace bano_ch=0 if s1a_11==6 
  replace bano_ch=1 if s1a_11==1 & s1a_12==1
  replace bano_ch=2 if s1a_11==1 & s1a_12==2
  replace bano_ch=3 if ((s1a_11==2 | s1a_11 == 4) & s1a_12!=4) | (  s1a_11==1 & s1a_12 == 3)
  replace bano_ch=4 if (s1a_11==1 |s1a_11==2 |s1a_11==3) & s1a_12==4
  replace bano_ch=5 if s1a_11 ==3 & s1a_12!=4
  replace bano_ch=6 if s1a_11 ==5 
  ***************
  ***banoex_ch***
  ***************
  gen banoex_ch =.
  replace banoex_ch = 0 if s1a_13==2
  replace banoex_ch = 1 if s1a_13==1
...
replace banomejorado_ch =0 if (bano_ch ==0 | bano_ch>=4) & bano_ch!=6
  ...
  ************
  gen sinbano_ch = 3
  replace sinbano_ch = 0 if s1a_11!=6
...
gen combust_ch= (s1a_20==4 | s1a_20==6)
...
replace des1_ch=1 if s1a_12==1 | s1a_12==2
replace des1_ch=2 if s1a_12==3
replace des1_ch=3 if s1a_12==4
...
replace des2_ch=1 if s1a_12==1 | s1a_12==2 | s1a_12==3 
replace des2_ch=2 if s1a_12==4 | s1a_12==5
...
gen piso_ch=0 if s1a_08==1 
replace piso_ch=1 if  s1a_08>=2 &  s1a_08<=7 
replace piso_ch=2 if  s1a_08==8
...
gen pared_ch=0 if s1a_05==6
replace pared_ch=1 if s1a_05>=1 & s1a_05<=5
replace pared_ch=2 if s1a_05==7
...
gen techo_ch=0 if s1a_07==4
replace techo_ch=1 if s1a_07>=1 & s1a_07<=3
replace techo_ch=2 if s1a_07==5
...
gen resid_ch =0 if s1a_16==5 | s1a_16==6
replace resid_ch=1 if s1a_16==2 | s1a_16==4
replace resid_ch=2 if s1a_16==1 | s1a_16==3
replace resid_ch=3 if s1a_16==7
replace resid_ch=. if s1a_16==.
...
gen cocina_ch=(s1a_19==1)
replace cocina_ch = . if  s1a_19==.
...
gen telef_ch=(s1a_24==1)
replace telef_ch = . if s1a_24==.
...
gen pnc_ci= (s7a_01e_a==1)

* Monto pension no contributiva
bys idh_ch: egen ing_pension = sum(s7a_01e_b)
...
rename s6b_11a codocupa
rename s6b_11ad codindustria
