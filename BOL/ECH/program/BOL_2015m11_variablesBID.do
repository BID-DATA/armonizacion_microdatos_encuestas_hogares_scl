...
*******************************************************	
	...
	*tab s3a_2bcod, m
	...
	gen byte afro_ci = .
	replace afro_ci = 1 if s3a_2bcod == "001" 
	replace afro_ci = 0 if s3a_2bcod != "001" & s3a_2a != .
	
	*********
	*ind_ci*
	*********
	gen byte ind_ci = .
	replace ind_ci = 1 if s3a_2bcod != "001" & s3a_2a == 1
	replace ind_ci = 0 if s3a_2bcod == "001" | s3a_2a == 2  | s3a_2a == 3
...
****************************
	...
	**************
	gen byte pension_ci=. 
	replace pension_ci=1 if s7a_1a>0 | s7a_1b>0 | s7a_1c>0 |  s7a_1d>0 
	recode pension_ci .=0 
	...
	**pensionsub_ci**
	***************
	gen byte pensionsub_ci=1 if s7a_1ea==1 
	recode pensionsub_ci .=0 if s7a_1ea==2 
...
egen ypen_ci=rsum(s7a_1a s7a_1b  s7a_1c s7a_1d  ), missing 
...
gen categoinac_ci = .
replace categoinac_ci = 1 if (s6a_09a==3 & condocup_ci==3) // Jubilados o pensionados
replace categoinac_ci = 2 if (s6a_09a==1 & condocup_ci==3) // Estudiantes
replace categoinac_ci = 3 if (s6a_09a==2 & condocup_ci==3) // Quehaceres domésticos
...
gen asiste_ci = .
replace asiste_ci = 0 if s5a_4 == 2  
replace asiste_ci = 1 if s5a_4 == 1 
...
* Variables incluidas por SCL/MIG Juan Camilo Perdomo
	...
	**********************
	...
	gen migrantiguo5_ci = 1 if inlist(s3a_1a,1,2) & migrante_ci==1 
	replace migrantiguo5_ci = 0 if s3a_1a == 3 & migrante_ci==1 
	replace migrantiguo5_ci = . if s3a_1a == 4 | migrante_ci!=1 
...
gen pnc_ci= (s7a_1ea==1)

* Monto pension no contributiva
bys idh_ch: egen ing_pension = sum(s7a_1eb)
