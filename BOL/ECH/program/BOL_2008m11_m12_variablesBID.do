...
* MGR Febrero 2016: id del inviduo se creó con la variable equivocada
*gen idp_ci= folio
...
gen region_c=.
/*
gen region_c=1 		if prov=="Andrés Ibañez"
replace region_c=2	if prov=="Cercado"
replace region_c=3	if prov=="Murillo"
replace region_c=4	if prov=="Nicolás Suárez"
replace region_c=5	if prov=="Oropeza"
replace region_c=6	if prov=="Tomas Frias"
label var region_c "Region" 
label def region_c 1"Andrés Ibañez" 2"Cercado" 3"Murillo" 4"Nicolás Suárez" 5"Oropeza" 6"Tomas Frias"
label val region_c region_c*/
...
** LCM: Estaba mal tomada la variable de referencia, se cambia s1_13 por s1_11 LCM dic2013
...
*******************************************************
	...
	*********	
	gen byte ind_ci = .
	replace ind_ci = 1 if inrange(s1_12, 1,6) 
	replace ind_ci = 0 if s1_12 == 7
...
****************************
	...
	***horaspri_ci***
	***************	
	gen horas1=s5_29b/60
	
	egen horaspri_ci=rsum(s5_29a horas1), missing
	replace horaspri_ci=. if s5_29a==. & horas1==.
	...
	***************	
	...
	replace afiliado_ci  = 0 if  s5_58b==2
	replace afiliado_ci  = 1 if  s5_58b==1		
...
gen ypensub_ci=s6_01eb if s6_01eb>1 & s6_01eb!=.
...
ACTIV_DE_HOG_PRIV_Y_ACTIV_NO DIF_DE_ HO 15
   SERV_DE_ORG_Y_ÓRG_EXTRATERRITORIALES 16
                                  Ns/Nr 99
*/
* Indicador corregido: la especificacion de rama en la variable caeb_ag inicia en 0 y no en 1. MGD 04/08/2014
...
* transporte *
**************
* No hay las actegorias semestral y anual (7 y 8).
gen ytranspo = .
...
* como no esta la variable frecuencia (s6_01ef) en la base supongo que es mensual (renta dignidad)
*este año estan mensuales en un principio se pagaba anualmente.
...
* remesas *
***********
*http://www.bcb.gob.bo/librerias/indicadores/otras/otras_imprimir2.php?qdd=3&qmm=11&qaa=2008
gen s6_112= .
replace s6_112 = s6_11 				if s6_11a==1
replace s6_112 = s6_11*8.87028		if s6_11a==2 /*euro*/
replace s6_112 = s6_11*7.07			if s6_11a==3 /*dolar*/
replace s6_112 = s6_11*2.05817		if s6_11a==4 /*peso argentino*/
replace s6_112 = s6_11*3.22835		if s6_11a==5 /*real*/
replace s6_112 = s6_11*.01039		if s6_11a==6 /*peso chileno*/
 ...
 *No se consideran por no ser educación formal sino "Alternativa" o "No formal"
		  ...
		  35 otros cursos (menor a 1 anio)
...
gen aedu_ci = .
replace aedu_ci = 0 if inlist(s4_03a, 11, 12, 13) // Ninguno, alfabetización, preescolar
replace aedu_ci = s4_03b if inlist(s4_03a, 14, 17) // Básico, primaria
replace aedu_ci = s4_03b + 5 if s4_03a == 15 // Intermedio
replace aedu_ci = s4_03b + 8 if inlist(s4_03a, 16, 18) // Secundaria 
replace aedu_ci = 12 if s4_03a == 35 // Imputación Otros curos

// Para educación Superior el codigo 5 u 8 indica egreso o titulación respectivamente

// Educación Superior:
replace aedu_ci = s4_03b + 12 if inlist(s4_03a, 26, 32, 33, 27, 28, 34) // Superior hasta 4 anios
replace aedu_ci = 17 if inlist(s4_03a, 26, 32, 33, 27, 28, 34) & inlist(s4_03b, 5, 8) // Egreso, titulación

*postgrado (1 anio)
replace aedu_ci = 17 if s4_03a == 29 & s4_03b == 1 // cursando
replace aedu_ci = 18 if s4_03a == 29  & s4_03b == 8 // terminado

*maestria (2 anios)
replace aedu_ci = 18 if s4_03a == 30 & s4_03b == 1 // 2do o 3er semestre aprobado
replace aedu_ci = 19 if s4_03a == 30  & s4_03b == 2 // 4to semestre aprobado
replace aedu_ci = 19 if s4_03a == 30  & s4_03b >= 5 & s4_03b <= 8 // egresado o titulado

*doctorado (4 anios)
replace aedu_ci = 20 if s4_03a == 31 & s4_03b == 1 // 2do o 3er semestre aprobado
replace aedu_ci = 21 if s4_03a == 31 & s4_03b == 2 // 4to o 5to semestre aprobado
replace aedu_ci = 22 if s4_03a == 31 & s4_03b == 3 // 6to o 7mo semestre aprobado
replace aedu_ci = 23 if s4_03a == 31 & s4_03b == 4 // 8vo semestre aprobado
replace aedu_ci = 23 if s4_03a == 31 & s4_03b >= 5 & s4_03b <= 8 // egresado o titulado
...
gen byte eduui_ci = (inlist(s4_06a, 26, 27, 28, 32, 34) | (s4_05 == 2 & (inlist(s4_03a, 26, 27, 28, 32, 34)) & s4_14 != 6))
...
gen byte eduuc_ci =  ((s4_05 == 2 & ((inlist(s4_03a, 26, 27, 28, 32, 34)) & s4_14 == 6 | inlist(s4_03a, 29, 30, 31)))| inlist(s4_06a, 29, 30, 31))
...
gen eduac_ci = . 
replace eduac_ci = 1 if (inlist(s4_03a, 26, 27, 28, 29, 30, 31, 34) | inlist(s4_06a, 26, 27, 28, 29, 30, 31, 34))
replace eduac_ci = 0 if (inlist(s4_03a, 32) | inlist(s4_06a, 32))
replace eduac_ci = . if (inlist(s4_06a, 33, 35) & s4_06a >= 24)
...
*pqnoasis1_ci*
**************
gen  pqnoasis1_ci = 1 if s4_14==2
replace pqnoasis1_ci = 2 if s4_14==3
replace pqnoasis1_ci = 3 if s4_14==4 
replace pqnoasis1_ci = 4 if s4_14==8
replace pqnoasis1_ci = 5 if s4_14==9 
replace pqnoasis1_ci = 6 if s4_14==6 
replace pqnoasis1_ci = 7 if s4_14==7  
replace pqnoasis1_ci = 8 if s4_14==5
replace pqnoasis1_ci = 9 if s4_14==1  | s4_14==10
...
gen edupub_ci = (s4_07 == 1 | s4_07 == 2)
replace edupub_ci = 0 if s4_07 == 3
...
**************
gen aguadisp1_ch = .
replace aguadisp1 = 1 if s8_12 ==1
replace aguadisp1 = 0 if s8_12 ==2
...
*****************
gen bano_ch=6
replace bano_ch=0 if s8_14==2 
replace bano_ch=1 if s8_14==1 & s8_16 ==1
replace bano_ch=2 if s8_14==1 & s8_16 ==2
replace bano_ch=3 if s8_14==1 & s8_16 ==3
replace bano_ch=4 if s8_14==1 & s8_16 ==4
***************
***banoex_ch***
***************
gen banoex_ch =.
replace banoex_ch = 0 if s8_15==2
replace banoex_ch = 1 if s8_15==1
...
************
gen sinbano_ch = 3
replace sinbano_ch = 0 if s8_14==1
...
gen des1_ch=.
replace des1_ch=0 if bano_ch==0
replace des1_ch=1 if s8_16==1 | s8_16==2
replace des1_ch=2 if s8_16==3
replace des1_ch=3 if s8_16==4
label var des1_ch "Tipo de desague según unimproved de MDG"
label def des1_ch 0"No tiene servicio sanitario" 1"Conectado a red general o cámara séptica"
label def des1_ch 2"Letrina o conectado a pozo ciego" 3"Desemboca en río o calle", add
label val des1_ch des1_ch
...
gen des2_ch=.
replace des2_ch=0 if bano_ch==0
replace des2_ch=1 if s8_16==1 | s8_16==2 | s8_16==3 
replace des2_ch=2 if s8_16==4
label var des2_ch "Tipo de desague sin incluir definición MDG"
label def des2_ch 0"No tiene servicio sanitario" 1"Conectado a red general, cámara séptica, pozo o letrina"
label def des2_ch 2"Cualquier otro caso", add
label val des2_ch des2_ch
...
***internet_ch***
*****************
gen internet_ch=(s8_29==1)
replace internet_ch = .   if  s8_29 == .
...
* Variables incluidas por SCL/MIG Fernando Morales
	...
	**********************
	...
	gen migantiguo5_ci=(migrante_ci==1 & inlist(s2_02,1,2)) if s2_02!=4 & migrante_ci!=.
...
* Variables incluidas por SCL/MIG Juan Camilo Perdomo
	...
	**********************
	...
	gen migrantiguo5_ci = 1 if inlist(s2_02,1,2) & migrante_ci==1 
	replace migrantiguo5_ci = 0 if s2_02 == 3 & migrante_ci==1 
	replace migrantiguo5_ci = . if s2_02 == 4 | migrante_ci!=1 
