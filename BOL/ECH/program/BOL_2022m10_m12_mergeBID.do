...
import spss using "$in\EH2022_Equipamiento.sav", case(lower)
...
compress
save "$in\eh2022_gastos_equipamiento_reshape.dta", replace


*** Gastos Alimentarios ***

clear
import spss using "$in\EH2022_Gastos Alimentarios.sav", case(lower)
...
compress
save "$in\eh2022_gastos_alimentarios_reshape.dta", replace


*** Persona ***

clear
import spss using "$in\EH2022_Persona.sav", case(lower)
*Modificación Cesar Lins - Feb 2021, data was updated by INE and some variable names changed to UPPERCASE
rename *, lower
sort folio nro
...
compress
save "$in\eh2022_persona_reshape.dta", replace


*** Gastos no Alimentarios ***
* En 2022 está disponible este módulo
/*
clear
...
clear
import spss using "$in\EH2022_Vivienda.sav", case(lower)
*Modificación Cesar Lins - Feb 2021, data was updated by INE and some variable names changed to UPPERCASE
rename *, lower
duplicates report folio
sort folio 
...
compress
save "$in\eh2022_vivienda_reshape.dta", replace
...
clear
use "$in\eh2022_persona_reshape.dta", clear

merge m:1 folio using "$in\eh2022_gastos_equipamiento_reshape.dta", force
drop _merge

merge m:1 folio using "$in\eh2022_gastos_alimentarios_reshape.dta", force
drop _merge

*merge m:1 folio using "$in\eh2022_gastos_no_alimentarios_reshape.dta", force
*drop _merge

merge m:1 folio using "$in\eh2022_vivienda_reshape.dta", force
drop _merge
