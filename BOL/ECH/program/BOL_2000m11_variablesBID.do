...
****************************
	...
	***horaspri_ci***
	***************	
	gen  byte horaspri_ci = hrstrap*diastrap if hrstrap!=0
	replace horaspri_ci = . if emp_ci!=1
	
	***************
	***horastot_ci ***
	***************	
	gen horassec=hrstras*diastras if hrstr!=0
	egen  byte horastot_ci  = rsum(horaspri_ci horassec), missing
	...
	***************	
	...
	replace afiliado_ci  = 0 if   afilafp ==2
	replace afiliado_ci  = 1 if   afilafp ==1		
...
* Modificacion MGD 07/24/2014: clasificacion CIUO -88
	...
	gen byte ocupa_ci=.
	replace ocupa_ci=1 if (ocupp>=210 & ocupp<=348)  & emp_ci==1
	replace ocupa_ci=2 if (ocupp>=110 & ocupp<=131)  & emp_ci==1
	replace ocupa_ci=3 if (ocupp>=410 & ocupp<=422) & emp_ci==1
	replace ocupa_ci=4 if ((ocupp>=520 & ocupp<=529) | (ocupp>=910 & ocupp<=911)) & emp_ci==1
	replace ocupa_ci=5 if ((ocupp>=510 & ocupp<=519) | (ocupp>=912 & ocupp<=916)) & emp_ci==1
	replace ocupa_ci=6 if ((ocupp>=610 & ocupp<=621) | (ocupp>=920 & ocupp<=921)) & emp_ci==1
	replace ocupa_ci=7 if ((ocupp>=710 & ocupp<=851) | (ocupp>=930 & ocupp<=933))& emp_ci==1
	...
	**************
	gen byte pension_ci=. 
	replace pension_ci=1 if ingjub>0 | ingben>0 | inginv>0 |  ingviu>0 |  ingorf>0
	recode pension_ci .=0 
...
replace lp_ci= 335.50 if depto==1 & zona_c==1
replace lp_ci= 325.10 if depto==2 & zona_c==1
replace lp_ci= 351.20 if depto==3 & zona_c==1
replace lp_ci= 295.60 if depto==4 & zona_c==1
replace lp_ci= 271.90 if depto==5 & zona_c==1
replace lp_ci= 351.20 if depto==6 & zona_c==1
replace lp_ci= 353.90 if depto==7 & zona_c==1
replace lp_ci= 353.90 if depto==8 & zona_c==1
replace lp_ci= 268.80 if depto==9 & zona_c==1
*replace lp_ci= 353.90 if  depto==10 & zona_c==1
                                

replace lp_ci=    231.60     if  zona_c==0
...
replace lpe_ci= 169.40 if depto==1 & zona_c==1
replace lpe_ci= 180.70 if depto==2 & zona_c==1
replace lpe_ci= 177.40 if depto==3 & zona_c==1
replace lpe_ci= 164.40 if depto==4 & zona_c==1
replace lpe_ci= 151.20 if depto==5 & zona_c==1
replace lpe_ci= 177.40 if depto==6 & zona_c==1
replace lpe_ci= 179.80 if depto==7 & zona_c==1
replace lpe_ci= 179.80 if depto==8 & zona_c==1
replace lpe_ci= 163.10 if depto==9 & zona_c==1
*replace lpe_ci= 179.80 if  depto==10 & zona_c==1
                                
replace lpe_ci=     131.60      if  zona_c==0
...
*replace condocup_ci=2 if (trabajo==2 | sondeo1==7 | sondeo2>1) & (bustrab==1 | bus4sem==1) & (queriatr==1)
replace condocup_ci=2 if (trabajo==2 | sondeo1==7 | sondeo2>7) & (bustrab==1 | bus4sem==1) & (queriatr==1)
...
replace rama_ci=9 if (ramap>=7111 & ramap<=9900) & emp_ci==1
...
egen transltot=rsum(transl1 transl2), missing
replace transltot=. if transl1==. & transl2==. 


*Rentas mensuales

local jjj="inginte ingalqui ingjub ingben inginv ingviu ingorf ingotro"
foreach i of local jjj {
replace `i'=. if `i'==0
}

egen rentastot=rsum(inginte ingalqui ingjub ingben inginv ingviu ingorf ingotro), missing
replace rentastot=. if inginte==. & ingalqui==. & ingjub==. & ingben==. & inginv==. & ingviu==. & ingorf==. & ingotro==. 
...
replace rentaimp_ch=rentaimp_ch*6.17 if malimp==2
...
replace aedu_ci=1 if (nivel==14 | nivel==17) & curso==1
replace aedu_ci=2 if (nivel==14 | nivel==17) & curso==2
replace aedu_ci=3 if (nivel==14 | nivel==17) & curso==3
replace aedu_ci=4 if (nivel==14 | nivel==17) & curso==4
replace aedu_ci=5 if (nivel==14 | nivel==17) & curso==5

replace aedu_ci=6 if (nivel==17 & curso==6) | (nivel==15 & curso==1)
replace aedu_ci=7 if (nivel==17 & curso==7) | (nivel==15 & curso==2)
replace aedu_ci=8 if (nivel==17 & curso==8) | (nivel==15 & curso==3)

replace aedu_ci=9 if (nivel==16 | nivel==18) & curso==1
replace aedu_ci=10 if (nivel==16 | nivel==18) & curso==2
replace aedu_ci=11 if (nivel==16 | nivel==18) & curso==3
replace aedu_ci=12 if (nivel==16 | nivel==18) & curso==4
replace aedu_ci=12 if nivel==28 & curso==0

replace aedu_ci=13 if (nivel>=22 & nivel<=27 & nivel~=24) & curso==1 
replace aedu_ci=14 if (nivel>=22 & nivel<=27 & nivel~=24) & curso==2
replace aedu_ci=15 if (nivel>=22 & nivel<=27 & nivel~=24) & curso==3
replace aedu_ci=16 if (nivel>=22 & nivel<=27 & nivel~=24) & curso==4

replace aedu_ci=16 if (nivel==22 | nivel==25 | nivel==26) & curso==5
replace aedu_ci=17 if (nivel==23 | nivel==27) & curso==5

replace aedu_ci=16 if (nivel==22 | nivel==25 | nivel==26) & curso==8
replace aedu_ci=17 if (nivel==23 | nivel==27) & curso==8

replace aedu_ci=18 if nivel==24 & curso==1
replace aedu_ci=19 if nivel==24 & curso==2
replace aedu_ci=21 if nivel==24 & curso==4
...
gen byte eduui_ci=0
replace eduui_ci=1 if (nivel==22 | nivel==25 | nivel==26) & (curso>=1 & curso<=3)
replace eduui_ci=1 if (nivel==23 | nivel==27) & (curso>=1 & curso<=4)
...
gen byte eduuc_ci=0
replace eduuc_ci=1 if (nivel==22 | nivel==25 | nivel==26) & (curso>=4 & curso<=8)
replace eduuc_ci=1 if (nivel==23 | nivel==27) & (curso>=5 & curso<=8)
replace eduuc_ci=1 if nivel==24
...
**************
gen byte eduac_ci=.
replace eduac_ci=1 if (nivel==23 | nivel==24)
replace eduac_ci=0 if (nivel==22 | (nivel>=25 & nivel<=27))
...
* Modificacion Marcela Rubio Septiembre 2014: correcion pequeno error en la sintaxis
gen des1_ch=.
...
replace des2_ch=1 if desague==1 | desague==2 | desague==3
replace des2_ch=2 if desague==4
...
replace vivialq_ch=vivialq_ch*6.17 if malqviv==2
replace vivialq_ch=. if alqviv==0
	...
	***********
	gen vivialqimp_ch=.
	replace vivialqimp_ch=alimp if alimp!=0
	replace vivialqimp_ch=vivialqimp_ch*6.17 if malimp==2 // para los alquileres que estan en US
...
* Variables incluidas por SCL/MIG Fernando Morales
	...
	*******************
	...
	gen migrante_ci=(dondenac==3) if dondenac!=. 	
