...
/* al construir este nuevo programa para las bases nuevas enviadas por DANE
se cambió la definición de los miembros del hogar y, con esta nueva definicion, se calcularon nuevamente 
las variables relativas a la composicion demografica del hogar.
CODIGO ANTERIOR:
1.gen miembros_ci=(relacion_ci>=1 & relacion_ci<=4) 
*se excluian a los no parientes*
2.by idh_ch, sort: egen nmayor21_ch=sum((relacion_ci>=1 & relacion_ci<=4) & edad>=21)
segun el diccionario de las variables armonizadas, estas variables hacen referencia a
la composicion demografica del hogar y NO de los familiares como se suguiere aca
*/
...
recode pension1 (999999=.) (999998=.)
recode pension2 (999999=.) (999998=.)
...
gen desemp2_ci=.
* el periodo tambien es una semana solo que se*
* incluye a quienes esperan respuesta de un trabajo*
...
replace yprid=. if (ca_30_T0601=="999999" | ca_30_T0601=="999998")

*incluye propinas y comisiones y excluye viaticos y pagos en especie*
...
replace yprid1=. if (ca_32_T06=="999999" | ca_32_T06=="999998")
...
*como no se puede identificar los ingresos de la actividad principal porque no estan discriminados *
*de esa manera, se creará una variable similar para identificar la no respuesta para ingresos *
*laborales generales*
...
/*gen ylnm_ci=real(ca_31_T0602) if emp_ci==1 & ca_31_T0601=="1"
replace ylnm_ci=. if (ca_31_T0602=="99999" | ca_31_T0602=="99998")
99998 no sabe, 99999 no informa*/
...
replace temo=. if (ca_33_T0602=="999999" | ca_33_T0602=="999998")
...
***  nrylm_ch  ***
******************
*esta variable no se encuentra en el listado se crea para generar el ingreso laboral del hogar 
*que no incluye las no respuestas, este ingreso es la alternativa al construido con ylmpri_ci*
...
replace yedc=0 if (yedc1==100 | yedc1==200) 
/*100 ninguno, 200 es primaria y ningun anio terminado*/

*** primaria 
replace yedc=1 if yedc1==201
replace yedc=2 if yedc1==202
replace yedc=3 if yedc1==203 
replace yedc=4 if yedc1==204 
replace yedc=5 if (yedc1==205 | yedc1==300) 

*** secundaria 
replace yedc=6 if yedc1==306 
replace yedc=7 if yedc1==307 
replace yedc=8 if yedc1==308 
replace yedc=9 if yedc1==309 
replace yedc=10 if yedc1==3010
replace yedc=11 if (yedc1==311 | yedc1==400) 

*** superior o universitario  *** 
replace yedc=12 if yedc1==401 
replace yedc=13 if yedc1==402 
replace yedc=14 if yedc1==403
replace yedc=15 if yedc1==404 
replace yedc=16 if yedc1==405 
replace yedc=17 if yedc1==406
replace yedc=18 if yedc1==407
...
gen aguared_ch=(ca_4_T00=="1" | ca_4_T00=="2")
...
gen aguamala_ch=.
replace aguamala_ch=(ca_4_T00 !="1" & ca_4_T00 !="2")
/*rio o manantial, pozo o algibe, pila publica, carrotanque, agua lluvia, otra*/
...
gen luz_ch=(ca_1_T003 =="1")
*1 si la vivienda tiene conexion a energia electrica*
*ver pregunta 1 de servicios publicos*
...
*La opción elegida no corresponde a la que se necesita. Se deja la programación inicial y se corrige abajo
*gen combust_ch=(ca_5_T001 =="1")

gen combust_ch=(ca_5_T001 =="2" | ca_5_T001 =="6")
...
g       aguamejorada_ch = 1 if (ca_4_T00 >=1 & ca_4_T00 <=2) | (ca_4_T00 >=4 & ca_4_T00 <=5) | ca_4_T00==7
replace aguamejorada_ch = 0 if  ca_4_T00==3 | ca_4_T00==6 | ca_4_T00==8
...
**********
gen sexo_ci=.
replace sexo_ci=1 if re_4_T01 =="1"
replace sexo_ci=2 if re_4_T01 =="2"
label define sexo_ci 1 "Hombre" 2 "Mujer"
label value sexo_ci sexo_ci
...
*****************
gen civil_ci=.
replace civil_ci=1 if re_6_T01=="5"
replace civil_ci=2 if re_6_T01=="1" | re_6_T01=="3" 
replace civil_ci=3 if re_6_T01=="2"
replace civil_ci=4 if re_6_T01=="4" 
...
replace trab=2 if (re_19_T03>="2" & re_19_T03<="8") & re_24_T03=="1"
...
gen asiste_ci=.
replace asiste_ci=1 if re_9_T01=="1"
replace asiste_ci=0 if re_9_T01=="2"
label variable asiste_ci "Asiste actualmente a la escuela"
...
gen pqnoasis_ci=real(re_18A_T0102)
*no se puede sacar para urbano por el problema de SUBTIPO*
...
gen edupub_ci=.
/*gen edupub_ci=(re_15_T01=="1")
*no se copnstruye porque no se puede construir en urbano por el problema de SUBTIPO*/
...
gen aguared_ch=(re_4_T00=="1" | re_4_T00=="2")
...
gen aguamala_ch=.
replace aguamala_ch=(re_4_T00 !="1" & re_4_T00 !="2")
...
gen luz_ch=(re_1i_T003 =="1")
*Se construyó como=1 si la vivienda tiene conexion a energía eléctrica*
...
gen combust_ch=.
replace combust_ch=(re_5_T001=="2"|re_5_T001=="6") 
*gas (2) o electricidad (6)*
...
gen des2_ch=.
*no existe como construir opcion 2, NO EXISTE OTRO CASO*
 ...
 ***aguamejorada_ch***
 *********************
destring re_4_T00 , replace
		
g       aguamejorada_ch = 1 if (re_4_T00 >=1 & re_4_T00  <=2) | (re_4_T00 >=4 & re_4_T00 <=5) | re_4_T00   ==7
replace aguamejorada_ch = 0 if  re_4_T00 ==3 | re_4_T00 ==6 | re_4_T00 ==8
