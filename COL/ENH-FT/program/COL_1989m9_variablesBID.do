...
gen desemp1_ci=0
replace desemp1_ci=(emp_ci==0 & activprin==2 & seeking1==1)
...
replace ynlm_ci= incomeother2 if incomeother1==1 & incomeother2<999999
...
gen aguamala2_ch=.
replace aguamala2_ch=1 if water==3 |water==4 |water==5 |water==6 |water==7 |water==8
replace aguamala2_ch=0 if water==1 |water==2 

** NOTE: do this for all hh infrastr. var **by idh_ch: egen aguamala_ch=min(aguamala2_ch)

gen aguamide_ch=.
/*NA*/

/*HOW DO I CODE THE ELECTRICITY VARIABLE SINCE IT RANGES FROM 1-5, not 1 or 3??*/
/*gen luz_ch=.*/
/*replace luz_ch=1 if electricity==*/
/* solo pregunta si la vivienda tiene conexion con energia electrica*/
/* we assume that 'la principal fuente de iluminacion es elec.*/
...
replace pared_ch=0 if walls==2 | walls==3 | walls==3 | walls==6 
...
gen telef_ch=.
/* in questionnaire but not coded in registros or available in dataset */
