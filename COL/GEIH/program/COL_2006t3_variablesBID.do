...
/*YL - > Nota:
La recolección de la Gran Encuesta Integrada de Hogares empezó el 7 de agosto de 2006 en su módulo central
de mercado laboral e ingresos y, a partir del 11 de septiembre, con su módulo de gastos de los hogares. 

A partir del 10 de julio de 2006 se amplió la cobertura de la ECH a 11 ciudades adicionales a las trece 
principales ciudades y áreas metropolitanas, al resto de cabeceras y al resto rural; cobertura que en la 
...
***factor_ch***
***************
/*NOTA: este factor fue dividido entre 2 tal como lo señala la instrucción del DANE al unir 2 meses.
Pues esta base no es el trimestre completo sino agosto más septiembre ya que 
...
***mes***
*********
gen byte mes_c=MES /*YL-> solo dos mes. En julio el formulario es diferente*/
...
/* 2014, 09 MLO: modificacion porque ya pocas observaciones en la base daría una cobertura de 1%
se recomienda pedir nuevamente la base con esta variable
gen pension_ci=1 if P7500S2A1>0 & P7500S2A1<. 
replace pension_ci=. if P7500S2A1==99*/

* DZ Septiembre 2017- se genera la variable pension contributiva
gen pension_ci=(P7500S2==1 | (P7500S2A1>0 & P7500S2A1!=.))
...
replace ypen_ci=. if pension_ci==0 | P7500S2A1==9999999999 | P7500S2A1==98 | P7500S2A1==99*/

* DZ Septiembre, 2017- se genera la variable valor de la pension contributiva
gen ypen_ci=P7500S2A1 
replace ypen_ci=. if pension_ci==0 | P7500S2A1==9999999999 | P7500S2A1==98 | P7500S2A1==99
...
*pensionsub_ci*
***************
*DZ Septiembre 2017-Se crea la variable de pensión subsidiada*
...
**ypensub_ci*
*****************
*DZ Septiembre 2017-Se crea la variable valor de la pensión subsidiada*
...
replace categosec_ci=0 if P7050==9 | P7050==8
...
gen ygasrep		= P6530S1
gen yprimtec	= P6540S1
gen yprimant	= P6550S1
gen yprimclim 	= P6560S1
...
* Modificaciones Marcela Rubio Septiembre 2014: se corrige problema de outliers
...
*2014, 02 agregue yjubila ypension, MLO
egen ynlm_ci=rsum(yayudafam ypension yjubila yremesas yayudainst yayudainste yintereses ycesantia)
*egen ynlm_ci=rsum(yayudafam yremesas yayudainst yayudainste yintereses ycesantia)
replace ynlm_ci=. if yayudafam==. & yremesas==. & yayudainst==. & yayudainste==. & yintereses==. & ycesantia==. & yjubila==. & ypension==.
