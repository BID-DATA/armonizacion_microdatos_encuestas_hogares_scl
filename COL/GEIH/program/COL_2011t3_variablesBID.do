...
Encuesta: GEIH
Round: t3
Autores: Jorge Luis Castañeda, Yessenia Loayza
...
* COL 2011
gen salmm_ci= 	17853.3333333333 *30
...
******************
	...
	*replace categopri_ci = 0 if p6430 == 8 
...
/*
	foreach var in p6500 p6510s1 p6590s1 p6600s1 p6610s1 p6620s1	///
		...
		p6750 p550 p7070 p7500s1a1 p7500s2a1 p7500s3a1 				///
	...
	recode p6760 (0=0.5) // JLC: ¿Por qué?
...
***** ynlm_ch *****
*******************
//	JLC: Nueva elaboración de las siguientes cuatro variables 
...
/* Missing values para no sabe no responde en maximo nivel educativo y  
   último anio aprobado. */		
	...
	replace p6220 = . if p6220 == 0

	g aedu_ci = .

* 0 anios de educacion 
	replace aedu_ci = 0 if p6210 == 1 | p6210 == 2 // Ninguo o prescolar.
...
//	JLC: Inconsistencia. dorm_ch > cuartos_ch	
	replace dorm_ch = cuartos_ch if dorm_ch >= cuartos_ch
