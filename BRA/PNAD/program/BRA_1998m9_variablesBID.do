...
segun la fuente, el monto bpc para adultos mayores fue de 130 reales. Se encuentran beneficiarios con dicho monto*/
gen pensionsub_ci=(yotr==130)
...
*********
gen lp_ci=.				
replace lp_ci=93.5606931452563	if region==4	& area==1		/*sur-urbana*/
replace lp_ci=85.172493048721	if region==4	& area==2		/*sur-rural */
replace lp_ci=95.4964315976016	if region==2	& area==1		/*noreste-urbana*/
replace lp_ci=85.172493048721	if region==2	& area==2		/*noreste-rural*/
replace lp_ci=74.8485545080227	if region==3	& area==1		/*sudeste-urbano*/
replace lp_ci=63.8793697947231	if region==3	& area==2		/*sudeste-rural*/
replace lp_ci=98.0774162552776	if region==1	& area==1		/*norte-urbano*/
replace lp_ci=85.8177392540517	if region==1	& area==2		/*norte-rural */
replace lp_ci=79.3652776262264	if region==5	& area==1		/*centro oeste-urbano*/
replace lp_ci=69.6865852335824	if region==5	& area==2		/*centro oeste-rural */
replace lp_ci=106.465616351813	if uf==33	& area==3		/*Rio de janeiro-metropolitano*/
replace lp_ci=90.334462364073	if uf==33	& area==1		/*Rio de janeiro-urbano*/
replace lp_ci=81.3010161031187	if uf==33	& area==2		/*Rio de janeiro-rural*/
replace lp_ci=107.11086247532	if uf==35	& area==3		/*Sao Paulo-metropolitano*/
replace lp_ci=94.8511854740943	if uf==35	& area==1		/*Sao paulo-urbano*/
replace lp_ci=77.429539149334	if uf==35	& area==2		 /*Sao paulo-rural*/
replace lp_ci=92.2702008164183	if uf==53	& area==3		/*Distrito federal-metropolitana*/
replace lp_ci=118.725293353039	if region==4	& area==3	& uf==43	/*Porto alegre: sur-metropolitana-rio grande de sul*/
replace lp_ci=98.0774162552776	if region==4	& area==3	& uf==41	/*curitiba:     sur-metropolitana-paraná*/
replace lp_ci=84.5272469252137	if region==2	& area==3	& uf==23	/*Fortaleza:    noreste-metropolitana-ceará*/
replace lp_ci=110.982339461834	if region==2	& area==3	& uf==26	/*recife:       noreste-metropolitana-pernambuco*/
replace lp_ci=104.529877817644	if region==2	& area==3	& uf==29	/*salvador:     noreste-metropolitana-bahia*/
replace lp_ci=83.2367545963757	if region==3	& area==3	& uf==31	/*belo horizonte:sureste-metropolitana-minas gerais*/
replace lp_ci=94.8511854740943	if region==1	& area==3	& uf==15	/*belem: noreste-metropolitana-pará*/
...
***********
gen lpe_ci=.				
replace lpe_ci=46.7803465726281	if region==4	& area==1		/*sur-urbana*/
replace lpe_ci=42.5862465243605	if region==4	& area==2		/*sur-rural */
replace lpe_ci=47.7482157988008	if region==2	& area==1		/*noreste-urbana*/
replace lpe_ci=42.5862465243605	if region==2	& area==2		/*noreste-rural*/
replace lpe_ci=37.4242772540114	if region==3	& area==1		/*sudeste-urbano*/
replace lpe_ci=31.9396848973615	if region==3	& area==2		/*sudeste-rural*/
replace lpe_ci=49.0387081276388	if region==1	& area==1		/*norte-urbano*/
replace lpe_ci=42.9088696270259	if region==1	& area==2		/*norte-rural */
replace lpe_ci=39.6826388131132	if region==5	& area==1		/*centro oeste-urbano*/
replace lpe_ci=34.8432926167912	if region==5	& area==2		/*centro oeste-rural */
replace lpe_ci=53.2328081759065	if uf==33	& area==3		/*Rio de janeiro-metropolitano*/
replace lpe_ci=45.1672311820365	if uf==33	& area==1		/*Rio de janeiro-urbano*/
replace lpe_ci=40.6505080515593	if uf==33	& area==2		/*Rio de janeiro-rural*/
replace lpe_ci=53.5554312376601	if uf==35	& area==3		/*Sao Paulo-metropolitano*/
replace lpe_ci=47.4255927370472	if uf==35	& area==1		/*Sao paulo-urbano*/
replace lpe_ci=38.714769574667	if uf==35	& area==2		 /*Sao paulo-rural*/
replace lpe_ci=46.1351004082091	if uf==53	& area==3		/*Distrito federal-metropolitana*/
replace lpe_ci=59.3626466765194	if region==4	& area==3	& uf==43	/*Porto alegre: sur-metropolitana-rio grande de sul*/
replace lpe_ci=49.0387081276388	if region==4	& area==3	& uf==41	/*curitiba:     sur-metropolitana-paraná*/
replace lpe_ci=42.2636234626069	if region==2	& area==3	& uf==23	/*Fortaleza:    noreste-metropolitana-ceará*/
replace lpe_ci=55.4911697309171	if region==2	& area==3	& uf==26	/*recife:       noreste-metropolitana-pernambuco*/
replace lpe_ci=52.2649389088221	if region==2	& area==3	& uf==29	/*salvador:     noreste-metropolitana-bahia*/
replace lpe_ci=41.6183772981878	if region==3	& area==3	& uf==31	/*belo horizonte:sureste-metropolitana-minas gerais*/
replace lpe_ci=47.4255927370472	if region==1	& area==3	& uf==15	/*belem: noreste-metropolitana-pará*/
...
http://dds.cepal.org/bdps/programa/?id=43
segun la fuente, el monto bpc para adultos mayores fue de 130 reales. Se encuentran beneficiarios con dicho monto*/
gen ypensub_ci=yotr if yotr==130
...
******************************
	...
	**********************
	...
	gen migantiguo5_ci=(migrante_ci==1 & (vivsep93==1 | (ufed93!=. & ufed93!=98) | (migrante_ci==1 & tpoufede==6) | (migrante_ci==1 & tpoufedc==4))) if migrante_ci!=. & !inrange(edad_ci,0,4)	
	...
	**********************
	...
	gen migrantiguo5_ci=(migrante_ci==1 & (vivsep93==1 | (ufed93!=. & ufed93!=98) | (migrante_ci==1 & tpoufede==6) | (migrante_ci==1 & tpoufedc==4))) if migrante_ci!=. & !inrange(edad_ci,0,4)
