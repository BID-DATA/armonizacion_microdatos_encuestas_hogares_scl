...
*********
gen lp_ci =.
replace lp_ci=77.3981740170383	if region==4	& area==1		/*sur-urbana*/
replace lp_ci=70.4590273632889	if region==4	& area==2		/*sur-rural */
replace lp_ci=78.9995155264849	if region==2	& area==1		/*noreste-urbana*/
replace lp_ci=70.4590273632889	if region==2	& area==2		/*noreste-rural*/
replace lp_ci=61.9185392068618	if region==3	& area==1		/*sudeste-urbano*/
replace lp_ci=52.8442705292356	if region==3	& area==2		/*sudeste-rural*/
replace lp_ci=81.134637584206	if region==1	& area==1		/*norte-urbano*/
replace lp_ci=70.9928079115635	if region==1	& area==2		/*norte-rural */
replace lp_ci=65.6550027807984	if region==5	& area==1		/*centro oeste-urbano*/
replace lp_ci=57.6482951252638	if region==5	& area==2		/*centro oeste-rural */
replace lp_ci=88.0737842379555	if uf==33	& area==3		/*Rio de janeiro-metropolitano*/
replace lp_ci=74.7292714787312	if uf==33	& area==1		/*Rio de janeiro-urbano*/
replace lp_ci=67.2563443105515	if uf==33	& area==2		/*Rio de janeiro-rural*/
replace lp_ci=88.6075647185414	if uf==35	& area==3		/*Sao Paulo-metropolitano*/
replace lp_ci=78.4657350458989	if uf==35	& area==1		/*Sao paulo-urbano*/
replace lp_ci=64.0536612510452	if uf==35	& area==2		 /*Sao paulo-rural*/
replace lp_ci=76.3306129881778	if uf==53	& area==3		/*Distrito federal-metropolitana*/
replace lp_ci=98.215613910598	if region==4	& area==3	& uf==43	/*Porto alegre: sur-metropolitana-rio grande de sul*/
replace lp_ci=81.134637584206	if region==4	& area==3	& uf==41	/*curitiba:     sur-metropolitana-paraná*/
replace lp_ci=69.9252468827029	if region==2	& area==3	& uf==23	/*Fortaleza:    noreste-metropolitana-ceará*/
replace lp_ci=91.8102478051232	if region==2	& area==3	& uf==26	/*recife:       noreste-metropolitana-pernambuco*/
replace lp_ci=86.4724426608203	if region==2	& area==3	& uf==29	/*salvador:     noreste-metropolitana-bahia*/
replace lp_ci=68.8576858538424	if region==3	& area==3	& uf==31	/*belo horizonte:sureste-metropolitana-minas gerais*/
replace lp_ci=78.4657350458989	if region==1	& area==3	& uf==15	/*belem: noreste-metropolitana-pará*/
...
***********
gen lpe_ci =.
replace lpe_ci=38.6990870085192	if region==4	& area==1		/*sur-urbana*/
replace lpe_ci=35.2295136816445	if region==4	& area==2		/*sur-rural */
replace lpe_ci=39.4997577632425	if region==2	& area==1		/*noreste-urbana*/
replace lpe_ci=35.2295136816445	if region==2	& area==2		/*noreste-rural*/
replace lpe_ci=30.9592696034309	if region==3	& area==1		/*sudeste-urbano*/
replace lpe_ci=26.4221352646178	if region==3	& area==2		/*sudeste-rural*/
replace lpe_ci=40.567318792103	if region==1	& area==1		/*norte-urbano*/
replace lpe_ci=35.4964039557818	if region==1	& area==2		/*norte-rural */
replace lpe_ci=32.8275013903992	if region==5	& area==1		/*centro oeste-urbano*/
replace lpe_ci=28.8241475626319	if region==5	& area==2		/*centro oeste-rural */
replace lpe_ci=44.0368921189777	if uf==33	& area==3		/*Rio de janeiro-metropolitano*/
replace lpe_ci=37.3646357393656	if uf==33	& area==1		/*Rio de janeiro-urbano*/
replace lpe_ci=33.6281721552758	if uf==33	& area==2		/*Rio de janeiro-rural*/
replace lpe_ci=44.3037823592707	if uf==35	& area==3		/*Sao Paulo-metropolitano*/
replace lpe_ci=39.2328675229495	if uf==35	& area==1		/*Sao paulo-urbano*/
replace lpe_ci=32.0268306255226	if uf==35	& area==2		 /*Sao paulo-rural*/
replace lpe_ci=38.1653064940889	if uf==53	& area==3		/*Distrito federal-metropolitana*/
replace lpe_ci=49.107806955299	if region==4	& area==3	& uf==43	/*Porto alegre: sur-metropolitana-rio grande de sul*/
replace lpe_ci=40.567318792103	if region==4	& area==3	& uf==41	/*curitiba:     sur-metropolitana-paraná*/
replace lpe_ci=34.9626234413515	if region==2	& area==3	& uf==23	/*Fortaleza:    noreste-metropolitana-ceará*/
replace lpe_ci=45.9051239025616	if region==2	& area==3	& uf==26	/*recife:       noreste-metropolitana-pernambuco*/
replace lpe_ci=43.2362213304102	if region==2	& area==3	& uf==29	/*salvador:     noreste-metropolitana-bahia*/
replace lpe_ci=34.4288429269212	if region==3	& area==3	& uf==31	/*belo horizonte:sureste-metropolitana-minas gerais*/
replace lpe_ci=39.2328675229495	if region==1	& area==3	& uf==15	/*belem: noreste-metropolitana-pará*/
...
******************************
	...
	**********************
	...
	gen migantiguo5_ci=(migrante_ci==1 & (vivsep90==1 | (ufed90!=. & ufed90!=98) | (migrante_ci==1 & tpoufede==6) | (migrante_ci==1 & tpoufedc==4))) if migrante_ci!=. & !inrange(edad_ci,0,4)	
	...
	**********************
	...
	gen migrantiguo5_ci=(migrante_ci==1 & (vivsep90==1 | (ufed90!=. & ufed90!=98) | (migrante_ci==1 & tpoufede==6) | (migrante_ci==1 & tpoufedc==4))) if migrante_ci!=. & !inrange(edad_ci,0,4)
