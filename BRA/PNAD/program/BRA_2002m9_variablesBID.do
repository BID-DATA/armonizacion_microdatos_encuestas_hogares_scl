...
segun la fuente, el monto bpc para adultos mayores fue de 200 reales. Se encuentran beneficiarios con dicho monto*/
gen pensionsub_ci=(v1273==200)
...
*********
gen lp_ci=.
replace lp_ci=125.689275318281	if region==4	& area==1		/*sur-urbana*/
replace lp_ci=114.420581640249	if region==4	& area==2		/*sur-rural */
replace lp_ci=128.289743047858	if region==2	& area==1		/*noreste-urbana*/
replace lp_ci=114.420581640249	if region==2	& area==2		/*noreste-rural*/
replace lp_ci=100.551420243633	if region==3	& area==1		/*sudeste-urbano*/
replace lp_ci=85.8154362411791	if region==3	& area==2		/*sudeste-rural*/
replace lp_ci=131.75703342724	if region==1	& area==1		/*norte-urbano*/
replace lp_ci=115.287404290056	if region==1	& area==2		/*norte-rural */
replace lp_ci=106.619178363584	if region==5	& area==1		/*centro oeste-urbano*/
replace lp_ci=93.6168395398288	if region==5	& area==2		/*centro oeste-rural */
replace lp_ci=143.025727105272	if uf==33	& area==3		/*Rio de janeiro-metropolitano*/
replace lp_ci=121.355162399014	if uf==33	& area==1		/*Rio de janeiro-urbano*/
replace lp_ci=109.219646126136	if uf==33	& area==2		/*Rio de janeiro-rural*/
replace lp_ci=143.892549645157	if uf==35	& area==3		/*Sao Paulo-metropolitano*/
replace lp_ci=127.422920507973	if uf==35	& area==1		/*Sao paulo-urbano*/
replace lp_ci=104.018710601031	if uf==35	& area==2		 /*Sao paulo-rural*/
replace lp_ci=123.95563012859	if uf==53	& area==3		/*Distrito federal-metropolitana*/
replace lp_ci=159.495356242457	if region==4	& area==3	& uf==43	/*Porto alegre: sur-metropolitana-rio grande de sul*/
replace lp_ci=131.75703342724	if region==4	& area==3	& uf==41	/*curitiba:     sur-metropolitana-paraná*/
replace lp_ci=113.553759100364	if region==2	& area==3	& uf==23	/*Fortaleza:    noreste-metropolitana-ceará*/
replace lp_ci=149.093485214231	if region==2	& area==3	& uf==26	/*recife:       noreste-metropolitana-pernambuco*/
replace lp_ci=140.425259265775	if region==2	& area==3	& uf==29	/*salvador:     noreste-metropolitana-bahia*/
replace lp_ci=111.820113910673	if region==3	& area==3	& uf==31	/*belo horizonte:sureste-metropolitana-minas gerais*/
replace lp_ci=127.422920507973	if region==1	& area==3	& uf==15	/*belem: noreste-metropolitana-pará*/
...
***********
gen lpe_ci=.				
replace lpe_ci=62.8446376591407	if region==4	& area==1		/*sur-urbana*/
replace lpe_ci=57.2102908201246	if region==4	& area==2		/*sur-rural */
replace lpe_ci=64.1448715239288	if region==2	& area==1		/*noreste-urbana*/
replace lpe_ci=57.2102908201246	if region==2	& area==2		/*noreste-rural*/
replace lpe_ci=50.2757101218165	if region==3	& area==1		/*sudeste-urbano*/
replace lpe_ci=42.9077181205895	if region==3	& area==2		/*sudeste-rural*/
replace lpe_ci=65.87851671362	if region==1	& area==1		/*norte-urbano*/
replace lpe_ci=57.6437021450278	if region==1	& area==2		/*norte-rural */
replace lpe_ci=53.3095891817919	if region==5	& area==1		/*centro oeste-urbano*/
replace lpe_ci=46.8084197699144	if region==5	& area==2		/*centro oeste-rural */
replace lpe_ci=71.5128635526361	if uf==33	& area==3		/*Rio de janeiro-metropolitano*/
replace lpe_ci=60.6775811995071	if uf==33	& area==1		/*Rio de janeiro-urbano*/
replace lpe_ci=54.6098230630681	if uf==33	& area==2		/*Rio de janeiro-rural*/
replace lpe_ci=71.9462748225786	if uf==35	& area==3		/*Sao Paulo-metropolitano*/
replace lpe_ci=63.7114602539864	if uf==35	& area==1		/*Sao paulo-urbano*/
replace lpe_ci=52.0093553005156	if uf==35	& area==2		 /*Sao paulo-rural*/
replace lpe_ci=61.9778150642951	if uf==53	& area==3		/*Distrito federal-metropolitana*/
replace lpe_ci=79.7476781212283	if region==4	& area==3	& uf==43	/*Porto alegre: sur-metropolitana-rio grande de sul*/
replace lpe_ci=65.87851671362	if region==4	& area==3	& uf==41	/*curitiba:     sur-metropolitana-paraná*/
replace lpe_ci=56.7768795501822	if region==2	& area==3	& uf==23	/*Fortaleza:    noreste-metropolitana-ceará*/
replace lpe_ci=74.5467426071154	if region==2	& area==3	& uf==26	/*recife:       noreste-metropolitana-pernambuco*/
replace lpe_ci=70.2126296328873	if region==2	& area==3	& uf==29	/*salvador:     noreste-metropolitana-bahia*/
replace lpe_ci=55.9100569553366	if region==3	& area==3	& uf==31	/*belo horizonte:sureste-metropolitana-minas gerais*/
replace lpe_ci=63.7114602539864	if region==1	& area==3	& uf==15	/*belem: noreste-metropolitana-pará*/
...
replace cotizando_ci = 1 if ((v9059==1 | v9099==1 | v9103==1 | v9120==2) & emp_ci==1)
...
replace ocupa_ci=8 if v4810==0  
replace ocupa_ci=9 if v4810==9 & emp_ci==1
...
http://dds.cepal.org/bdps/programa/?id=43
segun la fuente, el monto bpc para adultos mayores fue de 200 reales. Se encuentran beneficiarios con dicho monto*/
gen ypensub_ci=v1273 if v1273==200
...
replace aedu_ci = grado_asiste - 1 if inlist(nivel_asiste, 1, 3) // Regular ou 1º grau
replace aedu_ci = grado_asiste + 8 - 1  if inlist(nivel_asiste, 2, 4) // Regular ou 2º grau
...
replace aedu_ci = 0 if (inlist(nivel_asiste, 1, 3) & grado_asiste == .) // Regular ou 1º grau
replace aedu_ci = 8 if (inlist(nivel_asiste, 2, 4) & grado_asiste == .) // Regular ou 2º grau
...
* Sistema actual
replace aedu_ci = grado_no_asiste if nivel_no_asiste == 4 // Regular ou 1º grau
replace aedu_ci = grado_no_asiste + 8 if nivel_no_asiste == 5 // Regular ou 2º grau
...
* No finalizado
replace aedu_ci = 0 if (inlist(nivel_no_asiste, 1, 4)  & grado_no_asiste == . & inlist(finalizo, 3, .)) // Elementar, Regular ou 1º grau
...
replace aedu_ci = 8 if (nivel_no_asiste == 5 & grado_no_asiste == . & inlist(finalizo, 3, .)) // Regular ou 2º grau
...
replace aedu_ci = 8 if (nivel_no_asiste == 4 & grado_no_asiste == . & finalizo == 1) // Regular ou 1º grau
replace aedu_ci = 12 if (nivel_no_asiste == 3 & grado_no_asiste == . & finalizo == 1) // Medio 2 
replace aedu_ci = 12 if (nivel_no_asiste == 5 & grado_no_asiste == . & finalizo == 1) // Regular ou 2º grau
...
******************
* Line of code with indicator repiteult was deleted* Line of code with indicator repiteult was deleteddrop nivel_asiste grado_asiste grado_no_asiste nivel_no_asiste finalizo
...
* Variables incluidas por SCL/MIG Fernando Morales
	...
	*******************
	...
	replace migrante_ci = . if v5030 == 99
