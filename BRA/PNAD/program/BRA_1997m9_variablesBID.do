...
*Take out the hogares colectivos
keep if espdom==1
 ...
 ***vivi1_ch***
 **************
gen vivi1_ch=1 if tipodom==2
replace vivi1_ch=2 if tipodom==4
replace vivi1_ch=3 if tipodom==6
...
9: No responde*/
	...
	*********
	gen byte afro_ci = . 	 
	replace afro_ci = 1 if piel ==4 | piel ==8
	replace afro_ci = 0 if piel !=4 & piel !=8 & piel != 9 & piel != .
	
	*********
	*ind_ci*
	*********	
	gen byte ind_ci =. 		  
	replace ind_ci = 1 if piel ==0
	replace ind_ci = 0 if piel !=0 & piel != 9 & piel != .
...
segun la fuente, el monto bpc para adultos mayores fue de 120 reales. Se encuentran beneficiarios con dicho monto*/
gen pensionsub_ci=(yotr==120)
...
*********
gen lp_ci=.				
replace lp_ci=90.7728555187537	if region==4	& area==1		/*sur-urbana*/
replace lp_ci=82.6345994859231	if region==4	& area==2		/*sur-rural */
replace lp_ci=92.6509145727203	if region==2	& area==1		/*noreste-urbana*/
replace lp_ci=82.6345994859231	if region==2	& area==2		/*noreste-rural*/
replace lp_ci=72.6182844070644	if region==3	& area==1		/*sudeste-urbano*/
replace lp_ci=61.9759496223809	if region==3	& area==2		/*sudeste-rural*/
replace lp_ci=95.1549933642659	if region==1	& area==1		/*norte-urbano*/
replace lp_ci=83.2606192235022	if region==1	& area==2		/*norte-rural */
replace lp_ci=77.0004222605152	if region==5	& area==1		/*centro oeste-urbano*/
replace lp_ci=67.6101268636659	if region==5	& area==2		/*centro oeste-rural */
replace lp_ci=103.293249397097	if uf==33	& area==3		/*Rio de janeiro-metropolitano*/
replace lp_ci=87.6427570690144	if uf==33	& area==1		/*Rio de janeiro-urbano*/
replace lp_ci=78.8784813382973	if uf==33	& area==2		/*Rio de janeiro-rural*/
replace lp_ci=103.91926905529	if uf==35	& area==3		/*Sao Paulo-metropolitano*/
replace lp_ci=92.0248949145266	if uf==35	& area==1		/*Sao paulo-urbano*/
replace lp_ci=75.122363182733	if uf==35	& area==2		 /*Sao paulo-rural*/
replace lp_ci=89.5208161229809	if uf==53	& area==3		/*Distrito federal-metropolitana*/
replace lp_ci=115.18762353786	if region==4	& area==3	& uf==43	/*Porto alegre: sur-metropolitana-rio grande de sul*/
replace lp_ci=95.1549933642659	if region==4	& area==3	& uf==41	/*curitiba:     sur-metropolitana-paraná*/
replace lp_ci=82.0085798277294	if region==2	& area==3	& uf==23	/*Fortaleza:    noreste-metropolitana-ceará*/
replace lp_ci=107.675387242609	if region==2	& area==3	& uf==26	/*recife:       noreste-metropolitana-pernambuco*/
replace lp_ci=101.415190263745	if region==2	& area==3	& uf==29	/*salvador:     noreste-metropolitana-bahia*/
replace lp_ci=80.7565404319566	if region==3	& area==3	& uf==31	/*belo horizonte:sureste-metropolitana-minas gerais*/
replace lp_ci=92.0248949145266	if region==1	& area==3	& uf==15	/*belem: noreste-metropolitana-pará*/
...
***********
gen lpe_ci=.				
replace lpe_ci=45.3864277593769	if region==4	& area==1		/*sur-urbana*/
replace lpe_ci=41.3172997429615	if region==4	& area==2		/*sur-rural */
replace lpe_ci=46.3254572863601	if region==2	& area==1		/*noreste-urbana*/
replace lpe_ci=41.3172997429615	if region==2	& area==2		/*noreste-rural*/
replace lpe_ci=36.3091422035322	if region==3	& area==1		/*sudeste-urbano*/
replace lpe_ci=30.9879748111904	if region==3	& area==2		/*sudeste-rural*/
replace lpe_ci=47.577496682133	if region==1	& area==1		/*norte-urbano*/
replace lpe_ci=41.6303096117511	if region==1	& area==2		/*norte-rural */
replace lpe_ci=38.5002111302576	if region==5	& area==1		/*centro oeste-urbano*/
replace lpe_ci=33.8050634318329	if region==5	& area==2		/*centro oeste-rural */
replace lpe_ci=51.6466246985483	if uf==33	& area==3		/*Rio de janeiro-metropolitano*/
replace lpe_ci=43.8213785345072	if uf==33	& area==1		/*Rio de janeiro-urbano*/
replace lpe_ci=39.4392406691487	if uf==33	& area==2		/*Rio de janeiro-rural*/
replace lpe_ci=51.9596345276451	if uf==35	& area==3		/*Sao Paulo-metropolitano*/
replace lpe_ci=46.0124474572633	if uf==35	& area==1		/*Sao paulo-urbano*/
replace lpe_ci=37.5611815913665	if uf==35	& area==2		 /*Sao paulo-rural*/
replace lpe_ci=44.7604080614905	if uf==53	& area==3		/*Distrito federal-metropolitana*/
replace lpe_ci=57.5938117689301	if region==4	& area==3	& uf==43	/*Porto alegre: sur-metropolitana-rio grande de sul*/
replace lpe_ci=47.577496682133	if region==4	& area==3	& uf==41	/*curitiba:     sur-metropolitana-paraná*/
replace lpe_ci=41.0042899138647	if region==2	& area==3	& uf==23	/*Fortaleza:    noreste-metropolitana-ceará*/
replace lpe_ci=53.8376936213044	if region==2	& area==3	& uf==26	/*recife:       noreste-metropolitana-pernambuco*/
replace lpe_ci=50.7075951318723	if region==2	& area==3	& uf==29	/*salvador:     noreste-metropolitana-bahia*/
replace lpe_ci=40.3782702159783	if region==3	& area==3	& uf==31	/*belo horizonte:sureste-metropolitana-minas gerais*/
replace lpe_ci=46.0124474572633	if region==1	& area==3	& uf==15	/*belem: noreste-metropolitana-pará*/
...
http://dds.cepal.org/bdps/programa/?id=43
segun la fuente, el monto bpc para adultos mayores fue de 120 reales. Se encuentran beneficiarios con dicho monto*/
gen ypensub_ci=yotr if yotr==120
...
******************************
	...
	**********************
	...
	gen migantiguo5_ci=(migrante_ci==1 & (vivsep92==1 | (ufed92!=. & ufed92!=98) | (migrante_ci==1 & tpoufede==6) | (migrante_ci==1 & tpoufedc==4))) if migrante_ci!=. & !inrange(edad_ci,0,4)	
	...
	**********************
	...
	gen migrantiguo5_ci=(migrante_ci==1 & (vivsep92==1 | (ufed92!=. & ufed92!=98) | (migrante_ci==1 & tpoufede==6) | (migrante_ci==1 & tpoufedc==4))) if migrante_ci!=. & !inrange(edad_ci,0,4)
