...
segun la fuente, el monto bpc para adultos mayores fue de 465 reales. Se encuentran beneficiarios con dicho monto*/
gen pensionsub_ci=(v1273==465)
...
http://dds.cepal.org/bdps/programa/?id=43
segun la fuente, el monto bpc para adultos mayores fue de 465 reales. Se encuentran beneficiarios con dicho monto*/
gen ypensub_ci=v1273 if v1273==465
...
*********
gen lp_ci=.			
replace lp_ci=196.952559689598	if region==4	& area==1            /*sur-urbana*/	
replace lp_ci=179.294743948149	if region==4	& area==2            /*sur-rural */	
replace lp_ci=201.027440179069	if region==2	& area==1            /*noreste-urbana*/	
replace lp_ci=179.294743948149	if region==2	& area==2            /*noreste-rural*/	
replace lp_ci=157.562047734454	if region==3	& area==1            /*sudeste-urbano*/	
replace lp_ci=134.471057978337	if region==3	& area==2            /*sudeste-rural*/	
replace lp_ci=206.46061427986	if region==1	& area==1            /*norte-urbano*/	
replace lp_ci=180.65303755947	if region==1	& area==2            /*norte-rural */	
replace lp_ci=167.070102341941	if region==5	& area==1            /*centro oeste-urbano*/	
replace lp_ci=146.695699618994	if region==5	& area==2            /*centro oeste-rural */	
replace lp_ci=224.118430021309	if region_c==33	& area==3                /*Rio de janeiro-metropolitano*/	
replace lp_ci=190.161092149732	if region_c==33	& area==1	             /*Rio de janeiro-urbano*/
replace lp_ci=171.144982883085	if region_c==33	& area==2	             /*Rio de janeiro-rural*/
replace lp_ci=225.476723460385	if region_c==35	& area==3	             /*Sao Paulo-metropolitano*/
replace lp_ci=199.669146739994	if region_c==35	& area==1	             /*Sao paulo-urbano*/
...
replace lp_ci=206.46061427986	if region==4	& area==3	& region_c==41 /*curitiba:     sur-metropolitana-paraná*/
replace lp_ci=177.936450509074	if region==2	& area==3	& region_c==23 /*Fortaleza:    noreste-metropolitana-ceará*/
...
replace lp_ci=199.669146739994	if region==1	& area==3	& region_c==15 /*belem: noreste-metropolitana-pará*/
...
***********
gen lpe_ci=.				
replace lpe_ci=98.4762798447992	if region==4	& area==1		/*sur-urbana*/
replace lpe_ci=89.6473719740747	if region==4	& area==2		/*sur-rural */
replace lpe_ci=100.513720089535	if region==2	& area==1		/*noreste-urbana*/
replace lpe_ci=89.6473719740747	if region==2	& area==2		/*noreste-rural*/
replace lpe_ci=78.7810238672271	if region==3	& area==1		/*sudeste-urbano*/
replace lpe_ci=67.2355289891683	if region==3	& area==2		/*sudeste-rural*/
replace lpe_ci=103.23030713993	if region==1	& area==1		/*norte-urbano*/
replace lpe_ci=90.3265187797349	if region==1	& area==2		/*norte-rural */
replace lpe_ci=83.5350511709704	if region==5	& area==1		/*centro oeste-urbano*/
replace lpe_ci=73.3478498094971	if region==5	& area==2		/*centro oeste-rural */
replace lpe_ci=112.059215010655	if region_c==33	& area==3		    /*Rio de janeiro-metropolitano*/
replace lpe_ci=95.0805460748659	if region_c==33	& area==1		    /*Rio de janeiro-urbano*/
replace lpe_ci=85.5724914415426	if region_c==33	& area==2		    /*Rio de janeiro-rural*/
replace lpe_ci=112.738361730192	if region_c==35	& area==3		    /*Sao Paulo-metropolitano*/
replace lpe_ci=99.834573369997	if region_c==35	& area==1		    /*Sao paulo-urbano*/
...
replace lpe_ci=103.23030713993	if region==4	& area==3	& region_c==41	/*curitiba:     sur-metropolitana-paraná*/
replace lpe_ci=88.9682252545371	if region==2	& area==3	& region_c==23	/*Fortaleza:    noreste-metropolitana-ceará*/
...
replace lpe_ci=99.834573369997	if region==1	& area==3	& region_c==15	/*belem: noreste-metropolitana-pará*/
...
*Se crea la variable para los empleados adultos porque así es la armonizada, pero también se
*construye una variable que contiene la información para los niños.
...
replace horaspri_ci=. if horaspri_ci==99 |horaspri_ci==-1 | v4814!=1 
...
replace horastot_ci = . if  v4814!=1 |  (horaspri_ci==. & v9101==. & v9105==.) /*Necesitamos que sólo se fije en los empleados "adultos"*/
...
replace firmapeq_ci=1 if (v9008==9 | v9008==10) & ((v9016==2 & v9017<=5 & v9018==4) | (v9016==2 & v9017<=3 & v9018==2 & v9019<=3) | (v9016==4 & v9018==2 & v9019<=5) | (v9016==4 & v9018==4)) /*Empleador en Agro u otras actividades*/
...
replace ylmprik_ci=v7122 if edad_ci>=5 & edad_ci<=9
replace ylmprik_ci=. if  edad_ci>=5 & edad_ci<=9 & (v7122==-1 | v7122>=999999 |emp_ci==0)
...
replace ylnmprik_ci=v7125 if edad_ci>=5 & edad_ci<=9
replace ylnmprik_ci=. if edad_ci>=5 & edad_ci<=9 & (v7125==-1 | v7125>=999999 | emp_ci==0)
...
by idh_ch, sort: egen ylmk_ch=sum(ylmk_ci) if miembros_ci==1
...
by idh_ch, sort: egen ylnmk_ch=sum(ylnmk_ci) if miembros_ci==1
...
by idh_ch, sort: egen ylmnrk_ch=sum(ylmk_ci) if miembros_ci==1
