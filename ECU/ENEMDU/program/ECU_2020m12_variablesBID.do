...
* POTROT - Programas de otras transferencias monetarias no condicionadas:
		...
		* Bono de Protección Familiar (p79 p80)
		* Bono de Apoyo Nutricional (p81 p82)

*** Beneficiarios a nivel individual:
	gen byte pnc_ci = .
	gen byte ptmc_ci = (p75 == 1) if !missing(p75)
	gen byte potrot_ci = (p77 == 1 | p79 == 1 | p81 == 1) if !missing(p77) | !missing(p79) | !missing(p81)
...
*** Montos de transferencias a nivel individual:
	...
	foreach x in p78 p80 p82 {
		gen `x'_m = `x'
		replace `x'_m = . if `x' == 999999
	}
	egen double yotrot_ci = rowtotal(p78_m p80_m p82_m)
...
egen double ytransf_ci = rowtotal(ypnc_ci yptmc_ci yotrot_ci)
	...
	***ynlm_ci***
	*************
	foreach x in p71b p73b {
	gen `x'_m = `x'
	replace `x'_m = 0 if `x' == 999999
	}
	egen double ynlm_ci = rowtotal(p71b_m ypen_ci p73b_m remesas_ci ytransf_ci), m	
