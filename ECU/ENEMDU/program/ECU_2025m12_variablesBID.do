...
*B. INGRESOS NO LABORALES A NIVEL DE INDIVIDUO	
	...
	***ytot_ci***
	*************
	egen ytot_ci = rsum(ylm_ci ylnm_ci ynlm_ci ynlnm_ci), mi
...
*C. INGRESOS DEL HOGAR ***
	...
	***ylm_ch***
	************
	by idh_ch, sort: egen ylm_ch = total(ylm_ci) if miembros_ci == 1, mi

	*************
	***ylnm_ch***
	*************
	by idh_ch, sort: egen ylnm_ch = total(ylnm_ci) if miembros_ci == 1, mi
	...
	***ynlm_ch***
	*************
	by idh_ch, sort: egen ynlm_ch = total(ynlm_ci) if miembros_ci==1, mi
	...
	***ytot_ch***
	*************
	by idh_ch, sort: egen ytot_ch = total(ytot_ci) if miembros_ci==1, mi
