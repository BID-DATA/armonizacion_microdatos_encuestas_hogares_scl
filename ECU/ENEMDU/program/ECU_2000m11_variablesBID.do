...
gen region_c=int(ciudad/10000)
	...
	*************
	...
	egen idh_ch = group(area ciudad zona sector vivienda hogar)
...
****condocup_ci*
****************
/*gen gesbuscatra = (sitios==3 | amigos==4 | directo==5 | prensa==6 | agepu==7 | agepri==8 | tresne==9)
*2014, 01 MLO cambio del limite de edad de condocup_ci a 5+
gen condocup_ci=.
replace condocup_ci=1 if condact>=0 & condact<=4
replace condocup_ci=2 if trabajo==2 & actayuda==12 & aunotra==2 & gesbuscatra==1 /*genero desempleo abierto*/
replace condocup_ci=3 if condocup_ci!=1 & condocup_ci!=2
*replace condocup_ci=4 if edad<=9
replace condocup_ci=4 if edad<5
*/
*MGD-> se recodifica a menores inactivos que pueden estar trabajando la variable condact considera a menores de 0 a 9 y el limite de la encuesta es 5;
...
*************
gen pension_ci=0 
replace pension_ci=1 if (jub==1) /* A todas las per mayores de cinco*/
replace pension_ci=. if jub==.
label var pension_ci "1=Recibe pension contributiva"

*************
*ypen_ci*
*************
* MGD 12/17/2015: corrección de missings
gen ypen_ci=ingjub if pension_ci==1
replace ypen_ci=. if ingjub==99999999 
replace ypen_ci=. if ingjub==0
...
***************
gen pensionsub_ci=0 
replace pensionsub_ci=1 if bon==1
replace pensionsub_ci=. if bon==.
label var pensionsub_ci "1=recibe pension subsidiada / no contributiva"

*****************
**  ypensub_ci  *
*****************
recode ingbon (99999999=.) (10333333=.)
gen ypensub_ci=ingbon if pensionsub_ci==1
replace ypensub_ci=. if ingbon==0
...
label var pea_ci "Población Económicamente Activa"
	...
	*****************
	/*
	gen desalent_ci=(motnobus==1 | motnobus==2)
	label var desalent_ci "Trabajadores desalentados"
	*/
	*Modificado Mayra Sáenz - Agosto 2013
	***** El código mantiene como población de referencia a las personas inactivas (condocup_ci == 3) *****.
	gen byte desalent_ci = .
	replace desalent_ci = 1 if ((bustrama == 2 & (motnobus == 5 | motnobus == 6)) & condocup_ci == 3)
	...
	***** El código mantiene a la poblacion inactiva y a los menores de la edad límite de la PET como missing values en congruencia con la variable formal_ci *****.
	...
	replace afiliado_ci = 1 if (iess==1 & emp_ci==1)
	replace afiliado_ci = 0 if (iess != 1 & inlist(condocup_ci, 1, 2))
...
*Nota: seguridad social comprende solo los que en el futuro me ofrecen una pension.
	...
	***************
	/*
	replace subemp_ci=1 if (hormas>=4 & hormas<=6) & horastot_ci<=30 & emp_ci==1
	*/
	*Modificado Mayra Sáenz - Agosto 2013
	...
	*Modificado Mayra Sáenz - Agosto 2013
	/*
	 3 patrono o socio activo
           4 trabajador por cuenta propia
           5 trab. familiar no remunerado
           6 asalariado de gobierno
           7 asalariado de empresa privada
           8 empleado doméstico
           9 no informa
MLO: entiendo que estan mal los labels en esta variable. [daria 30% de trabajadores no remunerados[
   */
	...
	replace categopri_ci=3 if (catetrab>=6 & catetrab<=8) 
	replace categopri_ci=4 if catetrab==5
	replace categopri_ci=. if catetrab==9
	replace categopri_ci=. if emp_ci==0*/
/*MLO 	segun el diccionario:
CATEGORÍA OCUPACIONAL DE LA ACTIVIDAD PRINCIPAL (Pregunta 42: ¿En esta ocupación usted es o era?)
-1: Menores de 10 años
0: No aplicable
NO AGROPECUARIOS
1: Empleado u obrero de gobierno
...
AGROPECUARIOS
7: Trabajador agropecuario a sueldo o salario
8: Jornalero o peón agropecuario
9: Patrón de la finca
10: Trabajador agropecuario por cuenta propia
11: Trabajador familiar agropecuario no remunerado
12: EMPLEADO(A) DOMÉSTICO(A)

2014, 01 MLO: vuelvo a cambiar la version modificada por Mayra
*/
gen categopri_ci=.
...
* mod MLO 2015, 10
	...
	* Ultima actualizacion MGD 4/16/2014
	gen rama_ci=.
	replace rama_ci = 1 if (rama>=101  & rama<=650) & emp_ci==1
	replace rama_ci = 2 if (rama>=1010 & rama<=1429) & emp_ci==1
	replace rama_ci = 3 if (rama>=1500 & rama<=3720) & emp_ci==1
	replace rama_ci = 4 if (rama>=4010 & rama<=4100) & emp_ci==1
	replace rama_ci = 5 if (rama>=4500 & rama<=4550) & emp_ci==1
	replace rama_ci = 6 if (rama>=5000 & rama<=5520) & emp_ci==1
	replace rama_ci = 7 if (rama>=6010 & rama<=6421) & emp_ci==1
	replace rama_ci = 8 if (rama>=6510 & rama<=7499) & emp_ci==1
	...
	*antiguedad_ci*
	***************
	*La variable insumo sería p45, sin embargo, ésta tiene valores extremadamente altos.
	
	*Modificación Mayra Sàenz- Agosto 2013.  La p45 (estabil) hace referencia a que si en el trabajo actual tiene
	*nombramiento o contrato, por lo que se utiliza en la variable contrato_ci.
...
* MLO 2014, 04 se utiliza afiliacion para la seria entes de 2001
...
ingasa          long   %12.0g                 ingreso líquido por salario
ingasa1         long   %12.0g                 cuál fue su ingreso total por sueldo
ingasa2         long   %12.0g                 monto recibido por trabajo especies
ingsec          long   %12.0g                 ocup. secundaria cual fúe su ingreso por
                                                sueldo
inginv          long   %12.0g                 monto de ingresos derivados de bienes de
                                                capital
ingjub          long   %12.0g                 monto de ingresos por jubilación o pensión
ingotr          long   %12.0g                 monto por regalos o donaciones
ingbon          long   %12.0g                 ingreso por bono solidario
ingrl           int    %8.0g                  ingresos
retpat                                        monto de retiro del negocio.
retasa          long   %12.0g                 descuento por aportaciones al iess

*/

foreach var of varlist ingpat retpat ingasa retasa ingasa2 ingsec ///
retsec inginv ingjub ingotr ingbon {
...
replace `var'=. if `var'==10333333
	...
	***************
	...
	egen ylmpri_ci = rsum(ingpat retpat	ingasa retasa ) 
	replace ylmpri_ci =. if ingpat==. & retpat==. &	ingasa==. & retasa==. 
...
label var tcylmpri_ci "Identificador de top-code del ingreso de la actividad principal"
	...
	***ynlm_ci***
	*************
	replace ingbon = ingbon/10 if ingbon>=100 & ingbon<=900
	egen ynlm_ci=rsum(inginv ingjub ingbon ingotr)
	replace ynlm_ci = . if inginv==. & ingjub==. & ingbon==. & ingotr==.
...
* Line of code with indicator pqnoasis_ci was deleted	**************
	...
	*Mayra Sàenz- Agosto 2013. No existe esta variable en el cuestionario del año 2000. 
...
label var tecnica_ci "=1 formacion terciaria tecnica"
	...
	*El año 2000 la ENEMDU no levanta informacion de vivienda (no existe este modulo)
