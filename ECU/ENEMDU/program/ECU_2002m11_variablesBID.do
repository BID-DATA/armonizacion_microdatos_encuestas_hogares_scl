...
tostring idp_ci, replace
	...
	****zona_c***
	*************
	*gen zona_c=1 if area == 2 | area ==3 | area ==5
	*replace zona_c=0 if area ==1 
	gen zona_c=1 /*sólo es urbana*/
...
*Julio 2021	
    ...
    ***************
	***afroind_ci** Se pregunta solo a personas con 15 años o más
	***************										
**Pregunta- pe13 (0 missing values, 1 indígena, 2 blanco, 3 mestizo, 4 negro, 5 mulato, 6 otro) 
gen afroind_ci=. 
replace afroind_ci=1 if pe14 == 1
replace afroind_ci=2 if pe14 == 4 | pe14 == 5
replace afroind_ci=3 if pe14 == 2 | pe14 == 3 | pe14 ==6 
replace afroind_ci=9 if pe14==0 & edad_ci<15
...
************************
gen gesbuscatra = ((pe30>=1 & pe30<=7) | (pe30a>=1 & pe30a<=7))
*2014, 01 MLO cambio del limite de edad de condocup_ci a 5+
gen condocup_ci=.
...
************
	...
	****************
	gen desemp2_ci=0
	replace desemp2_ci=1 if desemp1_ci==1 | (motnobus>=5 & motnobus<=7) 
...
/*
gen pension_ci=0 
replace pension_ci=1 if (pe66==1) /* A todas las per mayores de cinco*/
replace pension_ci=. if pe66==.
...
gen pension_ci=0 
replace pension_ci=1 if (pe69==1) /* A todas las per mayores de cinco*/
replace pension_ci=. if pe69==.
label var pension_ci "1=Recibe pension contributiva"
...
gen pensionsub_ci=0 
replace pensionsub_ci=1 if pe69==1
...
gen salmm_ci=128.9
	...
	******************
	/*
              ...
              *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
			  5    Cuenta Propia
              *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
			  6    Trab. del hogar no remunerado
			  *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
              
			  7    Trab. Agrop. a sueldo o salario
              8    Jornalero/Peón agropecuario
             *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
			 9    Patrono o socio de la finca
             *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
			 10    Trab. Agrop. por cuenta propia
             *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
			 11    Trab. Fam. Agrop. no remunerado
             *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
			 12    Empleado(a) doméstico(a)
    */
	
	
	gen categopri_ci=.
	replace categopri_ci=1 if catetrab==4 | catetrab==9
	replace categopri_ci=2 if catetrab==5 | catetrab==10
	replace categopri_ci=3 if (catetrab>=1 & catetrab<=3) | catetrab==7 | catetrab==8 | catetrab==12
	replace categopri_ci=4 if catetrab==6 | catetrab==11
   ...
   */
	...
	gen categosec_ci=.
	replace categosec_ci=1 if cates==4 | cates==9
	replace categosec_ci=2 if cates==5 | cates==10
	replace categosec_ci=3 if (cates>=1 & cates<=3) | cates==7 | cates==8 | cates==12
	replace categosec_ci=4 if cates==6 | cates==11	
...
label value tipocontrato_ci tipocontrato_ci
	...
	***contrato_ci***
	*****************
	gen contrato_ci=(estabil>=2 & estabil<=5)
...
replace tipocontrato_ci=2 if (estabil==4)              & categopri_ci==3
replace tipocontrato_ci=3 if (estabil==3 | estabil==5) & categopri_ci==3
label var tipocontrato_ci "Tipo de contrato segun su duracion en act principal"
label define tipocontrato_ci 1 "Permanente/indefinido" 2 "Temporal" 3 "Sin contrato/verbal" 
label value tipocontrato_ci tipocontrato_ci
*/
...
/*
	...
	****************
	g durades_ci=.
	* MGD 04/02/2015: Mal base
	*gen durades_ci=tiembus/4.3 if tiembus != 999
	...
	***************
	...
	gen antiguedad_ci=pe44 if emp_ci==1
...
foreach var of varlist pe61  pe62a  pe63 pe64   pe65a   pe66   ///
 pe67a pe68a pe69a pe70a pe71a pe72a {
...
replace `var'=. if `var'== 999992
	...
	***************
	...
	egen ylmpri_ci = rsum(pe61  pe62a  pe63 pe64) 
	replace ylmpri_ci =. if pe61==. & pe62a==. & pe63==. & pe64==. 
...
label var tcylmpri_ci "Identificador de top-code del ingreso de la actividad principal"
	...
	*************
	...
	replace pe69a = pe72a/100 if pe72a>=1000 & pe72a!=.
	replace pe69a = pe72a/10 if pe72a>=100 & pe72a<=999
	egen ynlm_ci=rsum( pe68a pe69a pe70a pe71a pe72a)
	replace ynlm_ci = . if pe68a==. & pe69a==. & pe70a==. & pe71a==. & pe72a==. 
...
egen ytot_ci = rowtotal(ylm_ci ylnm_ci ynlm_ci ynlnm_ci)
	...
	*************
	...
	/*
	replace nivinst=. if (nivinst>=8)
	replace anoinst=. if (anoinst>=20)
	replace anoinst = 0 if (nivinst == 1)
	replace anoinst = 0 if (nivinst == 3)

	generat aedu_ci = .
	replace aedu_ci = 0          if nivinst==1 | nivinst==3
	replace aedu_ci = anoinst    if nivinst==2 
	replace aedu_ci = anoinst    if nivinst==4
	replace aedu_ci = anoinst+6  if nivinst==5
	replace aedu_ci = anoinst+12 if nivinst==6
	replace aedu_ci = anoinst+17 if nivinst==7
...
/** Modificaciones Marcela Rubio
	...
	replace aedu_ci = 1 if nivinst==4 & anoinst==1
	replace aedu_ci = 2 if nivinst==4 & anoinst==2
	replace aedu_ci = 3 if nivinst==4 & anoinst==3
	replace aedu_ci = 4 if nivinst==4 & anoinst==4
	replace aedu_ci = 5 if nivinst==4 & anoinst==5
	replace aedu_ci = 6 if nivinst==4 & (anoinst>=6 & anoinst<=9)
	replace aedu_ci = 7 if nivinst==5 & anoinst==1
	replace aedu_ci = 8 if nivinst==5 & anoinst==2
	replace aedu_ci = 9 if nivinst==5 & anoinst==3
	replace aedu_ci = 10 if nivinst==5 & anoinst==4
	replace aedu_ci = 11 if nivinst==5 & anoinst==5	
	replace aedu_ci = 12 if nivinst==5 & (anoinst>=6 & anoinst<=9)	
	replace aedu_ci = 13 if (nivinst==6 | nivinst==7) & anoinst==1
	replace aedu_ci = 14 if (nivinst==6 | nivinst==7) & anoinst==2
	replace aedu_ci = 15 if (nivinst==6 | nivinst==7) & anoinst==3
	replace aedu_ci = 16 if (nivinst==6 | nivinst==7) & anoinst==4
	replace aedu_ci = 17 if (nivinst==6 | nivinst==7) & (anoinst>=5 & anoinst<=9)
	replace aedu_ci = 19 if nivinst==8 & anoinst==1
	replace aedu_ci = 20 if nivinst==8 & anoinst==2
	replace aedu_ci = 21 if nivinst==8 & anoinst==3
	replace aedu_ci = 22 if nivinst==8 & anoinst==4
	replace aedu_ci = 23 if nivinst==8 & (anoinst>=5 & anoinst<=9)
	label var aedu_ci "Anios de educacion aprobados" 
	label value aedu_ci aedu_ci */
...
*	Mod Ivan B./EDU 2015,12
	...
	gen aedu_ci = .
	replace aedu_ci = 0   if nivinst==1 // Ningun nivel educativo
	replace aedu_ci = .   if nivinst==2 // Centro de alfabetización
	replace aedu_ci = 0   if nivinst==3 // Jardin de infantes
	replace aedu_ci= anoinst-1 if nivinst==4
	replace aedu_ci=0 if aedu_ci==-1
	replace aedu_ci = anoinst+6    if nivinst==5
	replace aedu_ci = anoinst+12  if nivinst==6 | nivinst==7
	replace aedu_ci = anoinst+17 if nivinst==8
	replace aedu_ci =. if (nivinst==. | anoinst==.)
	label var aedu_ci "Anios de educacion aprobados" 
	label value aedu_ci aedu_ci
...
* Line of code with indicator pqnoasis_ci was deleted* Line of code with indicator pqnoasis_ci was deleted* Line of code with indicator pqnoasis_ci was deleted* Line of code with indicator pqnoasis_ci was deleted* Line of code with indicator pqnoasis_ci was deleted* Line of code with indicator pqnoasis_ci was deleted* Line of code with indicator pqnoasis_ci was deleted	
...
label var tecnica_ci "=1 formacion terciaria tecnica"
	...
	*El año 2002 la ENEMDU sí levanta informacion de vivienda pero no se dispone de ese módulo.
...
*label var sinbano_ch "= 0 si tiene baño en la vivienda o dentro del terreno"
	...
	*************
	...
	replace des1_ch=1 if vi06==1 | vi06==2
	replace des1_ch=2 if vi06==3 | vi06==4
	...
	*************
	...
	replace des2_ch=1 if vi06==1 | vi06==2 | vi06==3 | vi06==4
	...
	*************
	gen piso_ch=. 		
	replace piso_ch=0 	if vi02==6
	replace piso_ch=1 	if vi02>=1 & vi02<=3
	replace piso_ch=2 	if vi02==4 | vi02==5 | vi02==7 
	replace piso_ch=. 	if vi02==.
	...
	***pared_ch***
	**************
	* variable no está disponible en esta encuesta
	gen pared_ch=.
	...
	***resid_ch***
	**************
	gen resid_ch =0    if vi12==1 | vi12==2
	replace resid_ch=1 if vi12==4
	replace resid_ch=2 if vi12==3
	replace resid_ch=3 if vi12==5
	replace resid_ch=. if vi12==.
	...
	*para dormir, pero que viven en vivienda de 2, 3 y 4 habitaciones se les asigna missing
	gen dorm_ch=vi04 
	replace dorm_ch=1 if vi04==0 & vi03==1
	replace dorm_ch=. if vi04==0 & vi03>1
	...
	***vivi1_ch***
	**************
	* Variable no esta disponible para este año
	gen vivi1_ch=.
	...
	*****************
	gen viviprop_ch=.
	replace viviprop_ch=0 if vi01==1
	replace viviprop_ch=1 if vi01==2
	replace viviprop_ch=2 if vi01==3
	replace viviprop_ch=3 if vi01>=4 & vi01<=6
