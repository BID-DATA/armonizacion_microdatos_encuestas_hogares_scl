...
*global base_in  = "$ruta\survey\\`PAIS'\\`ENCUESTA'\\`ANO'\\`ronda'\data_orig\\`PAIS'_`ANO'`ronda'.dta"
...
*cotizando_ci***
****************
/*solo a emplead@s y asalariad@s, difiere con los otros paises*/
...
/*
	...
	* Ultima actualizacion MGD 4/16/2014
	...
	gen rama_ci=.
	replace rama_ci = 1 if (p40>=111  & p40<=502) & emp_ci==1
	replace rama_ci = 2 if (p40>=1010 & p40<=1429) & emp_ci==1
	replace rama_ci = 3 if (p40>=1511 & p40<=3720) & emp_ci==1
	replace rama_ci = 4 if (p40>=4010 & p40<=4100) & emp_ci==1
	replace rama_ci = 5 if (p40>=4510 & p40<=4550) & emp_ci==1
	replace rama_ci = 6 if (p40>=5010 & p40<=5520) & emp_ci==1
	replace rama_ci = 7 if (p40>=6010 & p40<=6420) & emp_ci==1
	replace rama_ci = 8 if (p40>=6511 & p40<=7499) & emp_ci==1
	replace rama_ci = 9 if (p40>=7500 & p40<=9900) & emp_ci==1
...
* Line of code with indicator pqnoasis_ci was deleted	**************
    recode p09 (13=17)	// JV: No tiene opción "Falta de recursos tecnológicos". Recoded accordingly
...
replace aguafuente_ch = 10 if (vi08==6|vi08==9|vi08==4)
...
gen aguatrat_ch =9
	...
	*************
	...
	replace des1_ch=1 if vi07==1 | vi07==2
	replace des1_ch=2 if vi07==3 | vi07==4
	...
	*************
	...
	replace des2_ch=1 if vi07==1 | vi07==2 | vi07==3 | vi07==4
	...
	***piso_ch***
	*************
	gen piso_ch=1 		if vi03==1 |vi03==2|vi03==3 
	replace piso_ch=0 	if vi03==6
	replace piso_ch=1 	if vi03==4 |vi03==5|vi03==7 	
	replace piso_ch=. 	if vi03==.
	...
	*para dormir, pero que viven en vivienda de 2, 3 y 4 habitaciones se les asigna missing
	gen dorm_ch=vi05 if vi05!=99
	*Modificación Mayra Sáenz Agosto 2013. Se excluye a los que declaran 99 en vi05, que son los que no informan.
	replace dorm_ch=1 if vi05==0 & vi04==1
	replace dorm_ch=. if vi05==0 & vi04>1
	label var dorm_ch "Habitaciones para dormir"
	
	****************
	***cuartos_ch***
	****************
	gen cuartos_ch=vi04 if vi04!=99
	*Modificación Mayra Sáenz Agosto 2013. Se excluye a los que declaran 99 en vi04, que son los que no informan.
	...
	**************
	gen telef_ch=0
	replace telef_ch=1 if vi1415a==1
	replace telef_ch=. if vi1415a==.
	...
	***************
	gen refrig_ch=0
	replace refrig_ch=1 if  vi1401a==1
	replace refrig_ch=. if  vi1401a==.
	...
	*************
	gen auto_ch=0
	replace auto_ch=1 if vi1414a==1
	replace auto_ch=. if vi1414a==.
	...
	**************
	gen compu_ch=0
	replace compu_ch=1 if vi1404a==1
	replace compu_ch=. if vi1404a==.
