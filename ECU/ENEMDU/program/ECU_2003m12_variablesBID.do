...
*******************************************************
	...
	**Solo se pregunta a personas de 15 años o mas. 
	...
	gen byte afro_ci = . 
	replace afro_ci = 1 if pe13 == 4 | pe13 == 5
	replace afro_ci = 0 if pe13 != 4 & pe13 != 5 & pe13 != .
	...
	*********	
	gen byte ind_ci = .
	replace ind_ci = 1 if pe13 == 1
	replace ind_ci = 0 if pe13 != 1 & pe13 != .
...
label var pea_ci "Población Económicamente Activa"
	...
	***horaspri_ci***
	*****************
	gen horaspri_ci=hortrahp if hortrahp<150 
...
replace afiliado_ci = 1 if ((iess>=2 & iess<=5) & emp_ci==1)
replace afiliado_ci = 0 if (iess > 5 & inlist(condocup_ci, 1, 2))
...
replace cotizando_ci = 1 if (pe42f==1 & emp_ci==1)  /*Emplead@s y asalariad@s, ocupados y desoc*/
...
recode pe63 ( 222211=. ) (70000=.)
recode pe65b (222222=.) (300222=.) (150222=.) (70000=.)
...
*****************
gen bano_ch=6
replace bano_ch=0 if serv_hig==5
replace bano_ch=1 if serv_hig==1
replace bano_ch=2 if serv_hig==2
replace bano_ch=3 if serv_hig==3 
replace bano_ch=6 if serv_hig==4
...
************
gen sinbano_ch = 3
replace sinbano_ch = 0 if serv_hig!=5
...
gen aguatrat_ch =9
	...
	*/
	*Modificación Mayra Sáenz - Agosto 2013
	gen luz_ch=(alumbrad==1|alumbrad==2)
	label var luz_ch  "La principal fuente de iluminación es electricidad"
	...
	****************
	gen combust_ch=0
	replace combust_ch=1 if combusti==1 | combusti==3 
	replace combust_ch=. if combusti==.
	...
	*************
	gen des1_ch=.
	replace des1_ch=0 if serv_hig==5
	replace des1_ch=1 if serv_hig==1 | serv_hig==2
	replace des1_ch=2 if serv_hig==3 | serv_hig==4
	...
	* Modificaciones Marcela Rubio Septiembre 2014
	/*
	gen piso_ch=.
	replace piso_ch=0 	if piso_v==6
	replace piso_ch=1 	if piso_v==1 | piso_v==2 | piso_v==3 	
	replace piso_ch=2 	if piso_v==4 | piso_v==5 | piso_v==6
	...
	*/
	...
	replace piso_ch=0 	if piso_v==6
	replace piso_ch=1 	if piso_v==1 | piso_v==2 | piso_v==3 	
	replace piso_ch=2 	if piso_v==4 | piso_v==5 | piso_v==7
	label var piso_ch "Materiales de construcción del piso"  
	label def piso_ch 0"Piso de tierra" 1"Materiales permanentes" 2"Otros materiales"
	label val piso_ch piso_ch
	...
	*************
	...
	gen dorm_ch=dormitor
	replace dorm_ch=1 if dormitor==0 & vi03==1
	replace dorm_ch=. if dormitor==0 & vi03>1
	...
	***************
	...
	gen refrig_ch=0
	replace refrig_ch=1 if  refriger==1
	replace refrig_ch=. if  refriger==.
