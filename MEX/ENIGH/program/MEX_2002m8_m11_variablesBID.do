...
Agosto 28 de 2006 (MFP)
el programa que generaba la variable emp3_ci NO incluia a los trabajadores familiares sin remuneracion 
para esto se incluyeron dos condiciones:
gen trab_nr1=substr(trabajo,4,1)
...
gen relacion_ci=.
replace relacion_ci=1 if paren>=10 & paren<=12
replace relacion_ci=2 if paren==20 
replace relacion_ci=3 if paren>=30 & paren<=31
replace relacion_ci=4 if paren>=40 & paren<=59
replace relacion_ci=5 if paren>=60 & paren<=63
replace relacion_ci=5 if paren>=80 & paren<=90
replace relacion_ci=6 if paren==70
...
****condocup_ci*
****************
gen trab_rem=substr(trabajo,1,1)
gen trab_srem=substr(trabajo,2,1)
*Trabajo independiente remunerado
gen trab_espo=substr(trabajo,3,1)
gen trab_nr1=substr(trabajo,4,1)
gen trab_nr2=substr(trabajo,5,1)
*la variable trabajo resume las primeras cinco preguntas (02-06) del formulario.
generat condocup_ci=.
replace condocup_ci=1 if (trab_rem=="1" | trab_srem=="1" | trab_espo=="1"  | trab_nr1=="1" | trab_nr2=="1")
replace condocup_ci=1 if (trab_rem=="2" & trab_srem=="2" & trab_espo=="2" & trab_nr1=="2" & trab_nr2=="2") & (no_trabajo>="01" & no_trabajo<="09")
replace condocup_ci=2 if (trab_rem=="2" & trab_srem=="2" & trab_espo=="2" & trab_nr1=="2" & trab_nr2=="2") & (no_trabajo>="10" & no_trabajo<="11")
...
forval i=1(1)20 { 
replace cotizando_ci=1 if (presta1_`i'>="01" & presta1_`i'<="03") | (presta1_`i'=="09") & cotizando_ci==0
}
...
*lpe_ci***
*********
generat lpe_ci =672.27 if zona_c==1
replace lpe_ci =494.77  if zona_c==0
label var lpe_ci "linea de pobreza alimentaria oficial del pais"

*********
*lp2_ci***
*********
generat lp2_ci =824.54 if zona_c==1
replace lp2_ci =584.97  if zona_c==0
label var lp2_ci "linea de pobreza de capacidades"

*********
*lp_ci***
*********
generat lp_ci =1348.84 if zona_c==1
replace lp_ci =897.81 if zona_c==0
...
generat salmm_ci=42.15*30 if zona_salmm==1
replace salmm_ci=40.10*30 if zona_salmm==2
replace salmm_ci=38.30*30 if zona_salmm==3
...
gen trab=(trab_rem=="1" | trab_srem=="1" | trab_espo=="1"  | trab_nr1=="1" | trab_nr2=="1") /*ultimo mes*/
replace trab=2 if (trab_rem=="2" & trab_srem=="2" & trab_espo=="2" & trab_nr1=="2" & trab_nr2=="2")
gen byte causa=real(no_traba)  /*en ultimo mes*/

gen byte emp3_ci=(trab==1)
replace emp3_ci=1 if causa>0 & causa<=9
/* Es empleado: en el ultimo mes*/
replace emp3_ci=0 if causa==6 
/* no esta empleado porque esta esperando para que comiencen las labores agricola- es decir inactv*/
/* Es empleado: en el ultimo mes*/
...
gen desemp3_ci=(trab==2 & causa>=10 & causa<=11)
replace desemp3_ci=0 if causa>0 & causa<=9
...
gen categopri_ci=.
replace categopri_ci=1 if posicion=="6" | posicion=="7"
replace categopri_ci=2 if posicion=="5" 
replace categopri_ci=3 if posicion=="1" | posicion=="2" | posicion=="3" | posicion=="4" 
replace categopri_ci=4 if posicion=="8" 
...
gen categosec_ci=.
replace categosec_ci=1 if posici_s  =="6" | posici_s  =="7"
replace categosec_ci=2 if posici_s  =="5" 
replace categosec_ci=3 if posici_s  =="1" | posici_s  =="2" | posici_s  =="3" | posici_s  =="4" 
replace categosec_ci=4 if posici_s  =="8"
...
replace tipocontrato_ci=1 if contrato1=="1" 
replace tipocontrato_ci=2 if contrato1=="2"              
replace tipocontrato_ci=3 if contrato1=="3" | contrato1=="4"
label var tipocontrato_ci "Tipo de contrato segun su duracion en act principal"
label define tipocontrato_ci 1 "Permanente/indefinido" 2 "Temporal" 3 "Sin contrato/verbal" 
label value tipocontrato_ci tipocontrato_ci
*/
* Modificacion MGD condicionando a categopri_ci 06/17/2014
destring contrato1, replace
generat tipocontrato_ci=. /* Solo disponible para asalariados y trab independ*/
replace tipocontrato_ci=1 if contrato1==1 & categopri_ci==3
replace tipocontrato_ci=2 if contrato1==2 & categopri_ci==3             
replace tipocontrato_ci=3 if (contrato1>=3 | tipocontrato_ci==.) & categopri_ci==3
label var tipocontrato_ci "Tipo de contrato segun su duracion en act principal"
label define tipocontrato_ci 1 "Permanente/indefinido" 2 "Temporal" 3 "Sin contrato/verbal" 
label value tipocontrato_ci tipocontrato_ci
...
gen tcontrato_ci=.
replace tcontrato_ci=1 if contrato1=="1"
replace tcontrato_ci=0 if contrato1=="2" 
...
gen byte tot_tra=numper
gen byte tamfirma_ci=.
replace tamfirma_ci=0 if tot_tra<5
replace tamfirma_ci=1 if tot_tra>=5
/*
...
***categoinac_ci***
*******************
gen categoinac_ci =1 if ( no_trabajo=="13" & condocup_ci==3)
...
gen utilidades=ing_rpm /*renta*/
gen ayuda=ing_tm /*tranferencias*/
gen otros=ing_otm+ing_otm1 /*financieros y otros*/
...
replace yedc=9.5 if educab==12
replace yedc=10 if educab==13
replace yedc=10.5 if educab==14
replace yedc=11 if educab==15
replace yedc=11.5 if educab==16 | educab==17
replace yedc=12 if educab==18 
replace yedc=12.5 if educab==19
replace yedc=13 if educab==20 

*replace yedc=13.5 if educab==21 
replace yedc=12.5 if educab==21 

*replace yedc=14 if educab==22 
replace yedc=13 if educab==22 

*replace yedc=14.5 if educab==23 
replace yedc=13.5 if educab==23 

*replace yedc=15 if educab==24 
replace yedc=14 if educab==24 

*replace yedc=15.5 if educab==25
replace yedc=14.5 if educab==25

*replace yedc=16 if educab==26 
replace yedc=15 if educab==26 

*replace yedc=16.5 if educab==27 
replace yedc=15.5 if educab==27 

*replace yedc=17 if educab==28
replace yedc=16 if educab==28

*replace yedc=17.5 if educab==29
replace yedc=16.5 if educab==29

replace yedc=14 if educab==30

*replace yedc=18 if educab==31
replace yedc=17 if educab==31

*replace yedc=20 if educab==32
replace yedc=18 if educab==32

*replace yedc=21 if educab==33
replace yedc=21 if educab==33
...
***asiste_ci***
***************
destring asis_esc_f asis_esc_t,replace

gen asiste_ci=0
replace asiste_ci=1 if asis_esc_f==1 | asis_esc_t==1
...
********************************************
destring  agua* luz* bano* muros* techos* drenaje* combusti basura* ,replace
destring pisos num_cua cua_coc cua_dor  tipo_viv tenencia01 vehi* serv* apar*,replace

***************
**aguared_ch***
***************

gen aguared_ch=agua23
...
gen des1_ch=.
replace des1_ch=0 if drenaje35==0 
replace des1_ch=1 if drenaje35==2 | drenaje35==3
replace des1_ch=2 if drenaje35==4 
replace des1_ch=3 if drenaje35==1 
...
replace pared_ch=1 if muros1>=7 & muros1<=16
replace pared_ch=2 if muros1==17
drop muros1
...
replace techo_ch=1 if techo1>=4 & techo1<=16
replace techo_ch=2 if techo1==17
drop techo1
...
gen basura=basura38_1
gen resid_ch=0 if basura==6 | basura==7 
replace resid_ch=1 if basura==2 | basura==4
replace resid_ch=2 if basura==1 | basura==3
replace resid_ch=3 if basura==5 | basura==8
drop basura
...
***aguamejorada_ch***
*********************
g       aguamejorada_ch = 1 if (agua23 ==1 & (agua24 ==1 | agua24 ==2)) | agua25 ==1
replace aguamejorada_ch = 0 if (agua23 ==2 & (agua24 ==1 | agua24 ==2)) | agua25 ==2
		
*********************
***banomejorado_ch***
*********************
g       banomejorado_ch = 1 if (bano31 ==1 & (bano32>=1 &  bano32<=3) & (drenaje35 >=2 &  drenaje35<=3) & bano34 ==1) | (bano33 ==1 & bano34 ==1) 
replace banomejorado_ch = 0 if (bano31 ==1 & (bano32>=1 &  bano32<=3) & (drenaje35 >=2 &  drenaje35<=3) & bano34 ==2) | (bano33 ==1 & bano34 ==2) | ((bano31 ==1) & (drenaje35==1 |  drenaje35==4)) | bano31 ==2 | bano32 ==4
...
gen telef_ch=serv53_1+serv54_1
...
gen refrig_ch=apar59_24+apar60_24
replace refrig_ch=1 if refrig_ch>0
...
gen auto_ch=vehi56_1+vehi57_1+vehi56_2+vehi57_2+vehi56_3+vehi57_3
replace auto_ch=1 if auto_ch>0
...
gen compu_ch=apar59_10+apar60_10 
replace compu_ch=1 if compu_ch>0
...
gen internet_ch=serv53_4+serv54_4
replace internet_ch=1 if internet_ch>0

****************
****cel_ch******
****************

gen cel_ch=serv53_2+serv54_2
replace cel_ch=1 if cel_ch>0
...
gen viviprop_ch=.
replace viviprop_ch=0 if tenencia01==5 
replace viviprop_ch=1 if tenencia01>=1 & tenencia01<=3
replace viviprop_ch=2 if tenencia01==4
replace viviprop_ch=3 if tenencia01>=6 
