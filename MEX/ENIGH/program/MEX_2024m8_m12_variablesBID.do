...
****************************
	* Matriculación actual (nivel/grado) -- Esto está desactualizado, no corresponde a la codificación de ENIGH 2024
	/*
	...
	* QA manual
	...
	replace grado_top = 4 if nivelaprob == 5 & gradoaprob > 4 // normal
	replace grado_top = 5 if nivelaprob == 7 & gradoaprob > 4 // licenciatura
	replace grado_top = 3 if nivelaprob == 6 & gradoaprob > 2 // técnica o comercial
	replace grado_top = 3 if inlist(nivelaprob, 8, 9, 10) & gradoaprob > 3 // especialidad, maestria, doctorado
	
	
	// Nivel y grado aprobado 
	gen byte aedu_ci = . 
	replace aedu_ci = 0 if inlist(nivelaprob, 0, 1)  // ninguno o inicial
	replace aedu_ci = gradoaprob if nivelaprob == 2 // primaria
	replace aedu_ci = 6 + gradoaprob if nivelaprob == 3 // secundaria
	replace aedu_ci = 9 + gradoaprob if nivelaprob == 4 // prepa
	replace aedu_ci = 12 + grado_top if inrange(nivelaprob, 5, 7) // normal, tecnicatura, licenciatura
	replace aedu_ci = 16 + grado_top if inrange(nivelaprob, 8, 9) // especialidad, maestría
	replace aedu_ci = 19 + grado_top if nivelaprob== 10 // doctorado
	...
	**********
	...
	/*
		replace eduui_ci 
	replace eduui_ci = ///
		  (aedu_ci>12 & aedu_ci<16 & drnivelaprob==`LAP_SUPT')  | ///
		  (aedu_ci>12 & aedu_ci<17 & nivelaprob==`LAP_GRAD')  | ///
		  (aedu_ci>12 & aedu_ci<15 & nivelaprob==`LAP_PROF' & inlist(`antec_num',2,3))
		  */
	
	// Corregida - antes daba 0.31% lo que es demasiado bajo 
	
	// están estudiando actualmente superior, o su nivel más alto es superior pero no finalizaron sus estudios
	gen byte eduui_ci = 0
	replace eduui_ci = 1 if inrange(nivel, 10,11) // estan estudiando superior
	replace eduui_ci = 1 if nivelaprob == 5  & gradoaprob<4 // nivel máx superior incompleto (normal)
	replace eduui_ci = 1 if nivelaprob == 7  & gradoaprob<5 // nivel máx superior incompleto (lic)
	replace eduui_ci = 1 if nivelaprob == 6 & gradoaprob<3 // nivel máx superior incompleto (tecnicatura)
	replace eduui_ci = 0 if aedu_ci < 12
	...
	**********	
	...
	// Revisada
	gen byte eduuc_ci = .
	replace eduuc_ci = 1 if inrange(nivelaprob, 8,10) // estudios de posgrado
	replace eduuc_ci = 1 if nivelaprob == 5  & gradoaprob>=4
	replace eduuc_ci = 1 if nivelaprob == 7  & gradoaprob>=5
	replace eduuc_ci = 1 if nivelaprob == 6  & gradoaprob>=3
	replace eduuc_ci = 0 if aedu_ci <= 14
	replace eduuc_ci = . if aedu_ci==. 
	
	
	/*
	replace eduuc_ci = ///
		((aedu_ci>=14) & nivelaprob==`LAP_SUPT')              | ///
		((aedu_ci>=16) & nivelaprob==`LAP_GRAD')              | ///
		((aedu_ci>=16) & nivelaprob==`LAP_PROF' & inlist(antec_esc,2,3))  | ///
	...
	**********
	...
	gen byte eduac_ci = .
	replace eduac_ci = 1 if inrange(nivelaprob, 7, 10) // universitarios
	replace eduac_ci = 0 if inlist(nivelaprob, 5,6) // tecnicatura, normal
