...
*******************************************************
	...
	*afro_ci*
	*********
	**No tiene los labels --> se suponen los mismos de loas anteriores años
...
****************************
	...
	*******************
	...
	replace categoinac_ci = 1 if (ea17== 3 & condocup_ci == 3)
	replace categoinac_ci = 2 if  (ea17 == 2 & condocup_ci == 3)
	replace categoinac_ci = 3 if  (ea17 ==1 & condocup_ci == 3)
	...
	**************
	gen byte cesante_ci = .
	replace cesante_ci = 1 if condocup_ci == 2 & ea11 == 1
	replace cesante_ci = 0 if condocup_ci == 2 & ea11 == 2
	...
	***************
	...
	replace subemp_ci = 1 if total_hrs_last_week<=30  & total_hrs_last_week != . & ea33 == 1 & ea34== 1 & emp_ci == 1  //se utiliza la pregunta:  Available for additional work
	...
	****************
	...
	replace nempleos_ci = 1 if ea21==2 & emp_ci == 1 
	replace nempleos_ci = 2 if ea21==1 & emp_ci == 1 
	...
	***************	
	...
	replace horaspri_ci = total_hrs_last_week if total_hrs_last_week!= 999999 &  total_hrs_last_week!=. & ea19==2  & emp_ci == 1 //quedaria en missing las presonas que tienen más de un trabajo 
	...
	***************	
	...
	replace tiempoparc_ci  =( total_hrs_last_week<30 & ea33 == 2) if  total_hrs_last_week!= 999999 &  total_hrs_last_week!=.  & emp_ci == 1 
	...
	* Solo se tiene en cuenta los empleados del gobierno 
	...
	replace spublico_ci  = 0 if  ea25_1!=3 & emp_ci == 1 & ea25_1<8 
	replace spublico_ci  = 1 if  ea25_1==3 & emp_ci == 1  & ea25_1<8
	...
	**************
	gen byte ocupa_ci=.
	replace ocupa_ci=1 if ea23main_occ==2 | ea23main_occ==3 & emp_ci==1
	replace ocupa_ci=2 if  ea23main_occ==1 & emp_ci==1
	replace ocupa_ci=3 if  ea23main_occ==4 & emp_ci==1
	replace ocupa_ci=6 if  ea23main_occ==6 | ea23main_occ==7 & emp_ci==1
	replace ocupa_ci=7 if  ea23main_occ==8 & emp_ci==1
	replace ocupa_ci=8 if  ea23main_occ==0  & emp_ci==1
	replace ocupa_ci=9 if ea23main_occ==5 | ea23main_occ==9 & emp_ci==1
...
****************************
	...
	**************
	...
	replace razonesnoasis_ci = 1 if ed6 == 2 | ed6==3
	replace razonesnoasis_ci = 2 if ed6 == 7
	replace razonesnoasis_ci = 3 if ed6==4 | ed6==6
	replace razonesnoasis_ci = 4 if ed6 == 5
	replace razonesnoasis_ci = 5 if  ed6 == 888888 | ed6 == 1
...
****************************		
	...
	***********
	gen techo_ch=.
	* NOTA: REVISANDO METODOLÓGIA AÚN NO CREAR
	replace techo_ch = 0 if inlist(hh9, 8)
	replace techo_ch = 1 if inlist(hh9, 1, 2, 3, 5, 4)
	...
	*telef_ch*
	***********
	gen telef_ch=(hh13b==1) if hh13b!=999999
	...
	*refrig_ch*
	***********
	gen refrig_ch=(hh12b>1) if hh12b!=999999
	...
	*auto_ch*
	***********
	gen auto_ch=(hh12q>1) if hh12p!=999999

	
	***********
	*compu_ch*
	***********
	gen compu_ch=(hh12m>1) if hh12m!=999999

		
	***********
	*internet_ch*
	***********
	gen internet_ch=(hh13c==1) if hh13c!=999999
...
****************************		
    ...
    ****************
	...
	replace migrante_ci=1 if hl7new==3
