...
Round: t1 2025
Autores: Rocio Senra
Versión: Rocio Senra (SCL/GDI) - Email: rsenra@iadb.org, Fecha: Febrero 2026
...
**********************************
	...
	******
	gen zona_c=. 
	replace zona_c = 1 if p00a10==1 //Urbana
	replace zona_c = 0 if p00a10==2 //Rural
...
****************************
	...
	*********
	gen byte sexo_ci= .
	replace sexo_ci = 1 if p02a02 == 1
	replace sexo_ci = 2 if p02a02 == 2
...
**********************************
	...
	***************	
	...
	replace horaspri_ci = . if p05h01c == 999 
...
****************************
	...
	*************
	...
	/* 2025:
	(p05d07b) CUÁNTO DINERO RECIBIÓ bonos de productividad, de desempeño o por estímulos laborales
	(p05d08b) CUÁNTO DINERO RECIBIÓ ¿Cuánto le pagaron por trabajar en su período vacacional? 
	(p05d09b) CUÁNTO DINERO RECIBIÓ bono 14
	(p05d10b) CUÁNTO DINERO RECIBIÓ aguinaldo
	(p05d11b) CUÁNTO DINERO RECIBIÓ bono vacacional
	(p05d12b) CUÁNTO DINERO RECIBIÓ algún quinceavo sueldo o diferido
	...
	*/
	foreach var of varlist  p05g13b p05g14b{
	g `var'tdp_sec=`var'/12
	}
	egen ylmsec_ci=rsum(p05g12b p05g10 *tdp_sec p05g26), missing
	...
	*/
	foreach var of varlist  p06b01b p06b02b p06b03b{
	g `var'tdpotros=`var'/12
	}
	
    egen ylmotros_ci = rowtotal(*tdpotros) if emp_ci==1
...
****************************
	...
	*pqnoasis1_ci*
	**************
	gen pqnoasis1_ci=.  				// No incluye pregunta sobre asistencia. 
...
****************************		
    ...
    ****************
	...
	replace migrante_ci = 1 if p02a05a==3
	replace migrante_ci = 0 if p02a05a!=3
