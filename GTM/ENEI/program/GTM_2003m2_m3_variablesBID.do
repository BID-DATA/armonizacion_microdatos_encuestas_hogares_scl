...
**            1 kiché 2 Qeqchí 3 Kaqchikel 4 Mam 5 Garífuna 6 Ladino 7 Extranjero 98 Otro ¿cual?

	*********
	*afro_ci*
	*********
	gen byte afro_ci = .
	replace afro_ci = 1 if p03a04 == 5 
	replace afro_ci = 0 if p03a04 != 5 
	
	*********
	*ind_ci*
	*********	
	gen byte ind_ci = . 		
	replace ind_ci = 1  if p03a04 != 6 & p03a04 != 7 
	replace ind_ci = 0  if p03a04 == 6 | p03a04 == 7 
...
replace desalent_ci = 1 if (p04a10 == 2 & (p04a11 ==2 | p04a11 ==3) & condocup_ci == 3)
...
replace cotizando_ci = 1 if ((p05a25 == 1 & (p05a26 > 0 & p05a26 != .)) & emp_ci==1)
...
replace afiliado_ci = 1 if(p05a25==1 & emp_ci==1)
...
gen vtas =p10b02b/12
gen otrostrab =p10b03b/12
gen otrostrab2 =p10b04b/12
...
***ylmotros_ci***
*****************
egen ylmotros_ci=rsum(incentivo vtas otrostrab otrostrab2), missing
replace ylmotros_ci =. if (incentivo==. & vtas==. & otrostrab==. & otrostrab2==.)
...
* Line of code with indicator pqnoasis_ci was deleted* Line of code with indicator pqnoasis_ci was deleted* Line of code with indicator pqnoasis_ci was deleted              2    "Falta de maestro" ///
...
****************
generate aguared_ch =.
replace aguared_ch = 1 if p02a03a==1
replace aguared_ch = 0 if p02a03a==2
...
rename p05a03 codindustria
rename p05a02 codocupa
