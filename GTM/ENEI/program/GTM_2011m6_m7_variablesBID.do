...
*Con la variable hogar se generan hogares con 2926 miembros.
*g idh_ch=hogar
g idh_ch=formulario
...
recode ppa06 (4/10=4 "Otros parientes") (12 =5 "Otros no parientes") (11=6 "Empleado/a domestico/a"), g(relacion_ci)
...
**            1 xinka; 2 garifuna; 3 ladino; 4 extranjero; 5 maya
	...
	*********
	...
	replace afro_ci = 0 if ppa07 != 2 & ppa07 != . & ppa07 != 0
	...
	*********	
	...
	replace ind_ci = 0  if (ppa07 != 1 & ppa07 != 2 & ppa07 != 5) & ppa07 != . & ppa07 != 0
...
*cotizando_ci***
****************
* MGD 03/2016: a partir de este año se deja de generar cotizando a pesar de que si existe la variable
...
*************
gen rama_ci=.
replace rama_ci=1 if p04c04b>=1 & p04c04b<=3
replace rama_ci=2 if p04c04b>=5 & p04c04b<=9
replace rama_ci=3 if p04c04b>=10 & p04c04b<=34
replace rama_ci=4 if p04c04b>=35 & p04c04b<=39
replace rama_ci=5 if p04c04b>=41 & p04c04b<=43
replace rama_ci=6 if (p04c04b>=45 & p04c04b<=47) | (p04c04b>=55 & p04c04b<=56)
replace rama_ci=7 if (p04c04b>=49 & p04c04b<=53) | (p04c04b>=58 & p04c04b<=63)
...
*egen rem=rsum(p05a18b p05a19b), missing

*Modificación Mayra Sáenz Septiembre,2014 la variable p05a20b hace referencia a las remesas que no provienen 
*de familiares en el exterior
*SGR 05/11 Se vuelve al codigo original dado que los tres meses las remesas vienen de familiares en el exterior.
...
// Limpieza de errores de codificación:
replace p03a02 = . if p03a02 == 0 | p03a02 == 4
replace p03a03 = . if p03a03 == 6 
replace po3a04a = . if po3a04a == 0
...
gen aedu_ci = . 
replace aedu_ci = 0 if (po3a05a == 0 | po3a05a == 1) // Ninguno, Preprimaria
replace aedu_ci = po3a05b if po3a05a == 2 // Primaria
replace aedu_ci = po3a05b + 6 if po3a05a == 3 // Básico 
replace aedu_ci = po3a05b + 9 if po3a05a == 4 // Diversificado 
replace aedu_ci = po3a05b + 11 if po3a05a == 5 // Superior
replace aedu_ci = po3a05b + 16 if po3a05a == 6 // Maestría
// En este anio no se reporta doctorado. 
...
***eduui_ci***
**************
gen eduui_ci = (((po3a04a == 5 & po3a05a == 4) | po3a05a == 5) &  inrange(p03a06b, 103, 499)) 
...
***eduuc_ci***
**************
gen eduuc_ci = ((po3a05a == 5 & inrange(p03a06b, 500, 7100)) | po3a05a == 6 | po3a04a == 6)
...
***eduac_ci***
**************
gen eduac_ci = (inrange(p03a06b, 2002, 7020))
...
*/
g byte asispre_ci = 0 
replace asispre_ci = 1 if po3a04a == 1
