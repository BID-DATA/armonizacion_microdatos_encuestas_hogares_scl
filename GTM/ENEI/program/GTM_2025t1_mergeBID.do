...
import spss "$ruta\Hogares_ENEIC_T1-2025.sav" , clear
rename FACTOR factor_h
*label values * // se remueven labels porque hay conflicto con base a nivel de peronas
save "$ruta\ENEIC-1-2025_HOGARES.dta", replace 

import spss "$ruta\Personas_ENEIC_T1_2025.sav",clear
rename FACTOR factor_p
save "$ruta\ENEIC-1-2025_PERSONAS.dta", replace

*2. Merge

use "$ruta\ENEIC-1-2025_PERSONAS.dta", clear
sort NUM_HOGAR
merge m:1 NUM_HOGAR using "$ruta\ENEIC-1-2025_HOGARES.dta"
tab _merge
drop _merge
rename *, lower
