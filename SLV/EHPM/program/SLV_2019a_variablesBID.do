...
Encuesta: EHPM
Round: a
Autores:  Daniela Zuluaga - danielazu@iadb.org 
Fecha última modificación: Julio de 2019
...
* PNC - Pensiones sociales no contributivas: (Información por hogar)
	* 1 Pensión Básica Universal del Adulto Mayor r901==1 $ r903 r902
		// r901 ¿ALGUIEN EN EL HOGAR RECIBE PENSIÓN BÁSICA UNIVERSAL POR SER ADULTO MAYOR?
		// r902 ¿CON QUÉ PERIODICIDAD RECIBE LA PENSIÓN BÁSICA UNIVERSAL?
...
* PTMC - Programas de transferencias monetarias condicionadas:
	...
	* 3 Bono para Comunidades Solidarias (rurales y urbanas) r321a3== 1| r321a4== 1 // nivel hogar ? $ r919 r918 r922 r921
...
*** Beneficiarios a nivel individual:
	...
	gen byte becagob_ci = .   // Información solo a nivel hogar
	gen byte solidario_ci = . // Información solo a nivel hogar
	
	gen byte ptmc_ci = (becaesc_ci == 1 | becagob_ci == 1 | solidario_ci == 1)
...
**  ypensub_ci  *
*****************
* No se puede determinar el monto de la pension basica universal individual (sólo nivel hogar r903)
gen ypensub_ci = .
...
*** Beneficiarios a nivel hogar:
	gen byte pnc_ch = (r901 == 1) if !missing(r901) & miembros_ci == 1
	
	bys idh_ch: egen byte becaesc_ch = max(becaesc_ci)	
	gen byte becagob_ch = (r912 == 1) if !missing(r912)
	gen byte solidario_ch = (r917 == 1 | r920 == 1)	
	replace solidario_ch = . if r917 == . & r920 == .
...
*** Montos de transferencias a nivel hogar:
	...
	gen double ypnc_ch = r903 if r902 == 1 	 // 1 Mensual
	replace ypnc_ch = r903/2 if r902 == 2	 // 2 Bimensual
	replace ypnc_ch = r903/3 if r902 == 3 | r902 == 4	 // 3 Trimestral y 4 Otros
	replace ypnc_ch = r903/4 if r902 == 5	 // 5 Tres veces al año
	replace ypnc_ch = . if r903 == 99999999	 
	
	gen double ybecagob_ch = r913 if r912 == 1 // Becas educación del gobierno
	gen byte ysolidario_ch = r919/r918 if r918 != .
...
******************
* Line of code with indicator repiteult was deleted*DZ Jul 2017: cambio de nombre de la variable respecto a anio anterior*
...
/* r312 ¿Tiene la vivienda servicio de agua por cañería?
         ...
         5.0 No tiene
         6.0 Tiene pero no le cae (por más de un mes)
...
r313 ¿Forma de abastecimiento de agua de la vivienda
         ...
         5.0 Pozo protegido privado
		 5.1 Pozo protegido público
         6.0 Pozo no protegido privado
		 6.1 Pozo no protegido público
         7.0 Ojo de agua, río o quebrada
         8.0 Manantial protegido
         9.0 Manantial no protegido
        10.0 Colecta agua lluvia
        11.0 Acarreo de cañería del vecino(a)
        12.0 Chorro común
        13.0 Otros medios */

gen byte aguafuente_ch = .
replace aguafuente_ch = 1 if inlist(r312, 1, 2, 3, 4)	// Cañería (piped), red de distribución 
replace aguafuente_ch = 2  if inlist(r313, 2, 12.0)		// Llave pública, pila, standpipe
*replace aguafuente_ch = 3 if....						// Agua embotellada
replace aguafuente_ch = 4  if inlist(r313, 4, 4.1, 5.0, 5.1)	// Pozo protegido
replace aguafuente_ch = 5  if r313 == 10.0				// Agua de lluvia
replace aguafuente_ch = 6  if r313 == 3					// Camión, cisterna, aljibe
replace aguafuente_ch = 7  if r312 == 4.1				// Otra fuente mejorada
replace aguafuente_ch = 7  if inlist(r313, 1, 8.0, 11.0)	// Otra fuente mejorada
replace aguafuente_ch = 8  if r313 == 7.0					// Rio, vertiente, lago
replace aguafuente_ch = 9  if inlist(r313, 6.0, 6.1, 9.0)	// Otra fuente no mejorada  
replace aguafuente_ch = 10 if r313 == 13.0				// Otra fuente sin clasificación
...
replace bano_ch=4 if (r315 ==1 | r315 ==2) & (r319a==2 | r319a==3)
...
***banoex_ch***
***************
generate banoex_ch=1 if r318==2
replace banoex_ch=0 if r318!=2
...
************
	...
	replace sinbano_ch = 1 if r315a==4 & r316==1 | ( r315a==4 & r316==2 & inlist(r319a, 1,2))
	replace sinbano_ch = 2 if r315a==4 & r316==2 & (r319a==3|r319a==4) 
...
****************
gen combust_ch=0
replace combust_ch=1 if  r322==1 | r322==2 | r322==3
...
replace des1_ch=1 if r317>=1 & r317<=4
replace des1_ch=2 if r317>=5 & r317<=10
...
replace des2_ch=1 if r317>=1 & r317<=10
...
**************
gen telef_ch=0
replace telef_ch=1 if r3241a==1
replace telef_ch=. if r3241a==.
...
*DZ Jul 2017: cambio de nombre de la variable respecto a anio anterior*
gen refrig_ch=0
replace refrig_ch=1 if  r32605a==1
replace refrig_ch=. if  r32605a==.
...
*DZ Jul 2017: cambio de nombre de la variable respecto a anio anterior*
gen auto_ch=0
replace auto_ch=1 if r32612a==1
replace auto_ch=. if r32612a==.
...
*DZ Jul 2017: cambio de nombre de la variable respecto a anio anterior*
gen compu_ch=0
replace compu_ch=1 if r32609a==1
replace compu_ch=. if r32609a==.
...
*DZ Jul 2017: cambio de nombre de la variable respecto a anio anterior*
gen internet_ch=0
replace internet_ch=1 if r3243a==1
replace internet_ch=. if r3243a==.
...
*DZ Jul 2017: cambio de nombre de la variable respecto a anio anterior*
gen cel_ch=0
replace cel_ch=1 if r3242a==1 
replace cel_ch=. if r3242a==.
...
replace afiliado_ci = 1 if ((r601 >= 1 & r601 <= 2) & emp_ci==1)
replace afiliado_ci = 0 if (r601 > 2 & inlist(condocup_ci, 1, 2))
...
**pension_ci*
*************
gen byte pension_ci = (r44407a > 0) if !missing(r44407a) /* A todas las personas mayores de cinco años*/
...
gen lp_ci =.
*zona urbana
replace lp_ci= 53.62*2 if  zona_c == 1

*zona rural
replace lp_ci= 33.92*2 if  zona_c == 0
...
gen lpe_ci =.
*zona urbana
replace lpe_ci= 53.62 if zona_c == 1

*zona rural
replace lpe_ci= 33.92 if  zona_c == 0
...
gen tipocobsalud_ci=1 if r601<=5
replace tipocobsalud_ci=2 if r601==6 |  r601==7 |  r601==9
replace tipocobsalud_ci=0 if r601==8
...
gen probsalud_ci=1 if r602!=10
replace probsalud_ci=0 if r602==10
...
*** distancia_ci  ***
*********************
gen distancia_ci=1 if r611==4
recode distancia_ci(.=0) if r611!=.
...
*** costo_ci  ***
*****************
gen costo_ci=1 if r611==3
recode costo_ci (.=0) if r611!=.
...
*** atencion_ci  ***
********************
gen atencion_ci=1 if r611==1 | r611==2 | r611==5 | r611==11
recode atencion_ci (.=0) if r611!=.
