...
Encuesta: EHPM
Round: a
Autores:  Marcela G. Rubio - mrubio@iadb.org 
Fecha última modificación: 8 de diciembre de 2015
...
gen nivel_asiste = r204
gen grado_asiste = r205

replace aedu_ci = 0 if inlist(nivel_asiste, 0, 1) // Educación inicial, parvularia
replace aedu_ci = grado_asiste - 1 if inlist(nivel_asiste, 2, 3) // Básica, Media
replace aedu_ci = grado_asiste + 12 - 1 if inlist(nivel_asiste, 4, 5) // Superior 

* Para los que no asisten 
gen nivel_no_asiste = r217a
gen grado_no_asiste = r217b

replace aedu_ci = 0 if inlist(nivel_no_asiste, 8, 1) // Ninguno, parvularia
replace aedu_ci = grado_no_asiste if inlist(nivel_no_asiste, 2, 3) // Básica, Media
replace aedu_ci = grado_no_asiste + 12 if inlist(nivel_no_asiste, 4, 5) // Superior

* Para los que nunca han asistido
...
drop nivel_asiste grado_asiste 
...
replace aguafuente_ch = 4  if inrange(r313, 4.05, 4.15) | inrange(r313, 5.05, 5.15)	// Pozo protegido
replace aguafuente_ch = 5  if r313 == 10				// Agua de lluvia
replace aguafuente_ch = 6  if r313 == 3					// Camión, cisterna, aljibe
replace aguafuente_ch = 7  if inrange(r312, 4.05, 4.15)			// Otra fuente mejorada
...
replace aguafuente_ch = 9  if inrange(r313, 6.05, 6.15)			// Otra fuente no mejorada  
...
replace afiliado_ci = 1 if ((r108a >= 1 & r108a <= 2) & emp_ci==1)
replace afiliado_ci = 0 if (r108a > 2 & inlist(condocup_ci, 1, 2))
...
replace cotizando_ci = 1 if (r422 == 1 & emp_ci==1)
...
replace cotizando_ci1=1 if (r422==1) & cotizando_ci1==0 /*solo a emplead@s y asalariad@s, difiere con los otros paises*/
...
gen lpe_ci =.
*zona urbana
replace lpe_ci= 175.58 if mes_c==1 & zona_c == 1
replace lpe_ci= 175.47 if mes_c==2 & zona_c == 1
replace lpe_ci= 175.34 if mes_c==3 & zona_c == 1
replace lpe_ci= 176.37 if mes_c==4 & zona_c == 1
replace lpe_ci= 180.58 if mes_c==5 & zona_c == 1
replace lpe_ci= 184.8  if mes_c==6 & zona_c == 1
replace lpe_ci= 189.53 if mes_c==7 & zona_c == 1
replace lpe_ci= 192.58 if mes_c==8 & zona_c == 1
replace lpe_ci= 191.07 if mes_c==9 & zona_c == 1
replace lpe_ci= 193.1  if mes_c==10 & zona_c == 1
replace lpe_ci= 191.83 if mes_c==11 & zona_c == 1
replace lpe_ci= 190.42 if mes_c==12 & zona_c == 1

*zona rural
replace lpe_ci= 122.98 if mes_c==1 & zona_c == 0
replace lpe_ci= 123.26 if mes_c==2 & zona_c == 0
replace lpe_ci= 124.15 if mes_c==3 & zona_c == 0
replace lpe_ci= 125.76 if mes_c==4 & zona_c == 0
replace lpe_ci= 128.81 if mes_c==5 & zona_c == 0
replace lpe_ci= 130.29 if mes_c==6 & zona_c == 0
replace lpe_ci= 133.51 if mes_c==7 & zona_c == 0
replace lpe_ci= 138.45 if mes_c==8 & zona_c == 0
replace lpe_ci= 138.31 if mes_c==9 & zona_c == 0
replace lpe_ci= 139.06 if mes_c==10 & zona_c == 0
replace lpe_ci= 135.98 if mes_c==11 & zona_c == 0
replace lpe_ci= 133.5  if mes_c==12 & zona_c == 0
...
*http://www.mtps.gob.sv/index.php?option=com_content&view=article&catid=25:avisos-ciudadano&id=1699:tabla-salario-minimo-vigente-a-partir-de-enero-2014&Itemid=76 

g salmm_ci=.
replace salmm_ci=120 if rama_ci==1
replace salmm_ci=220 if rama_ci==3
replace salmm_ci=242.40 if rama_ci==6
replace salmm_ci=194 if salmm_ci==. /* promedio de resto de ramas*/
...
******************************
* PTMC: bonos comunidades solidarias rurales/urbanas (r325a3 r325a4) 
* PNC: 	pensión bósica universal r325a5
* Del 2012 al 2015 no se preguntan montos 
