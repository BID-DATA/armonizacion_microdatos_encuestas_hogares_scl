...
Fecha última modificación: 2 de Enero de 2014
...
replace aguadist_ch=3 if r313==2 |  (r313>4.0 &r313<4.2)| (r313>5.0 &r313<5.2)| (r313>6.0 &r313<6.2)| r313==11
...
*zona urbana
replace lpe_ci=176.98	if mes_c==1 & zona_c==1
replace lpe_ci=175.71	if mes_c==2 & zona_c==1
replace lpe_ci=175.47	if mes_c==3 & zona_c==1
replace lpe_ci=175.71	if mes_c==4 & zona_c==1
replace lpe_ci=174.55	if mes_c==5 & zona_c==1
replace lpe_ci=175.47	if mes_c==6 & zona_c==1
replace lpe_ci=174.45	if mes_c==7 & zona_c==1
replace lpe_ci=173.28	if mes_c==8 & zona_c==1
replace lpe_ci=173.42	if mes_c==9 & zona_c==1
replace lpe_ci=173.14	if mes_c==10 & zona_c==1
replace lpe_ci=173.93	if mes_c==11 & zona_c==1
replace lpe_ci=173.04   if mes_c==12 & zona_c==1


*zona rural
replace lpe_ci=138.03	if mes_c==1 & zona_c==0
replace lpe_ci=137.59	if mes_c==2 & zona_c==0
replace lpe_ci=137.01	if mes_c==3 & zona_c==0
replace lpe_ci=137.14	if mes_c==4 & zona_c==0
replace lpe_ci=134.50	if mes_c==5 & zona_c==0
replace lpe_ci=133.20	if mes_c==6 & zona_c==0
replace lpe_ci=131.31	if mes_c==7 & zona_c==0
replace lpe_ci=131.76	if mes_c==8 & zona_c==0
replace lpe_ci=131.59	if mes_c==9 & zona_c==0
replace lpe_ci=130.56	if mes_c==10 & zona_c==0
replace lpe_ci=129.39	if mes_c==11 & zona_c==0
replace lpe_ci=127.37   if mes_c==12 & zona_c==0
...
**salmm_ci***
*************
*gen salmm_ci= 237 /*sector industria - fuente Ministerio de Trabajo y prevision social*/

*2015 MGD: salarios segun actividad
g salmm_ci=.
replace salmm_ci=111 if rama_ci==1
replace salmm_ci=203 if rama_ci==3
replace salmm_ci=224 if rama_ci==6
replace salmm_ci=180 if salmm_ci==.
