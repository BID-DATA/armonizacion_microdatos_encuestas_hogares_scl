...
Fecha última modificación: 10 de Septiembre de 2014
...
*/
*Modificado Mayra Sáenz -  Marzo 2016: El código 4 y 5 corresponden a :
/*
(4) Propietario de la vivienda en terreno público: Es aquel que edifica su vivienda en terrenos del estado o
municipales. Ejemplos. A orilla de la línea férrea, a orilla de las quebradas, debajo de puentes etc.

(5) Propietario de la vivienda en terreno privado: Es aquel que edifica su vivienda en terrenos que son propiedad de
...
*2013
*zona urbana
replace lpe_ci= 173.28 if mes_c==1 & zona_c == 1
replace lpe_ci= 174.17 if mes_c==2 & zona_c == 1
replace lpe_ci= 174.96 if mes_c==3 & zona_c == 1
replace lpe_ci= 174.55 if mes_c==4 & zona_c == 1
replace lpe_ci= 174.69 if mes_c==5 & zona_c == 1
replace lpe_ci= 175.58 if mes_c==6 & zona_c == 1
replace lpe_ci= 175.47 if mes_c==7 & zona_c == 1
replace lpe_ci= 174.55 if mes_c==8 & zona_c == 1
replace lpe_ci= 174.45 if mes_c==9 & zona_c == 1
replace lpe_ci= 173.55 if mes_c==10 & zona_c == 1
replace lpe_ci= 173.93 if mes_c==11 & zona_c == 1
replace lpe_ci= 174.17 if mes_c==12 & zona_c == 1

*zona rural
replace lpe_ci= 126.93 if mes_c==1 & zona_c == 0
replace lpe_ci= 126.34 if mes_c==2 & zona_c == 0
replace lpe_ci= 126.62 if mes_c==3 & zona_c == 0
replace lpe_ci= 125.45 if mes_c==4 & zona_c == 0
replace lpe_ci= 125.62 if mes_c==5 & zona_c == 0
replace lpe_ci= 125.76 if mes_c==6 & zona_c == 0
replace lpe_ci= 124.73 if mes_c==7 & zona_c == 0
replace lpe_ci= 124.59 if mes_c==8 & zona_c == 0
replace lpe_ci= 124.29 if mes_c==9 & zona_c == 0
replace lpe_ci= 123.7 if mes_c==10 & zona_c == 0
replace lpe_ci= 123.43 if mes_c==11 & zona_c == 0
replace lpe_ci= 123.43 if mes_c==12 & zona_c == 0
...
**salmm_ci***
*************
/* Salario mínimo de la industria
gen salmm_ci= 228 /*http://www.mtps.gob.sv/index.php?option=com_content&view=article&catid=25:avisos-ciudadano&id=1570:tarifas-de-salarios-minimos-vigentes-1-de-julio-a-31-de-diciembre-de-20&Itemid=76*/
label var salmm_ci "Salario minimo legal"*/

*2015 MGD: salarios segun actividad
g salmm_ci=.
replace salmm_ci=113 if rama_ci==1
replace salmm_ci=207 if rama_ci==3
replace salmm_ci=229 if rama_ci==6
replace salmm_ci=183 if salmm_ci==. /* promedio de resto de ramas*/
