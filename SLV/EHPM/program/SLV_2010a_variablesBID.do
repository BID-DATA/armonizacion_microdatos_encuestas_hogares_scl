...
Encuesta: EHPM
Round: a
Autores: Enero 2012 - Yanira Oviedo
...
************
gen emp_ci=0
replace emp_ci=1 if r414!=.
label var emp_ci "Ocupado"

****************
***desemp1_ci***
****************
gen desemp1_ci=(emp_ci==0 &  r407==1)
...
***desemp2_ci*** 
****************
gen desemp2_ci=(desemp1_ci==1 | (emp_ci==0 & r409==18))
...
replace firmapeq_ci=0 if r421>5 & r421!=. & r421!=998
...
replace aguafuente_ch = 7  if inlist(r313, 1, 13) /* Cañería vecino/acarreo vecino - otra mejorada */
...
replace aguafuente_ch = 10 if r313 == 14   /* Otros medios                    - sin info */

*************
*aguadist_ch*
*************
gen aguadist_ch=0
replace aguadist_ch= 1 if  (r312==1 |r312==2| r313==1 | r313==3 )
replace aguadist_ch= 2 if  r312==3|r312==4
replace aguadist_ch=3 if r313==2 | r313==12 | r313==13
...
replace techo_ch=0 	if r302>=5 & r302<=8
...
gen lpe_ci =.
*zona urbana
replace lpe_ci=160.87	if mes_c==1 & zona_c==1
replace lpe_ci=162.79	if mes_c==2 & zona_c==1
replace lpe_ci=165.36	if mes_c==3 & zona_c==1
replace lpe_ci=162.93	if mes_c==4 & zona_c==1
replace lpe_ci=160.63	if mes_c==5 & zona_c==1
replace lpe_ci=168.17	if mes_c==6 & zona_c==1
replace lpe_ci=167.79	if mes_c==7 & zona_c==1
replace lpe_ci=165.87	if mes_c==8 & zona_c==1
replace lpe_ci=169.82	if mes_c==9 & zona_c==1
replace lpe_ci=176.09	if mes_c==10 & zona_c==1
replace lpe_ci=179.93	if mes_c==11 & zona_c==1
replace lpe_ci=179.18   if mes_c==12 & zona_c==1

*zona rural
replace lpe_ci=112.18	if mes_c==1 & zona_c==0
replace lpe_ci=115.54	if mes_c==2 & zona_c==0
replace lpe_ci=118.46	if mes_c==3 & zona_c==0
replace lpe_ci=112.90	if mes_c==4 & zona_c==0
replace lpe_ci=110.85	if mes_c==5 & zona_c==0
replace lpe_ci=116.54	if mes_c==6 & zona_c==0
replace lpe_ci=115.95	if mes_c==7 & zona_c==0
replace lpe_ci=117.29	if mes_c==8 & zona_c==0
replace lpe_ci=121.68	if mes_c==9 & zona_c==0
replace lpe_ci=123.70	if mes_c==10 & zona_c==0
replace lpe_ci=127.37	if mes_c==11 & zona_c==0
