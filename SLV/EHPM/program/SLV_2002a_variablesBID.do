...
***************
gen ine01= depto
label define depto 1 "Ahuachapán" ///
2 "Santa Ana" ///
3 "Sonsonate" ///
4 "Chalatenango" ///
5 "La Libertad" ///
...
replace desalent_ci = 1 if ((busco == 2 & (pqnobus== 2 | pqnobus == 3) & condocup_ci == 3))
...
replace viviprop_ch=1 if tenencia==3
replace viviprop_ch=2 if tenencia==2
replace viviprop_ch=3 if tenencia>=4 & tenencia<=7


gen viviitit_ch=.
/*NA*/

gen vivialq_ch=r308c if tenencia==1
...
replace cotizando_ci = 1 if (segsoc == 1 & emp_ci==1)
...
replace lpe_ci= 123.90	if mes_c==2 & zona_c==1
replace lpe_ci= 123.90	if mes_c==3 & zona_c==1
replace lpe_ci= 127.50	if mes_c==4 & zona_c==1
...
replace lpe_ci= 128.70	if mes_c==8 & zona_c==1
replace lpe_ci= 126.30	if mes_c==9 & zona_c==1
replace lpe_ci= 127.50	if mes_c==10 & zona_c==1
replace lpe_ci= 127.50	if mes_c==11 & zona_c==1
replace lpe_ci= 127.50  if mes_c==12 & zona_c==1

*zona rural
replace lpe_ci=96.60	if mes_c==1 & zona_c==0
replace lpe_ci=93.60    if mes_c==2 & zona_c==0
replace lpe_ci=93.60	if mes_c==3 & zona_c==0
replace lpe_ci=93.60	if mes_c==4 & zona_c==0
replace lpe_ci=93.60	if mes_c==5 & zona_c==0
replace lpe_ci=92.10	if mes_c==6 & zona_c==0
replace lpe_ci=92.10	if mes_c==7 & zona_c==0
replace lpe_ci=92.10	if mes_c==8 & zona_c==0
...
replace lpe_ci=90.90    if mes_c==12 & zona_c==0
...
***tecnica_ci**
*************
gen tecnica_ci=(nivelcursa==5 | nivaprob==5)
