...
Detalle de procesamientos o modificaciones anteriores:
 opened on:  16 Aug 2006, 12:38:04
*******************************************************************************************************
*****                                     HONDURAS 1986                                               *
*****                    ECFT 1986 (Encuesta Continua de Fuerza de Trabajo)                           *
*****                                      39.596 personas                                            *
*****                                       7.550 hogares                                             *
*******************************************************************************************************
****************************************************************************/
...
****idh_ch*****
*************** 
egen idh_ch=group(region depto estrato colonia sector vivienda hogar)
...
***horastot_ci***
*****************
egen horastot_ci=rsum(horaspri horassec), missing
...
***tamfirma_ci***
*****************
*La pregunta es la p39 pero no esta en la base.
gen tamfirma_ci=.
...
replace spublico_ci= 0 if categ > 1 & categ <= 8
...
*************
gen ocupa_ci=.
replace ocupa_ci=1 if (ocup >=0 & ocup <=9) & condocup_ci==1
replace ocupa_ci=2 if ocup >=10 & ocup <=12
replace ocupa_ci=3 if ocup >=20 & ocup <=27
replace ocupa_ci=4 if ocup >=30 & ocup <=33
replace ocupa_ci=5 if ocup >=90 & ocup <=95 
replace ocupa_ci=6 if ocup >=40 & ocup <=45
replace ocupa_ci=7 if (ocup >=50 & ocup <=81) 
*replace ocupa_ci=8
replace ocupa_ci=9 if ocup ==98
...
*tamemp_ci
*************
*perstrab  1 "De 1 a 2 personas" 2 "De 3 a 4 personas" 3 "De 5 a 9 personas" 4 "De 10 personas y mas"

gen tamemp_ci =.
replace tamemp_ci=1 if (perstrab==1 | perstrab==2)
replace tamemp_ci=2 if (perstrab==3)
replace tamemp_ci=3 if (perstrab==4)
...
replace ylmpri_ci=yocupp if yocupp<9999 & edad>9 & yocupp>0

replace ylmpri_ci=ganocupp if ganocupp<9999 & edad>9 & ganocupp>0

replace ylmpri_ci=0 if yocupp==0 & ganocupp==0 & edad>9 & trabajo==1
...
***************
gen ylmsec_ci=.
replace ylmsec_ci=yocups if yocups<9999 & edad>9 & yocups>0
replace ylmsec_ci=ganocups if ganocups<9999 & edad>9 & ganocups>0
...
* Modificaciones Marcela Rubio Septiembre 2014: variable habia sido generada con error 
...
gen asiste_ci=.
replace asiste_ci=1 if asiste==1
replace asiste_ci=0 if asiste==2
label var asiste_ci "Personas que actualmente asisten a centros de enseñanza"
...
replace aguadist_ch=3 if agua==3
...
replace bano_ch=1 if servsan==1 | servsan==2

replace bano_ch=0 if servsan==3

gen banoex_ch=.

replace banoex=1 if usoserv==1

replace banoex=0 if usoserv==2
...
/* We think that there was a change in the sampling frame for rural areas, but we have no further evide
> nce to prove it. 
> Thus, we'll leave this variable missing for this year.
...
replace dorm_ch=nrodorm if nrodorm>=0

gen cuartos_ch=.

replace cuartos_ch=nrohab if nrohab>=0
...
replace vivialq_ch=pagomens if pagomens>0 & viviprop_ch==0
