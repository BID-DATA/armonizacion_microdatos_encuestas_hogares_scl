...
Última versión: David Cornejo - Email: dcor@iadb.org, davidcornejoarias@gmail.com
Fecha última modificación: Junio de 2024
...
************************************
	...
	*Para quienes no asisten:
	...
	replace aedu_ci=9+2+ed08 if (ed05==9) & ed08<99 //11 (basica y media) 
	replace aedu_ci=9+2+4+ed08 if (ed05==10) & ed08<99 //11 (basica y media) + 4 (grado)

	*Para quienes asisten actualmente:
	replace aedu_ci=0 if (ed10>=1 & ed10<=3)
	replace aedu_ci=ed13 - 1 if ed10==4 & ed13<99
	replace aedu_ci=9+ed13 - 1 if ed10==5 //9 años de basica
	replace aedu_ci=9+2+ed13 - 1 if (ed10==6 | ed10==7 | ed10==8) //9(basica) + 2(media)
	replace aedu_ci=9+2+ed13 - 1 if (ed10==9) //11 (basica y media) + 4 (grado)
	replace aedu_ci=9+2+4+ed13 - 1 if (ed10==10) & ed13<99 //11 (basica y media) + 4 (grado)
	
	*Para los que no fueron en el 2022, pero si en el 2021
	...
	replace aedu_ci=9+2+ed17 if (ed16==9 ) & aedu_ci==. //11 (basica y media) + 4 (grado)
	replace aedu_ci=9+2+4+ed17 if (ed16==10) & aedu_ci==. //11 (basica y media) + 4 (grado)
...
order region_BID_c pais_c anio_c mes_c zona_c factor_ch	idh_ch	idp_ci	factor_ci upm_ci estrato_ci sexo_ci edad_ci ///
