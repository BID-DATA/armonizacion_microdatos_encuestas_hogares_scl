...
gen horastot_ci=hrsocupt  if hrsocupt~=99 &  hrsocupt>0
replace  horastot=. if hrsocupt==. 
...
*************
gen ocupa_ci=.
replace ocupa_ci=1 if (ocupac >=25 & ocupac <=1955) & emp_ci==1
...
*replace ocupa_ci=8
*replace ocupa_ci=9 if (ocupac >=9913 & ocupac <=9969) 
...
generat lp_ci= 201.96 if zona_c==1 
replace lp_ci= 112.34 if zona_c==0
...
***************
	...
	g asispre_ci=.
	replace asispre_ci=1 if asiste==1 & nivel==2 & edad_ci<8
	recode asispre_ci (.=0)
...
replace aguared_ch = . if aguaa==.
*/

gen aguared_ch = (aguaa ==1 | aguaa==2)
replace aguared_ch = . if aguaa==0
...
rename ocupac codocupa
rename ramact codindustria
