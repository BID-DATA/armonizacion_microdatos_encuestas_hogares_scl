...
*******************************************************************************************************
*****                                      HONDURAS 1994                                               
*****                EPHPM 1994 (Encuesta Permanente de Hogares de Propositos Multiples)               
*****                                     27.891 personas                                              
*****                                       5.320 hogares                                              
*******************************************************************************************************
...
***factor_ch***
***************
egen idh_ch=group(region depto municip muestra vivienda hogar)
...
***antiguedad_ci***
*******************
* Nota MGD 9/25/2014: se prefier dejar como missing ya que no coincide con la serie a pesar de estar bien generada la variable.
* Se utiliza una variable creada de años.
gen antiguedad_ci=.
/*replace antiguedad=aniost if emp==1 & aniost<99 & aniost>0
replace antiguedad=0.5 if tiempotr==1 & antiguedad_ci==. & emp==1 
...
replace  nempleos_ci=2 if emp_ci==1 & pluriemp==1
...
generat lp_ci=460.08 if zona_c==1
replace lp_ci=255.92 if zona_c==0
...
califact:
          -1 <5 anos
           0 no aplicable
          11 ocupados
          12 ocupados que no trabajan
          21 cesante
          22 buscan trabajo 1era vez
          33 jubilados/pensionados
          34 rentistas
          35 estudiantes
          36 labores del hogar
          37 incapacitados
          38 otros inactivos
*/
gen categoinac_ci =1 if (califact==33 & condocup_ci==3)
replace categoinac_ci = 2 if  (califact==35 & condocup_ci==3)
replace categoinac_ci = 3 if  (califact==36 & condocup_ci==3)
...
replace  ylmpri_ci=0 if yocupp==0 & ganocupp==0 & edad>9 & (hrsocupp>0 & hrsocupp<99)

replace  ylmpri_ci=0 if categ==8 & edad>9 & (hrsocupp>0 & hrsocupp<99)
...
replace  ylmsec_ci=0 if yocups==0 & ganocups==0 & edad>9 & (hrsocups>0 & hrsocups<99)
...
replace  grado=. if grado==9 | grado==-9999999

replace  nivel=. if nivel==-9999999
...
trapri_ci trapri_ch progpub_ci progpub_ch trapub_ci  trapub_ch capital_ci capital_ch otros_ci otros_ch ypen_ch ytotal_ci  ytotal_ch ytotalpc_ch quintil_ci ///N
