...
replace cotizando_ci = 1 if ((aux_a>=1 & aux_a<=6) |  (aux_b>=1 & aux_b<=6)| (aux_c>=1 & aux_c<=6)| (aux_d>=1 & aux_d<=6) | (aux_e>=1 & aux_e<=6) | (aux_f>=1 & aux_f<=6) | (aux_g>=1 & aux_g<=6) & emp_ci==1)
...
/* La respuesta es a nivel de persona, por lo que un mismo hogar puede tener diferentes respuestas. Por lo que se utiliza la jefatura de hogar.
    ...
    at06_1. En su casa
           1 Si
           2 No */
gen internet_jh = ((at06==1 | inrange(at06, 11, 16)) & at04==1) & relacion_ci==1						  // Posee conexión a internet
replace internet_jh = . if  at06==. & (at04==. | at04==9) & relacion_ci==1	  // No tiene
...
clonevar  codocupa=ce25
clonevar  codindustria=ce28cod
