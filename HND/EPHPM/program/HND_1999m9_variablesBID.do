...
replace  horastot_ci=. if hrstrab==. 
...
replace durades_ci=0.5 if ctpobusc==1 & (tpobusc==0  | tpobusc==.)
...
*************
gen ocupa_ci =.
replace ocupa_ci=1 if ocup >=0 & ocup <=1897
replace ocupa_ci=2 if ocup >=1993 & ocup <=2545
replace ocupa_ci=3 if ocup >=2570 & ocup <=3484
replace ocupa_ci=4 if ocup >=3505 & ocup <=3940
replace ocupa_ci=5 if ocup >=9122 & ocup <=9988 
replace ocupa_ci=6 if ocup >=3963 & ocup <=4541
replace ocupa_ci=7 if (ocup >=4561 & ocup <=9099) 
*replace ocupa_ci=8
*replace ocupa_ci=9 
...
*************
gen rama_ci=.
replace  rama_ci=1 if (rama>=64 & rama<=448) & emp_ci==1
replace  rama_ci=2 if (rama>=505 & rama<=824) & emp_ci==1
replace  rama_ci=3 if (rama>=883 & rama<=6015) & emp_ci==1
replace  rama_ci=4 if (rama>=6049 & rama<=6178) & emp_ci==1
replace  rama_ci=5 if (rama>=6238 & rama<=6288) & emp_ci==1
replace  rama_ci=6 if (rama>=6301 & rama<=6493) & emp_ci==1
replace  rama_ci=7 if (rama>=6553 & rama<=7315) & emp_ci==1
replace  rama_ci=8 if (rama>=7372 & rama<=8067) & emp_ci==1
replace  rama_ci=9 if (rama>=8128 & rama<=9833) & emp_ci==1
...
**ypen_ci*
*************
gen ypen_ci=ypenju if ypenju>=0 & ypenju<99999
...
generat lp_ci=984.60 if zona_c==1
replace lp_ci=547.68 if zona_c==0
...
gen yprodu2= yprodu if yprodu<99999 & yprodu>=0 & yprodu!=9999
egen ylnmpri_ci=rsum(yalim2 yropa2 yhabita2 ytrans2 yotro2 yprodu2), missing
replace ylnmpri_ci=. if yalim2==. & yropa2==. & yhabita2==. & ytrans2==. & yotro2==. & yprodu2==.
replace ylnmpri_ci=0 if (ralim==6 & rropa==6 & rhabita==6 & rtrans==6 & rotro==6) | retprodu==6
...
***ynlm_ci***
*************
*Daniela Zuluaga-Noviembre 2017: Se modifica sintaxis dado que las variables ya se encuentran mensualizadas.
...
egen ynlm_ci=rsum(ypenju2 ysubsi2 yalquile2 ybonos2 yremesa2 yayuda2 yayupar2 yotros2), missing
...
*	autocons_ci 
******************************
gen autocons_ci =yprodu if yprodu<99999 & yprodu>=0 
replace autocons_ci =0 if yprodu==0 & edad>9 & (categopri==1 | categopri==2)
...
***capital_ci***
***************
egen capital_ci= rowtotal(yalquile2), missing
...
replace banoex_ch=1 if usoserv==7

replace banoex_ch=0 if usoserv==8
