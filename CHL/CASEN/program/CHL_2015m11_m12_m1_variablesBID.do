...
gen byte afro_ci=. 
	...
	***************
gen     ind_ci=. 
replace ind_ci=1 if (r3 >=1 & r3 <=9 ) 
replace ind_ci=0 if r3==10	
...
*Feb 2021
	...
	***afroind_ci***
	***************
*Pgta: Pueblos indígenas, pertenece usted o es descendiente de alguno de ellos? 
*(r3) (Aimara 1; Rapa-Nui o Pascuenses 2; Quechua 3; Mapuche 4; Atacame�o (Likan-Antai) 
*5; Collas 6; Kawashkar o Alacalufes 7; Y�mana o Yag�n 8; Diaguita 9; 
*No pertenece a ning�n pueblo ind�gena 10; No sabe/no responde 99)
gen     afroind_ci=. 
...
gen dis_ch=. 
	...
	*ISOalpha3_dis_ci*
	******************
/*s31c1=> s31. ¿Tiene Ud. alguna de las siguientes condiciones permanentes
...
5. Sordera o dificultad para oír aún usando audífonos
6. Ceguera o dificultad para ver aún usando lentes
7. No ti ene ninguna condición de larga duración	
9. Sin dato */
gen byte CHL_dis_ci =.
replace  CHL_dis_ci =0 if s31c1==7
replace  CHL_dis_ci =. if s31c1==9
replace  CHL_dis_ci =1 if s31c1>=1 & s31c1<=6
...
* desalent_ci  * 
**************** 
*AJUSTE SEGUN METODOLOGIA. JANET PORRAS. 26/11/25
/* o7. ¿Cuál es la razón o razones por la(s) que no buscó trabajo o
realizó alguna gesti ón para iniciar una acti vidad por cuenta propia en
las últi mas cuatro semanas?
...
16. No ti ene interés en trabajar */
gen       desalent_ci=. 
...
label val rama_ci rama_ci
		...
		**************
/* Nota:
yoprcor = ingreso ocupacion principal
...
ytrabajocor = ingreso laboral (Corresponden a los ingresos que obtienen las 
...
* ylnmpri_ci   * 
**************** 
*https://observatorio.ministeriodesarrollosocial.gob.cl/storage/docs/casen/2015/Libro_de_Codigos_Casen_2015.pdf
*https://observatorio.ministeriodesarrollosocial.gob.cl/storage/docs/casen/2015/Manual_Casen_2015.pdf
*JANET PORRAS. 26/11/25
...
* ynlm_ci      * 
**************** 
*yautcor:     ingreso autonomo corregido del hogar
...
* auto_ch     * 
***************
bysort idh_ch: egen auto_ch=sum(r12==1 & r13b>=1 & r13b<=9)
...
replace   afiliado_ci=0 if o28==2 | o28==9
...
/* VERSION ANTERIOR. y26.2 ¿Me podría indicar el monto de 
y26_2bm1 jubilación o pensión de vejez (Monto 1 ($) 
y26_2bm2 con Aporte Previsional Solidario (Monto 2 ($)
y26_2c   jubilación o pensión de vejez (Monto ($))
y26_2em1 jubilación o pensión de invalidez (Monto 1 ($)
y26_2em2 con Aporte Previsional Solidario (Monto 2 ($)
y26_2f   jubilación o pensión de invalidez
y26_2g   montepío o pensión de viudez (Monto ($)
y26_2h   pensión de orfandad (Monto ($)
y26_2i   pensión por leyes especiales (Monto ($)
y26_2j   otra pensión o jubilación (Monto ($)
...
replace tipopen_ci = 5 if (otros> 0 & otros != .)
...
label define t 12   "Jub_invali" 13 "Jub_viud"  14 "Jub_orfan" 15"Jub_otro", add
label define t 23   "Inv_viudez" 24 "Inv_orfan" 25 "Inv_otro", add 
label define t 34   "Viud_orfan" 35 "Viud_otro" 45 "Orfan_otro", add 
...
*MGD-oct-2016: solamente isntitucion de pensión d einvalidez, pero hay para cada tipo
...
*o16.En su trabajo principal, ¿qué tipo de contrato o acuerdo de trabajo ti ene…?
...
gen    pension_ci=1 if  y26_3c_in<9 & y26_3c_in !=.
recode pension_ci .=0 
...
*y26_1am:  Monto pension basica solidaria de vejez
*y26_1dm:  Monto pension basica solidaria de invalidez
*y26_2bm1: Monto de la Jubilación o Pensión de Vejez Monto1($)
*y26_2bm2: con Aporte Previsional Solidario Monto2($)
*y26_2em1: Monto de la Jubilación o Pensión de Invalidez Monto1 ($)
*y26_2em2: con Aporte Previsional Solidario Monto2 ($)
*SE AGREGO LOS APORTES PREVISIONALES POR VEJEZ E INVALIDEZ. JANET PORRAS. 26/11/12 
egen auxpens=rsum(y26_1am  y26_1dm y26_2bm1 y26_2bm2 y26_2em1 y26_2em2), missing
...
*Si no se especifica el pais de la madre se considera missing. JANET PORRAS. 26/11/25
	...
	replace   migrante_ci=. if (r1b==3 & r1bespp_cod=="") | (r1b==3 & r1bespp_cod==999)
   ...
   *tipo_bienestar*
   ****************	
*https://observatorio.ministeriodesarrollosocial.gob.cl/storage/docs/casen/2015/CASEN_2015_Situacion_Pobreza.pdf   
*JANET PORRAS. 26/11/25	
...
/* Calculo POBREZA 
	...
	gen pobreza1 = 3
	replace pobreza1 = 2 if yae < lp
	replace pobreza1 = 1 if yae < li
*/
