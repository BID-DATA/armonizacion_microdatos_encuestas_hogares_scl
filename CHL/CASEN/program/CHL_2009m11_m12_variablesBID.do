...
****************************
	...
	***************	
	...
	gen  byte horaspri_ci =o16 if o16!=999
	
	***************
	***horastot_ci ***
	***************	
	gen  byte horastot_ci  =o16 if o16!=999
	...
	***************	
	...
	replace cotizando_ci  = 0 if o32==7
	...
	***************	
	...
	replace afiliado_ci  = 0 if o31==2
	...
	***************	
	gen  byte afiliado_ci = .
	replace afiliado_ci  =o32 if o32<=5
	...
	**************
	gen byte pension_ci=. 
	replace pension_ci=1 if yjubaj>0 | yinvaj>0 | ymonaj>0 | yorfaj>0 | yvitaj>0 
	recode pension_ci .=0 if yjubaj!=0 & yinvaj!=0 & ymonaj!=0 &  yorfaj!=0 & yvitaj!=0  
	
	***************
	**pensionsub_ci**
	***************
	gen byte pensionsub_ci = . 
	replace pensionsub_ci = 1 if ypensaj>0 | ypresaj>0 
	recode pensionsub_ci .=0 if ypensaj!=0 & ypresaj!=0 
...
****************
generate aguared_ch =.
replace aguared_ch = 1 if v8<=3 
replace aguared_ch = 0 if v8>3
...
*****************
gen aguafuente_ch=.
replace aguafuente_ch = 1 if v8<=3 & v9<=2
...
*************
gen aguadist_ch=0
replace aguadist_ch=1 if v9==1
replace aguadist_ch=2 if v9==2
replace aguadist_ch=3 if v9==3
...
***aguamide_ch***
*****************
gen aguamide_ch = 1 if v8<=2
replace aguamide_ch  = 0 if v8>2
...
egen ypen_ci=rsum(yjubaj yvitaj yinvaj ymonaj yorfaj), missing
...
egen  ypensub_ci=rsum(ypensaj ypresaj), missing
...
gen categoinac_ci=.
replace categoinac_ci=1 if o6==18 & condocup_ci == 3
replace categoinac_ci=2 if o6==17 & condocup_ci == 3
replace categoinac_ci=3 if o6==7 & condocup_ci == 3
