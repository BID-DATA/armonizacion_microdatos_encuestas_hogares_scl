...
Encuesta: ENFT
Round: m4
Autores: Marcela G. Rubio (mrubio@iadb.org),13 de Abril de 2015
...
/* Modificacion 2015, 04 LC
eft_periodo	eft_moneda	eft_tasa_a_dolar
2/2014	1	43.43 (Peso)
2/2014	2	1 (Dolares)
2/2014	5	1.08 (Dolar Canadiense)
2/2014	13	0.89 (Franco)
2/2014	14	.74 (Euro)
*/	
...
*Si la información está en dólares se multiplica por 43.43


*Modulo Ingresos del Exterior
gen pension_int=monto_ing_pension_mes	 		    if  moneda_ing_pension_mes==1
replace pension_int=monto_ing_pension_mes*43.43   	if  moneda_ing_pension_mes==2
replace pension_int=(monto_ing_pension_mes*1.08)*43.43     if  moneda_ing_pension_mes==5
replace pension_int=(monto_ing_pension_mes*0.74)*43.43     if  moneda_ing_pension_mes==14
replace pension_int=. if recibio_ing_pension_mes==2


gen interes_int=monto_ing_interes_mes	 		    if  moneda_ing_interes_mes==1
replace interes_int=monto_ing_interes_mes*43.43	    if  moneda_ing_interes_mes==2
replace interes_int=(monto_ing_interes_mes*1.08)*43.43 	if  moneda_ing_interes_mes==5
replace interes_int=(monto_ing_interes_mes*0.74)*43.43  if  moneda_ing_interes_mes==14
...
replace r_jul=monto_jul*43.43 	if moneda_jul==2
replace r_jul=(monto_jul*1.08)*43.43 	if moneda_jul==5
replace r_jul=(monto_jul*0.74)*43.43 	if moneda_jul==14

gen r_ago=monto_ago		    	if moneda_ago==0
replace r_ago=monto_ago		    if moneda_ago==1
replace r_ago=monto_ago*43.43 	if moneda_ago==2
replace r_ago=(monto_ago*1.08)*43.43 	if moneda_ago==5
replace r_ago=(monto_ago*0.74)*43.43 	if moneda_ago==14

gen r_sep=monto_sep 	    	if moneda_sep==0
replace r_sep=monto_sep 		if moneda_sep==1
replace r_sep=monto_sep*43.43	if moneda_sep==2
replace r_sep=(monto_sep*1.08)*43.43 	if moneda_sep==5
replace r_sep=(monto_sep*0.74)*43.43 	if moneda_sep==14
...
* 2015, 05 modif LC se incorporó en valor cero opciones 4,5 y 6 que se refieren a gas.
replace luz_ch=0 if tipo_alumbrado_viv==99 | tipo_alumbrado_viv==4 | tipo_alumbrado_viv==5 | tipo_alumbrado_viv==6
...
replace combust_ch=0 if combustible_cocina==99 | combustible_cocina==4 | combustible_cocina==5
...
gen piso_ch=1
replace piso_ch=0 if piso==3 | piso==9 
...
gen lp_ci =.
replace lp_ci =  4664 if zona_c==1
replace lp_ci =  4153 if zona_c==0
...
*********
gen lpe_ci =.
replace lpe_ci =  2100 if zona_c==1
replace lpe_ci =  2012 if zona_c==0
...
* DOM 2014
* Salario minimo se renueva cada dos años, el último incremento fué en el 2013: http://www.omlad.gob.do/ResolucionesSalariales.aspx
*para los trabajadores de empresas pequeñas, que tengan en capital de más de un millón de pesos 
gen salmm_ci=6880
