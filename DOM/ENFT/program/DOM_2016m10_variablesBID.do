...
Encuesta: ENFT
Round: m4
Autores: Mayra Saenz mayras@iadb.org/ saenzmayra.a@gmail.com
Daniela Zuluaga - Octubre de 2017
...
*Si la información está en dólares se multiplica por 45.9


*Modulo Ingresos del Exterior
gen pension_int=monto_ing_pension_mes	 		    if  moneda_ing_pension_mes==1
replace pension_int=monto_ing_pension_mes*45.9   	if  moneda_ing_pension_mes==2
replace pension_int=(monto_ing_pension_mes*1.3)*45.9     if  moneda_ing_pension_mes==5
replace pension_int=(monto_ing_pension_mes*0.89)*45.9     if  moneda_ing_pension_mes==14
replace pension_int=. if recibio_ing_pension_mes==2


gen interes_int=monto_ing_interes_mes	 		    if  moneda_ing_interes_mes==1
replace interes_int=monto_ing_interes_mes*45.9	    if  moneda_ing_interes_mes==2
replace interes_int=(monto_ing_interes_mes*1.3)*45.9 	if  moneda_ing_interes_mes==5
replace interes_int=(monto_ing_interes_mes*0.89)*45.9  if  moneda_ing_interes_mes==14
...
replace r_jul=monto_jul*45.9 	if moneda_jul==2
replace r_jul=(monto_jul*1.3)*45.9 	if moneda_jul==5
replace r_jul=(monto_jul*0.89)*45.9 	if moneda_jul==14

gen r_ago=monto_ago		    	if moneda_ago==0
replace r_ago=monto_ago		    if moneda_ago==1
replace r_ago=monto_ago*45.9 	if moneda_ago==2
replace r_ago=(monto_ago*1.3)*45.9 	if moneda_ago==5
replace r_ago=(monto_ago*0.89)*45.9 	if moneda_ago==14

gen r_sep=monto_sep 	    	if moneda_sep==0
replace r_sep=monto_sep 		if moneda_sep==1
replace r_sep=monto_sep*45.9	if moneda_sep==2
replace r_sep=(monto_sep*1.3)*45.9 	if moneda_sep==5
replace r_sep=(monto_sep*0.89)*45.9 	if moneda_sep==14
...
label def nivel 1 "Pre-escolar" 2 "Primaria" 3 "Secundario" 4 "Técnico-Vocacional" 5 "Universitario" 6 "Post-Universitario" 7 "Ninguno" 8 "Quisqueya Aprende"
lab val ult_nivel_alcanzado nivel 
lab val se_matriculo nivel

gen aedu_ci = . 
replace aedu_ci  = 0 if inlist(ult_nivel_alcanzado, 1, 7, 8) // prescolar, ninguno, quisqueya
...
replace eduui_ci = 0 if se_matriculo == 6
