...
gen tiempoparc_ci=(tothoras>=1 & horastot_ci<=30 & desea_trab_mas_horas==2)
...
gen gobierno=monto_gobierno_ing_nac 	
...
*Información cambiaria que viene en la base
...
*Si la información está en dólares se multiplica por 33.11
*Si la información está en euros se multiplica por 44.14

gen pension_int=monto_ing_pension_mes	 		if  moneda_ing_pension_mes==1
replace pension_int=monto_ing_pension_mes*33.11	if  moneda_ing_pension_mes==2
replace pension_int=(monto_ing_pension_mes*0.75)*33.11	if  moneda_ing_pension_mes==14
replace pension_int=. if recibio_ing_pension_mes==2

gen interes_int=monto_ing_interes_mes	 		if  moneda_ing_interes_mes==1
replace interes_int=monto_ing_interes_mes*33.11	if  moneda_ing_interes_mes==2
replace interes_int=(monto_ing_interes_mes*0.75)*33.11	if  moneda_ing_interes_mes==14
...
replace otros_int=(monto_otros_ing_sem*33.11)/6	if moneda_otros_ing_sem==2
replace otros_int=((monto_otros_ing_sem*0.75)*33.11)/6	if moneda_otros_ing_sem==14
...
replace r_jul=monto_jul*33.11 if moneda_jul==2
replace r_jul=(monto_jul*0.75)*33.11	if moneda_jul==14

gen r_ago=monto_ago			if moneda_ago==0
replace r_ago=monto_ago		if moneda_ago==1
replace r_ago=monto_ago*33.11 if moneda_ago==2
replace r_ago=(monto_ago*0.75)*33.11 if moneda_ago==14

gen r_sep=monto_sep 		if moneda_sep==0
replace r_sep=monto_sep 		if moneda_sep==1
replace r_sep=monto_sep*33.11 if moneda_sep==2
replace r_sep=(monto_sep*0.75)*33.11	if moneda_sep==14
...
*el 9.1% de los individuos que afirmaron haber recibido remesas.
...
**************
/*
EFT_COMO_ELIMINA_BASURA	1	LA RECOGE EL AYUNTAMIENTO
EFT_COMO_ELIMINA_BASURA	2	LA RECOGE UNA EMPRESA PRIVADA
EFT_COMO_ELIMINA_BASURA	3	LA RECOGE EL TRICICLO O CARRETILLERO
EFT_COMO_ELIMINA_BASURA	4	LA QUEMAN
EFT_COMO_ELIMINA_BASURA	5	LA TIRAN AL PATIO O SOLAR
EFT_COMO_ELIMINA_BASURA	6	LA TIRAN A UN VERTEDERO
EFT_COMO_ELIMINA_BASURA	7	LA TIRAN A UNA CAÑADA
EFT_COMO_ELIMINA_BASURA	99	OTRO
*/
