...
gen contrato_ci=.
replace contrato_ci = 1 if firmo_contrato== 1
replace contrato_ci = 0 if firmo_contrato== 2 | firmo_contrato==3
...
gen segsoc_ci=.
replace segsoc_ci = 1 if afiliado_afp==1
replace segsoc_ci = 0 if afiliado_afp==2
label var segsoc_ci "Personas que tienen seguridad social AFP por su trabajo"
*/
...
*Si la información está en dólares se multiplica por 32.32
*Si la información esta en dolar canadiense se multiplica por 27.39
*Si la información está en euros se multiplica por 38.94

*Modulo Ingresos del Exterior
gen pension_int=monto_ing_pension_mes	 		    if  moneda_ing_pension_mes==1
replace pension_int=monto_ing_pension_mes*32.32   	if  moneda_ing_pension_mes==2
replace pension_int=(monto_ing_pension_mes*1.18)*32.32  	if  moneda_ing_pension_mes==5
replace pension_int=(monto_ing_pension_mes*0.83)*32.32     if  moneda_ing_pension_mes==14
replace pension_int=. if recibio_ing_pension_mes==2

gen interes_int=monto_ing_interes_mes	 		    if  moneda_ing_interes_mes==1
replace interes_int=monto_ing_interes_mes*32.32	    if  moneda_ing_interes_mes==2
replace interes_int=(monto_ing_interes_mes*1.18)*32.32 if  moneda_ing_interes_mes==5
replace interes_int=(monto_ing_interes_mes*0.83)*32.32 if  moneda_ing_interes_mes==14
...
replace otros_int=(monto_otros_ing_sem*32.32)/6	if moneda_otros_ing_sem==2
replace otros_int=((monto_otros_ing_sem*1.18)*32.32)/6	if moneda_otros_ing_sem==5
replace otros_int=((monto_otros_ing_sem*0.83)*32.32)/6	if moneda_otros_ing_sem==14
...
replace r_jul=monto_jul*32.32 	if moneda_jul==2
replace r_jul=(monto_jul*1.18)*32.32 if moneda_jul==5
replace r_jul=(monto_jul*0.83)*32.32 if moneda_jul==14

gen r_ago=monto_ago		    	if moneda_ago==0
replace r_ago=monto_ago		    if moneda_ago==1
replace r_ago=monto_ago*32.32 	if moneda_ago==2
replace r_ago=(monto_ago*1.18)*32.32 if moneda_ago==5
replace r_ago=(monto_ago*0.83)*32.32 if moneda_ago==14

gen r_sep=monto_sep 	    	if moneda_sep==0
replace r_sep=monto_sep 		if moneda_sep==1
replace r_sep=monto_sep*32.32 	if moneda_sep==2
replace r_sep=(monto_sep*1.18)*32.32 if moneda_sep==5
replace r_sep=(monto_sep*0.83)*32.32 if moneda_sep==14
...
// Note: se debe tener en cuenta que los ingresos por comisiones, propinas, etc son 
// de todas las ocupaciones no solo del ingreso principal(formulario) pero aquí se las considera dentro del trabajo principal para hacerlo comparable al 2004.
// Aunque al final esto se suma en ylm_ci
...
egen ylnmpri_ci=rsum(alimentos vivienda1 transporte otros)
replace ylnmpri_ci=. if alimentos==. & vivienda1==. & transporte==. & otros==. 
label var ylnmpri_ci "Ingreso laboral NO monetario actividad principal"   

// Note: se debe tener en cuenta que los ingresos por comisiones, propinas, etc son 
// de todas las ocupaciones (formulario) pero aquí se las considera dentro del trabajo principal.
// Aunque esto se suma en ylm_ci
...
egen ynlm_ci=rsum(pension intereses alquiler remesasnales otrosing gobierno pension_int interes_int otros_int remesas_prom dividendos)
...
gen piso_ch=1 if (piso!=9 | piso!=10) & piso!=.
replace piso_ch=0 if piso==9
replace piso_ch=2 if piso==10
...
replace pared_ch=2 if pared_exterior==14
...
replace techo_ch=1 if techo==1 | techo==2 | techo==4
...
replace afiliado_ci = 1 if (afiliado_afp == 1 & emp_ci==1)
...
*****************
/*
gen tipocontrato_ci1=. /* Hay muchos missings en tipo de contrato, solo se puede saber si tiene contrato o no */
replace tipocontrato_ci1=1 if tipo_contrato==1 & categopri_ci==3
replace tipocontrato_ci1=2 if (tipo_contrato==2 | tipo_contrato==3) & categopri_ci==3
replace tipocontrato_ci1=3 if firmo_contrato==2 & categopri_ci==3
