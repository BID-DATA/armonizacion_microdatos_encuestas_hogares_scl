...
Round: m10
Autores: 
Última versión: Mayra Sáenz, 22 de Septiembre de 2013
...
*Si la información está en dólares se multiplica por 39.17
...
replace pension_int=monto_ing_pension_mes*39.17   	if  moneda_ing_pension_mes==2
replace pension_int=(monto_ing_pension_mes*0.78)*39.17     if  moneda_ing_pension_mes==14
...
replace interes_int=monto_ing_interes_mes*39.17	    if  moneda_ing_interes_mes==2
replace interes_int=(monto_ing_interes_mes*0.78)*39.17 	if  moneda_ing_interes_mes==14
...
replace otros_int=(monto_otros_ing_sem*39.17)/6	if moneda_otros_ing_sem==2
replace otros_int=(monto_otros_ing_sem*53.70)/6	if moneda_otros_ing_sem==14
...
replace r_jul=monto_jul*39.17 	if moneda_jul==2
replace r_jul=(monto_jul*0.78)*39.17 	if moneda_jul==14

gen r_ago=monto_ago		    	if moneda_ago==0
replace r_ago=monto_ago		    if moneda_ago==1
replace r_ago=monto_ago*39.17 	if moneda_ago==2
replace r_ago=(monto_ago*0.78)*39.17 	if moneda_ago==14

gen r_sep=monto_sep 	    	if moneda_sep==0
replace r_sep=monto_sep 		if moneda_sep==1
replace r_sep=monto_sep*39.17 	if moneda_sep==2
replace r_sep=(monto_sep*0.78)*39.17 	if moneda_sep==14
...
gen tamemp_o=1 if cant_pers_trab>0 & cant_pers_trab<=3
replace tamemp_o=2 if cant_pers_trab>=4 & cant_pers_trab<=6
replace tamemp_o=3 if cant_pers_trab>=7 & cant_pers_trab!=.
