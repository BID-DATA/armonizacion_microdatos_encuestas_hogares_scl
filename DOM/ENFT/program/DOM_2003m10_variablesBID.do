...
*local base_in  = "$ruta\survey\\`PAIS'\\`ENCUESTA'\\`ANO'\\`ronda'\data_merge\\DOM2003EA_BID.dta"
/* MLO: no se encontro el do file que armonizaba la base, se tomo la base armonizada y se agregaron nuevas variables,
Queda pendiente hacer la armonizacion nuevamente a partir de la base original - dejar resto de do file*/
...
replace desalent_ci = 1 if (busco_trab_mes_ant == 2 & inlist(motivo_no_busca_trab, 1, 11) & condocup_ci == 3)
...
/*
gen subemp_ci= (tothoras>=1 & tothoras<=30) & emp_ci==1 & desea_trab_mas_horas==1
...
* Nota MGD 09/26/2014: al poner los ceros como missing se corrige la tendencia de la variable, sin embargo hayq ue tomar en cuenta que casi el 50% 
* de los ocupados tiene "cero" en la variable.
egen antiguedad_ci= rsum(tiempo_lab_anos temp1 temp2), missing 
replace antiguedad_ci=. if emp_ci==0  | antiguedad_ci==0
replace antiguedad_ci=. if tiempo_lab_dias==. & tiempo_lab_mes==. & tiempo_lab_anos==.
label var antiguedad_ci "Antiguedad en la actividad actual en anios"
...
*****************
/*
gen tipocontrato_ci=. /* Hay muchos missings en tipo de contrato, solo se puede saber si tiene contrato o no */
replace tipocontrato_ci=1 if tipo_contrato==1 & categopri_ci==3
replace tipocontrato_ci=2 if (tipo_contrato==2 | tipo_contrato==3) & categopri_ci==3
replace tipocontrato_ci=3 if firmo_contrato==2 & categopri_ci==3
...
foreach var of varlist ing_ocup_princ  monto_interes_ing_nac monto_alquiler_ing_nac monto_remesas_ing_nac monto_otros_ing_nac monto_gobierno_ing_nac ing_ocup_secun monto_ing_pension_mes monto_ing_interes_mes monto_equiv_regalo monto_otros_ing_sem monto_jul monto_ago monto_sep {
                sum `var'
                recode `var' 99999=.
...
/*
	1/2003	1	PESO	20.02
1/2003	2	DOLARES	1.00
1/2003	14	EURO	1.00
1/2003	13	FRANCO	1.51
2/2003	1	PESO	25.75
2/2003	2	DOLARES	1.00
2/2003	12	BOLIVAR	1,552.98
2/2003	14	EURO	0.93
2/2003	13	FRANCO	1.42
*/
...
replace pension_int=monto_ing_pension_mes*25.75   	if  moneda_ing_pension_mes==2

*Modificado Mayra Sáenz - Febrero 2014 Al considerar las pensiones en bolivares se distorsionan los ingresos
* que pasan de 2240.27 en el año 2002 a 15061.35 en 2003. Por lo tanto, no se considera este valor, ya que es
* únicamente una observación en toda la muestra.

*replace pension_int=(monto_ing_pension_mes*1552.98)*25.75     if  moneda_ing_pension_mes==12
replace pension_int=(monto_ing_pension_mes*1.42)*25.75     if  moneda_ing_pension_mes==13
replace pension_int=(monto_ing_pension_mes*0.93)*25.75     if  moneda_ing_pension_mes==14
replace pension_int=. if recibio_ing_pension_mes==2


gen interes_int=monto_ing_interes_mes	 		    if  moneda_ing_interes_mes==1
replace interes_int=monto_ing_interes_mes*25.75    if  moneda_ing_interes_mes==2
replace interes_int=(monto_ing_interes_mes*0.93)*25.75	if  moneda_ing_interes_mes==14
...
replace otros_int=(monto_otros_ing_sem*25.75)/6	if moneda_otros_ing_sem==2
replace otros_int=((monto_otros_ing_sem*0.93)*25.75)/6	if moneda_otros_ing_sem==14
...
replace r_jul=monto_jul*25.75 	if moneda_jul==2
replace r_jul=(monto_jul*0.93)*25.75 	if moneda_jul==14

gen r_ago=monto_ago		    	if moneda_ago==0
replace r_ago=monto_ago		    if moneda_ago==1
replace r_ago=monto_ago*25.75 	if moneda_ago==2
replace r_ago=(monto_ago*0.93)*25.75 	if moneda_ago==14

gen r_sep=monto_sep 	    	if moneda_sep==0
replace r_sep=monto_sep 		if moneda_sep==1
replace r_sep=monto_sep*25.75 	if moneda_sep==2
replace r_sep=(monto_sep*0.93)*25.75 	if moneda_sep==14
...
*NOTA: esta información parece que se recogió semestral, pero en la base sólo aparecen las preguntas
*relacionadas con el  3er trimestre. Por ello aparece información de remesas promedio de cero para 
*el 10.6% de los individuos que afirmaron haber recibido remesas.
...
***piso_ch***
*************
*Mayra Sáenz- Febrero 2014. En este año no se encuentran observaciones con categoría 21 (otros) en piso.
...
* Variables
 ...
 gen combustible	     =combustible_cocin 		
...
/* Regions
 1. Region Distrito Nacional
 2. Region Valdesia
 3. Region Este
 4. Region Nordeste
 5. Region Cibao Central
 6. Region Norcentral
 7. Region Dajabon
 8. Region Del Valle
 9. Region Enriquillo
 */
 ...
 gen 	 region2=1 if prov==1  | prov==32				
 replace region2=2 if prov==17 | prov==21 | prov==29 | prov==31  	
 replace region2=3 if prov==8  | prov==11 | prov==12 | prov==23 | prov==30 
 replace region2=4 if prov==6  | prov==14 | prov==19 | prov==20
 replace region2=5 if prov==13 | prov==24 | prov==28
 replace region2=6 if prov==9  | prov==18 | prov==25
 replace region2=7 if prov==5  | prov==15 | prov==26 | prov==27
 replace region2=8 if prov==2  | prov==7  | prov==22
 replace region2=9 if prov==3  | prov==4  | prov==10 | prov==16
...
/*
Alumbrado Público
9a ¿Tienen las calles alumbrado público?
...
No tiene ninguno__
 ...
 a. Radio		h. Aire acondicionado	o. Motocicleta
 b. Televisor		i. Teléfono		p. Abanico eléctrico
 c. Estufa		j. Celular		q. Video vhs
 d. Refrigerador	k. Aspirador eléctrico	r. Horno eléctrico
 e. Lavadora		l. Máquina de coser	s. Horno de microondas
 f. Calentador		m. Licuadora		t. Inversor
 g. Plancha eléctrica	n. Automóvil		u. Computador
*/
 ...
 gen apar_elec=1 if radio==1 | televisor==1 | refrigerador==1 | lavadora==1 | plancha_elect==1   /*
	       ...
	       */| video==1 | horno_elect==1 | horno_microndas==1 | inversor==1 | computadora==1 
...
* Gender classification of the population refers to the head of the household.
 ...
 gen	 SFUELS=0 if (combustible>=1 & combustible<=6)
 replace SFUELS=1 if (combustible==4 | combustible==5)
...
* Gender classification of the population refers to the head of the household.
 ...
 gen     TELCEL=0 if (telefono==1 | telefono==2) & (celular==1 | celular==2)
