...
Encuesta: ENFT
Round: m4
Autores: Mayra Saenz mayras@iadb.org/ saenzmayra.a@gmail.com, 9 de Diciembre de 2016
...
2/2014	1	44.96 (Peso)
2/2014	2	1 (Dolares)
2/2014	5	1.26 (Dolar Canadiense)
2/2014	13	0.96 (Franco)
2/2014	14	0.91 (Euro)
*/	
...
*Si la información está en dólares se multiplica por 44.96


*Modulo Ingresos del Exterior
gen pension_int=monto_ing_pension_mes	 		    if  moneda_ing_pension_mes==1
replace pension_int=monto_ing_pension_mes*44.96   	if  moneda_ing_pension_mes==2
replace pension_int=(monto_ing_pension_mes*1.26)*44.96     if  moneda_ing_pension_mes==5
replace pension_int=(monto_ing_pension_mes*0.91)*44.96     if  moneda_ing_pension_mes==14
replace pension_int=. if recibio_ing_pension_mes==2


gen interes_int=monto_ing_interes_mes	 		    if  moneda_ing_interes_mes==1
replace interes_int=monto_ing_interes_mes*44.96	    if  moneda_ing_interes_mes==2
replace interes_int=(monto_ing_interes_mes*1.26)*44.96 	if  moneda_ing_interes_mes==5
replace interes_int=(monto_ing_interes_mes*0.91)*44.96  if  moneda_ing_interes_mes==14
...
replace r_jul=monto_jul*44.96 	if moneda_jul==2
replace r_jul=(monto_jul*1.26)*44.96 	if moneda_jul==5
replace r_jul=(monto_jul*0.91)*44.96 	if moneda_jul==14

gen r_ago=monto_ago		    	if moneda_ago==0
replace r_ago=monto_ago		    if moneda_ago==1
replace r_ago=monto_ago*44.96 	if moneda_ago==2
replace r_ago=(monto_ago*1.26)*44.96 	if moneda_ago==5
replace r_ago=(monto_ago*0.91)*44.96 	if moneda_ago==14

gen r_sep=monto_sep 	    	if moneda_sep==0
replace r_sep=monto_sep 		if moneda_sep==1
replace r_sep=monto_sep*44.96	if moneda_sep==2
replace r_sep=(monto_sep*1.26)*44.96 	if moneda_sep==5
replace r_sep=(monto_sep*0.91)*44.96 	if moneda_sep==14
...
************************************************************************************************************/
*Mayra Sáenz: tomado de http://www.one.gob.do/Buscador/SearchMain 
*Boletín de Estadísticas Oficiales de Pobreza Monetaria, Octubre 2015
...
gen lp_ci =.
replace lp_ci = 4748.7 if zona_c==1
replace lp_ci = 4228.0 if zona_c==0
...
*********
gen lpe_ci =.
replace lpe_ci =  2138.2 if zona_c==1
replace lpe_ci =  2048.7 if zona_c==0
