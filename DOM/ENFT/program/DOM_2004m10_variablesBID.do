...
***segsoc_ci***
***************
*Esta pregunta no se levanto en el 2004 por eso la variable "afiliado_afp" no tiene observaciones
gen segsoc_ci=.
...
* Las variables desde comisiones hasta beneficios no se levantaron por eso no tiene observaciones
...
*Si la información está en dólares se multiplica por 42.54
*Si la información está en euros se multiplica por 51.88

*Modulo Ingresos del Exterior
gen pension_int=monto_ing_pension_mes	 		    if  moneda_ing_pension_mes==1
replace pension_int=monto_ing_pension_mes*42.54   	if  moneda_ing_pension_mes==2
replace pension_int=(monto_ing_pension_mes*0.82)*42.54 if  moneda_ing_pension_mes==14
replace pension_int=. if recibio_ing_pension_mes==2
*existe solo una observacion con libra Italiana para el 2011 pero la base no reporta la tasa de cambio

gen interes_int=monto_ing_interes_mes	 		    if  moneda_ing_interes_mes==1
replace interes_int=monto_ing_interes_mes*42.54	    if  moneda_ing_interes_mes==2
replace interes_int=(monto_ing_interes_mes*0.82)*42.54 if  moneda_ing_interes_mes==14
...
replace otros_int=(monto_otros_ing_sem*42.54)/6	if moneda_otros_ing_sem==2
replace otros_int=((monto_otros_ing_sem*0.82)*42.54)/6	if moneda_otros_ing_sem==14
...
gen r_jul=monto_jul			    if moneda_jul==0 & recibio_remesa==1
replace r_jul=monto_jul		    if moneda_jul==1 & recibio_remesa==1
replace r_jul=monto_jul*42.54 	if moneda_jul==2 & recibio_remesa==1
replace r_jul=(monto_jul*0.82)*42.54	if moneda_jul==14 & recibio_remesa==1

gen r_ago=monto_ago		    	if moneda_ago==0 & recibio_remesa==1
replace r_ago=monto_ago		    if moneda_ago==1 & recibio_remesa==1
replace r_ago=monto_ago*42.54 	if moneda_ago==2 & recibio_remesa==1
replace r_ago=(monto_ago*0.82)*42.54 if moneda_ago==14 & recibio_remesa==1

gen r_sep=monto_sep 	    	if moneda_sep==0 & recibio_remesa==1
replace r_sep=monto_sep 		if moneda_sep==1 & recibio_remesa==1
replace r_sep=monto_sep*42.54 	if moneda_sep==2 & recibio_remesa==1
replace r_sep=(monto_sep*0.82)*42.54 if moneda_sep==14 & recibio_remesa==1
...
egen ynlm_ci=rsum(pension intereses alquiler remesasnales otrosing gobierno pension_int interes_int otros_int remesas_prom dividendos), missing
