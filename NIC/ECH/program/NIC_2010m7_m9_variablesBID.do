...
**************
gen civil_ci=.
replace civil_ci=1 if s07p18==1 | s07p18==7
...
replace condocup_ci=4 if edad_ci<10 /*A parir del 2010 la PET asciende a 14*/
label define condocup_ci 1"ocupados" 2"desocupados" 3"inactivos" 4"menor que 10"
...
replace desalent_ci = 1 if (s11p07 == 2 & (s11p14 == 4 | s11p14 == 10) & condocup_ci == 3)
...
replace afiliado_ci = 1 if((s07p12==1 | s07p12==2 | s07p12==4) & emp_ci==1)
...
replace cotizando_ci = 1 if (s11p33a==1 & emp_ci==1)
...
/*
gen tamemp_ci=s11p33
replace tamemp_ci=. if s11p33==9
label define tamemp_ci 1 "1 persona" 2 "2 a 5 personas" 3 "6 a 9 personas" ///
4 "10 a 20 personas" 5 "21 a 50 personas" 6 "51 a 100 personas" 7 "101 y más personas" 
...
gen ymensual=s12p49_1p11  if emp_ci==1
replace ymensual= .       if s12p49==. | s12p49==2 | s12p49==3

gen ymensual2=s12p49_1s11 if emp_ci==1
replace ymensual2= .      if s12p49==. | s12p49==2 | s12p49==3

 
*Aguinaldo, Bono y vacaciones
gen aguinaldo=s12p49_2p12/s12p49_2mp12 		if s12p49_2mp12!=99 & s12p49_2p12a==1
gen aguinaldo2= s12p49_2s12/s12p49_2ms12 	if s12p49_2ms12!=99 & s12p49_2s12a==1

gen bono= s12p49_2p13/s12p49_2mp13			if s12p49_2mp13!=99 & s12p49_2p13a==1
gen bono2= s12p49_2s13/s12p49_2ms13			if s12p49_2ms13!=99 & s12p49_2s13a==1
...
*Ojo existe alguien que gasta mas de los que gana 
...
***ylmpri_ci***
***************
egen ylmpri_ci=rsum(ymensual aguinaldo bono vacaciones yindmonet)
...
***ylnmpri_ci***
****************
gen ylnmpri_ci=ynomonetario
label var ylnmpri_ci "Ingreso laboral NO monetario actividad principal"   

***************
***ylmsec_ci***
***************
egen ylmsec_ci=rsum(ymensual2 aguinaldo2 bono2 vacaciones2 yindmonet2)
...
***ynlm_ci***
*************
egen ynlm_ci=rsum(yrentaprop yrentainmuebles ytransferencias)
...
Se utlilizan s07p15a y s07p15b - grado y anios escolar más alto que aprobó.
...
* Ignorado y ns a missing 
replace s07p15a = . if s07p15a == 99
replace s07p15b = . if s07p15b == 9

gen aedu_ci = . 
replace aedu_ci = 0 if inlist(s07p15a, 0, 1) // Ninguno, Prescolar 
replace aedu_ci = s07p15b if s07p15a == 2 // Primaria
replace aedu_ci = 6 + s07p15b if s07p15a == 3 // Secundaria 
replace aedu_ci = 9 + s07p15b if inlist(s07p15a, 4, 5) // Técnico básico, medio
replace aedu_ci = 11 + s07p15b if inlist(s07p15a, 6, 7) // Técnico Superior, Universitario
replace aedu_ci = 11 + 5 + s07p15b if s07p15a == 8 // Maestría 
replace aedu_ci = 11 + 5 + 2 + s07p15b if s07p15a == 9 // Doctorado
...
***asipre_ci***
***************
/* Todos aquellos asistentes que no declaren haber aprobado el ultimo anio
 de prescolar.
*/
gen byte asispre_ci = (s07p16==1 & s07p15a == 1 & s07p15b < 3)
...
**************
gen eduac_ci = .
replace eduac_ci = 1 if inlist(s07p15a, 7, 8 ,9) // Universitario, Maestría, Doctorado
replace eduac_ci = 0 if (s07p15a == 6) // Técnico Superior
...
clonevar codindustria=s11p26
clonevar codocupa=s11p24
