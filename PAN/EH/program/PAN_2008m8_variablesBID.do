...
* Alternativa 2: segun la codificacion de p10_18. MGD 06/09/2014
gen condocup_ci=.
replace condocup_ci=1 if p10_18>= 1 & p10_18<= 5 
...
*2014, 02 incorporacion ", missing" MLO
...
drop jub alqui loter becas agro otroy ayfam ynlme ynlmd ynlme1 ynlmd1 aguin
...
gen vivi1_ch=.
gen vivi2_ch=.
*Viviprop_ch sí consta en el cuestionario pero no en la base.
gen viviprop_ch=.
gen vivitit_ch=.
*Vivialq_ch  sí consta en el cuestionario pero no en la base.
gen vivialq_ch=.
gen vivialqimp_ch=.
...
gen lp_ci =.
replace lp_ci= 76.68 if areareco==1 & dist==1 /* Cdad. Panamá*/
replace lp_ci= 76.68 if areareco==1 & dist==3 /* Zona urbana districto san miguelito*/
replace lp_ci= 84.03 if ((dist!=1 & dist!=3) & areareco==1) | areareco==2  /* resto urbano o rural*/
...
gen lpe_ci =.
replace lpe_ci= 39.88 if areareco==1 & dist==1 /* Cdad. Panamá*/
replace lpe_ci= 39.88 if areareco==1 & dist==3 /* Zona urbana districto san miguelito*/
replace lpe_ci= 41.29 if ((dist!=1 & dist!=3) & areareco==1) | areareco==2  /* resto urbano o rural*/
