...
****idh_ch****
**************
egen idh_ch =group(prov dist corre estra unidad cuest hogar areareco u_p)
...
replace condocup_ci=2 if p10_17 == 6
replace condocup_ci=3 if p10_17 >= 7 & p10_17 <= 14 | p10_17 == 0
...
replace desalent_ci = 1 if (p10_17==8 & condocup_ci == 3)
...
g ocupa_ci=.
replace ocupa_ci=1 if (aux_p26>=140 & aux_p26<=662) & emp_ci==1
replace ocupa_ci=2 if (aux_p26>=1 & aux_p26<=139) & emp_ci==1
replace ocupa_ci=3 if (aux_p26>=663 & aux_p26<=817) & emp_ci==1
replace ocupa_ci=4 if ((aux_p26>=880 & aux_p26<=917) | (aux_p26>=1555 & aux_p26<=1565)) & emp_ci==1
replace ocupa_ci=5 if ((aux_p26>=818 & aux_p26<=879) | (aux_p26>=1566 & aux_p26<=1608)) & emp_ci==1
replace ocupa_ci=6 if (aux_p26>=918 & aux_p26<=1000)  & emp_ci==1
replace ocupa_ci=7 if ((aux_p26>=1001 & aux_p26<=1554) | (aux_p26>=1609 & aux_p26<=1649)) & emp_ci==1
replace ocupa_ci=8 if  (aux_p26>=1650 & aux_p26<=1651)& emp_ci==1
replace ocupa_ci=9 if (aux_p26>=1652 & aux_p26<=1653 ) & emp_ci==1
...
***durades_ci***p20
****************
gen d=p20-200 if p20>=201 & p20<299
replace d=0.5 if p20==100
...
* Line of code with indicator repiteult was deleted* Line of code with indicator repite_ci was deleted* Line of code with indicator repite_ci was deleted
...
gen lp_ci =.
replace lp_ci= 63.13 if zona_c==1 & dist==1 /* Cdad. Panamá*/
replace lp_ci= 63.13 if zona_c==1 & dist==3 /* Zona urbana districto san miguelito*/
replace lp_ci= 71.21 if ((dist!=1 & dist!=3) & zona_c==1) | zona_c==0  /* resto urbano o rural*/
...
gen lpe_ci =.
replace lpe_ci= 30.13 if zona_c==1 & dist==1 /* Cdad. Panamá*/
replace lpe_ci= 30.13 if zona_c==1 & dist==3 /* Zona urbana districto san miguelito*/
replace lpe_ci= 31.44 if ((dist!=1 & dist!=3) & zona_c==1) | zona_c==0  /* resto urbano o rural*/
...
replace afiliado_ci = 1 if (p5==1 & emp_ci==1)
replace afiliado_ci = 0 if ((p10_17 >= 1 & p10_17 <= 6) & inlist(condocup_ci, 1, 2))
...
*******************
*******************
* David Cornejo Febrero 2023: no existe la variable cotizante ni afiliado
...
*2014,01 elimino siguiente linea MLO
*replace ypen_ci=.
