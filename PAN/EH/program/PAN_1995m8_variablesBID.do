...
****idh_ch*****
***************
egen idh_ch=group(provin distrito estrato upm nrobol tregionp areap)
...
replace condocup_ci=3 if  sumactiv >= 8 &  sumactiv <= 14 |  sumactiv == 0 |  sumactiv == 7 | sumactiv == 6
...
gen condocup_ci=.
replace condocup_ci=1 if sumactiv >= 1 & sumactiv <= 4  
replace condocup_ci=2 if (sumactiv >= 5 & sumactiv <= 7) | ((sumactiv >=9 & sumactiv <=12) & (tpobus==1))
...
replace rama_ci=2 if ramaactv>=1200 & ramaactv<=1429 & emp_ci==1
...
replace tamfirma_ci=1 if tamest>=2 & tamest<=5 /* 5 o mas*/
...
* PAN 1995
gen salmm_ci= . /*195.50*/
replace salmm_ci= 156 if rama_ci==1
replace salmm_ci= 198.4 if rama_ci==2
replace salmm_ci= 186.4 if rama_ci==3
replace salmm_ci= 198.4 if rama_ci==4
replace salmm_ci= 248.8 if rama_ci==5
replace salmm_ci= 186.4 if rama_ci==6
replace salmm_ci= 198.4 if rama_ci==7
replace salmm_ci= 225.6 if rama_ci==8
replace salmm_ci= 198.4 if rama_ci==9
replace salmm_ci= 199.64 if salmm_ci==.
...
gen cesante_ci=1 if tpoultrj>200 & tpoultrj!=999 & condocup_ci==2
