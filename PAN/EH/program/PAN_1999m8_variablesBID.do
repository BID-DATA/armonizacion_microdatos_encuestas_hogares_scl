*use "${surveysFolder}\ARM\PAN\1999\Orig_data\h_ago99.dta", clear
...
****idh_ch*****
***************
egen idh_ch=group(prov dist estra unidad cuest regireco areareco)
...
gen salmm_ci= . /*212.14*/
replace salmm_ci= 201.6 if rama_ci==1
replace salmm_ci= 233.6 if rama_ci==2
replace salmm_ci= 212.8 if rama_ci==3
replace salmm_ci= 225.6 if rama_ci==4
replace salmm_ci= 289.6 if rama_ci==5
replace salmm_ci= 212.8 if rama_ci==6
replace salmm_ci= 225.6 if rama_ci==7
replace salmm_ci= 257.6 if rama_ci==8
replace salmm_ci= 246.93 if rama_ci==9
replace salmm_ci= 234.01 if salmm_ci==.
