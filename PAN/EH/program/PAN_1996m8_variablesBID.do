...
rename *,lower /*put all variables lowercase*/
...
egen idh2_ch=group(provin estrato upm nrobol tregionp areap)

/*version 7.0
forvalues i=1(1)9764 {
qui sum idh_ch if id_hogar==`i'
qui gen desv`i' =r(sd) if id_hogar==`i'
list id_hogar idh_ch dist personas parentco nroper personas matri g4to fracaso recurso trabajar interes enfermed otros if desv`i'>0 & desv`i'<.
}
*/
...
gen tamfirma_ci=.
replace tamfirma_ci=0 if tamest==1 
* menos de 5 
replace tamfirma_ci=1 if tamest>=2 & tamest<=5 
* mas de 5 
...
*MLO: la base que tenemos en mecovi no tiene esta variable
*pareciera que se corrio sobre otra base el do file anterior
*podría activarse si se consigue nuevamente la base original que la incluya
gen agui=.
