...
label var	p4c_otro	"	Otro motivo por el que no está inscrito	"
label var   p4q  "¿Es usted una persona con discapacidad?"
label var   p4q_tipo  "¿Qué tipo de discapacidad tiene?"
...
label var	p8_16	"	Condición de actividad	"
...
label var	h5_int_mov	"	¿Internet movil...?	"
...
*******************************************************
	...
	*Pregunta: ¿Se considera usted negro(a) o afrodescendiente? (p4f) 1- SI, 2- NO
	gen byte afro_ci = . 	  
	replace afro_ci = 1 if p4f>=1 & p4f<=7
	replace afro_ci = 0 if p4f == 8
	...
	*Pregunta: ¿Se considera usted indígena? (p4d) 1- SI, 2- NO	
	gen byte ind_ci = .
	replace ind_ci = 1 if p4d>=1 & p4d<=10 
	replace ind_ci = 0 if p4d == 11 
...
replace desalent_ci = 1 if (p8_16 == 10 & condocup_ci == 3)
...
gen internet_ch = 0
replace internet_ch = 1 if ((h5_int_mov == 1 & h5_serv_in == 2) | (h5_int_mov == 2 & h5_serv_in == 1) | (h5_int_mov == 1 & h5_serv_in == 1))
replace internet_ch = 0 if (h5_int_mov == 2 & h5_serv_in == 2)
