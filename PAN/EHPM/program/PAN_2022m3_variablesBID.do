...
label var	provincia	"Provincia incluye código de Panamá Oeste (13)	"
label var	prov	    "Provincia excluye código de Panamá Oeste	"
...
label var	p2	        "Sexo	"
label var	p3	        "Edad	"
label var	p4	        "¿Tiene ud. Seguro social?	"
...
label var   p4q_discap  "¿Es usted una persona con discapacidad?"
label var   p4q_tipo_d  "¿Qué tipo de discapacidad tiene?"
label var	p4d	        "¿Se considera usted indígena?	"
*label var	p4d_otro	"Otro grupo indígena al que pertenece	"
label var	p4f        	"¿Se considera usted negro(a) o afrodescendiente?	"
*label var	p4f_otro	"Otro afrodescendiente	"
...
label var p4o_pais      "¿Cuál es su país de origen?"
label var p4p_cuando    "¿Cuándo llegó usted a vivir a panamá?"
label var p4p_anio      "¿En qué año llegó?"
...
label var	p7b_veces	"¿Cuántas veces ha repetido?	"
...
label var	p23	        "¿Para qué tipo de trabajo estuvo o estará disponible	"
label var	p24	        "¿Cuánto tiempo hace…?	"
label var	p25	        "¿Motivo dejó su último trabajo?	"
label var	p25_otro	"Otro de la pregunta P25	"
label var	p27	        "¿Dónde trabaja o trabajó...?	"
label var	p27a_otro	"Código de otros de la pregunta P27	"
...
label var	p33_sitio   "Provincia – DISTRITO – CORREGIMIENTO	"
...
label var	p39e_horas  "¿Cuántas horas trabajó la semana pasada en su otro trabajo..?	"
...
label var	h2b_celula	"¿Teléfono celular…?	"
label var	h2e_radio	"¿Radio…?	"
label var	h2a_televi	"¿Televisor…?	"
label var	h2a1_tv_cu	"¿Cuántos TV...?	"
label var	h2a2_cable	"Señal de Cable TV	"
...
label var	h5_serv_in	"¿Internet fija...?	"
label var	h5_serv__a	"¿Internet movil...?	"
...
gen civil_ci=.
replace civil_ci=1 if p4l==7
replace civil_ci=2 if p4l==1 | p4l==4
replace civil_ci=3 if p4l==2 | p4l==3 | p4l==5
replace civil_ci=4 if p4l==6
label var civil_ci "Estado civil"
label define civil_ci 7 "Soltero" 2 "Union formal o informal" 3 "Divorciado o separado" 4 "Viudo"
...
gen condocup_ci=.
replace condocup_ci=1 if ocup_deso==3
replace condocup_ci=2 if ocup_deso==1 
replace condocup_ci=3 if ocup_deso==2 
...
replace desalent_ci = 1 if (p08_16 == 10 & condocup_ci == 3)
...
* Line of code with indicator pqnoasis_ci was deleted* Line of code with indicator pqnoasis_ci was deletedlabel define pqnoasis 1 "Falta de interes" 2 "Falta de recursos económicos" 3 "No tiene la edad requerida" 4 "Ya se graduó" 5 "No se ofrece el nivel o grado escolar en la comunidad" 6 "Necesita trabajar" 7 "Se casó" 8 "Está muy distante de su vivienda" 9 "Enfermedad" 10 "Embarazo" 11 "Quehaceres domesticos" 12 "Otros"
* Line of code with indicator pqnoasis_ci was deleted
**************
*pqnoasis1_ci*
**************
...
**********************
g razonesnoasis_ci = .
replace razonesnoasis_ci = 1 if inlist(p5a, 2, 6)
replace razonesnoasis_ci = 2 if p5a==1
replace razonesnoasis_ci = 3 if inlist(p5a, 7, 9, 10, 11)
replace razonesnoasis_ci = 4 if inlist(p5a, 5, 8)
replace razonesnoasis_ci = 5 if inlist(p5a, 3, 4, 12)
...
* Line of code with indicator repite_ci was deleted* Line of code with indicator repite_ci was deleted*solo para personas de 5 a 18 años 
...
* PAN 2018-2019: https://www.mitradel.gob.pa/nuevas-tasas-salario-minimo-region-1-region-2/
* Se estima en base a tabla en general documentation - external data
gen salmm_ci= . 
replace salmm_ci= 416.80 if rama_ci==1
replace salmm_ci= 684.00 if rama_ci==2
replace salmm_ci= 486.00 if rama_ci==3
replace salmm_ci= 753.60 if rama_ci==4
replace salmm_ci= 730.80 if rama_ci==5
replace salmm_ci= 515.10 if rama_ci==6
replace salmm_ci= 633.60 if rama_ci==7
replace salmm_ci= 736.80 if rama_ci==8
replace salmm_ci= 654.09 if rama_ci==9
replace salmm_ci= 623.42 if salmm_ci==.
...
replace categoinac_ci=1 if (p08_16==11 | p08_16==12) & condocup_ci==3
*Estudiantes
replace categoinac_ci=2 if p08_16==13 & condocup_ci==3
*Quehaceres del Hogar
replace categoinac_ci=3 if p08_16==14 & condocup_ci==3
*Otra razon
...
* Variables incluidas por SCL/MIG Fernando Morales
	...
	*******************
	...
	destring p4o_pais, replace
	gen migrante_ci=(p4o_pais!=1) if p4o_pais!=.
	...
	**********************
	...
	gen migantiguo5_ci=(migrante_ci==1 & (p4p_cuando==1 | (p4p_anio<2015 & p4p_cuando==2))) if migrante_ci!=. & !inrange(edad_ci,0,4) & p4p_cuando!=3
