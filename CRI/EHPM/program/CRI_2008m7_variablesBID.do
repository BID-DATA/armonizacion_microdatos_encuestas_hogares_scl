...
capture drop lp_ci
gen lp_ci = .
replace lp_ci =  77096 if zona_c==1
replace lp_ci =  58947 if zona_c==0
...
gen lpe_ci = .
replace lpe_ci =   35301 if zona_c==1
replace lpe_ci =   28981 if zona_c==0
...
*2008  junio(fuente OIT-CEPAL observatorio de la crisis)
* 2015 MGD cambio por datos oficiales M. Trabajo segun zona urbana y rural
gen salmm_ci= 	156624 if zona_c==0
replace salmm_ci= 196777 if zona_c==1
...
En 2008 no se codifica secundaria técnica ignorada 39
ni parauniversitaria ignorada 49
...
replace aedu_ci = 6 if (b08 == 29) // Secundaria Ignorada
replace aedu_ci = 11 if (b08 == 59) //  Superior Ignorada
...
gen byte edupc_ci = inlist(b08, 16, 29)
...
gen byte edusc_ci = inlist(b08, 25, 36, 59)
...
gen byte edus2c_ci = inlist(b08, 25, 36, 59)
...
gen luz_ch=.
replace luz_ch=1 if v14>=1 & v14<=7
replace luz_ch=0 if v14==8 
