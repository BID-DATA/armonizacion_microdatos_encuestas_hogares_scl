...
Encuesta: EHPM
Round: m7
Autores: Yanira Oviedo, Junio 2010
...
capture drop lp_ci
gen lp_ci = .
replace lp_ci =     26084 if zona_c==1
replace lp_ci =     20516 if zona_c==0
...
gen lpe_ci = .
replace lpe_ci =    11965 if zona_c==1
replace lpe_ci =    10414 if zona_c==0
...
*cesante_ci* 
*************
* MGD 12/4/2015: condicionado a que este desempleado.
...
* 2001 junio (fuente OIT-CEPAL observatorio de la crisis) 66815.00
...
replace cotizando_ci = 1 if (((b06>=1 & b06<=4)) & emp_ci==1)
...
replace afiliado_ci = 1 if((b06>0 & b06<99) & emp_ci==1)
...
replace rama_ci=8 if (c17>=65110 & c17<=74210) & emp_ci==1
replace rama_ci=9 if (c17>=74301 & c17<=99000) & emp_ci==1
...
replace ylmpri_ci=. if ingprinc>=99999999
...
g yasal  = ingprinc 
replace yasal=. if ingprinc >= 99999999

recode c33a (9999999=.)
g yganpri =.
replace yganpri= (c33a*8)*22  if c33a1 ==1
replace yganpri= c33a*4.3  if c33a1 ==3
replace yganpri= c33a*2    if c33a1 ==4
replace yganpri= c33a      if c33a1 ==5
replace yganpri= c33a/4    if c33a1 ==6
replace yganpri= c33a/6    if c33a1 ==7
replace yganpri= c33a/12   if c33a1 ==8
...
egen ylmpri_ci= rsum(yasal yganpri), missing
...
/*
* Mayra Sáenz Abril 2014: Se debe revisar la inclusión de los rubros detallados abajo:
*Se incluye el ingreso por pensiones, subsidios, becas, otras transferencias, rentas
...
egen ynlm_ci= rsum(ingpension ingsubsi ingbecas ingotrans ingrentas), missing
*/
* Mayra Sáenz Abril 2014: Se debe revisar la inclusión de los rubros detallados abajo:
/*
recode c33b (9999999=.)
gen autoconsumo=.
replace autoconsumo= (c33b*8)*22  if c33b1 ==1
replace autoconsumo= c33b*4.3  if c33b1 ==3
replace autoconsumo= c33b*2    if c33b1 ==4
replace autoconsumo= c33b      if c33b1 ==5
replace autoconsumo= c33b/4    if c33b1 ==6
replace autoconsumo= c33b/6    if c33b1 ==7
replace autoconsumo= c33b/12   if c33b1 ==8

gen ylnmpri_ci=autoconsumo
*/
...
replace aedu_ci=. if b10==2 //Educacion Especial. Solo como check
...
replace aedu_ci=0 if b10==19
replace aedu_ci=6 if b10==29
replace aedu_ci=6 if b10==39
replace aedu_ci=11 if b10==49
replace aedu_ci=11 if b10==59
...
gen byte eduui_ci=0
replace eduui_ci=1 if (b10>=41 & b10<=42)
...
gen byte eduuc_ci=0
replace eduuc_ci=1 if b10==43 | (b10>=55 & b10<=58)
...
gen auto_ch=.
replace auto_ch=1 if  v16o==1
replace auto_ch=0 if  v16o==2
...
/*NA*/
	...
	********************
	gen miglac_ci  = .
	label var miglac_ci "=1 si es migrante de un pais LAC, calculado sobre migrantes"
