...
capture drop lp_ci
gen lp_ci = .
replace lp_ci =  66529 if zona_c==1
replace lp_ci =  50945 if zona_c==0
...
gen lpe_ci = .
replace lpe_ci =  28319 if zona_c==1
replace lpe_ci =  23550 if zona_c==0
...
****************
gen cotizando_ci=.
*replace cotizando_ci=1 if (b07>=1 & b07<=3) | b073==1
replace cotizando_ci=1 if (b07>=1 & b07<=3)
*MLO: la variable b073 esta en el diccionario pero no en la base
...
****************
gen tipopen_ci=.
*MLO: la variable b072 esta en el diccionario pero no en la base
...
replace aux_p = c35a/6 if c35a1==7 /*semestre*/
...
* 2007 junio (fuente OIT-CEPAL observatorio de la crisis)
* 2015 MGD cambio por datos oficiales M. Trabajo segun zona urbana y rural
gen salmm_ci= 	135356 if zona_c==0
replace salmm_ci= 171540 if zona_c==1
...
replace rama_ci=8 if (c17>=65110 & c17<=73200) & emp_ci==1
replace rama_ci=9 if (c17>=74110 & c17<=99999) & emp_ci==1
...
gen edupub_ci=.
replace edupub_ci = 1 if inlist(b091, 1, 2) & asiste_ci == 1 // Incluye a los semi públicos 
replace edupub_ci = 0 if b091 == 3 & asiste_ci == 1
