...
replace relacion_ci=4 	if a3>=4 & a3<=11
...
*cotizando_ci***
****************
gen cotizando_ci=1 if (a11==1 | a11==2) 
...
*
gen afiliado_ci=.
replace afiliado_ci = 0 if a11==0 | a11==9 | a11==10
replace afiliado_ci =1 if a11>0 & a11<9
...
replace ypen_ci=h9h1/2 if  h9h2 ==2
replace ypen_ci=h9h1/3 if  h9h2 ==3
replace ypen_ci=h9h1/4 if  h9h2 ==4
replace ypen_ci=h9h1/6 if  h9h2 ==6
replace ypen_ci=h9h1/12 if  h9h2 ==8
replace ypen_ci=. if  h9h2 ==9
...
*Revisar el pdf  file:///Z:/general_documentation/data_externa/Salarios/CostaRica_Lista_salarios_IISemestre_2013%20(1).pdf
...
TNCG: Trabajador no Calificado Genéricos ¢257.219,78* 
TSCG: Trabajador Semicalificado Genéricos ¢277.073,82* 
TCG: Trabajador Calificado Genéricos ¢291.516,18* 
TMED Técnico Medio Educación Diver. ¢310.412,99* 
TEG: Trabajador Especializado Genéricos ¢332.646,95* 
...
1/1/2013 8,416.72	9,164.03	9,340.79	11,194.82	251,191.19	270,579.90	284,683.77	303,137.69	324,850.54	373,583.85	403,484.51	457,646.94	549,195.15
1/7/2013 8,618.72	9,383.97	9,564.97	11,463.50	257,219.78	277,073.82	291,516.18	310,412.99	332,646.95	382,549.86	413,168.14	468,630.47	562,375.83
*/
* 2015 MGD: Para el salario promedio se tomó a los salarios para trabajadores genéricos+el promedio de los agrícolas.  a 07/2013
* Los salarios agrícolas estan por jornada diaria, por lo que se multiplica por 25.8 días equivalentes a trabajar 6 días por semana al mes (4.3 semanas)..

gen salmm_ci= 282041.54
...
replace aedu_ci=11+4 if a14==89
...
********************************************************************************************************************************
gen byte eduuc_ci = ((a14 >= 41 & a14 <= 59) & inlist(a16b, 1, 2, 3, 4, 5 , 6)) | (a14 >= 71 & a14 <= 89)
...
********************************************************************************************************************************
gen eduac_ci = . 
replace eduac_ci = 1 if (a14 >= 51 & a14 <= 89)
...
* Modificaciones Marcela Rubio Septiembre 2014: cambio en sintaxis error en variable utilizada
/*
gen asiste_ci=.
...
********************************************************************************************************************************
gen edupub_ci=.
replace edupub_ci=1 if (a15==1 | a15==2 ) & asiste_ci==1 // incluye los semi publicos
replace edupub_ci=0 if (a15==3 ) & asiste_ci==1 // incluye los extranjeros
...
* PTMC: Avancemos (a9a)
* PNC: Pensión régimen no contributivo (h9e h9e1)
