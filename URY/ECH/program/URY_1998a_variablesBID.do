...
*tamemp_ci***
*************
gen tamemp_ci=trabmeno
replace tamemp_ci=. if  trabmeno==0
...
*************
gen salmm_ci = .
replace salmm_ci = 950   if mes_c==1 | mes_c==2  | mes_c==3  | mes_c==4 | mes_c==5 | mes_c==6  
replace salmm_ci = 990   if mes_c==7  | mes_c==8 | mes_c==9 | mes_c==10 | mes_c==11 | mes_c==12
label var	salmm_ci	"Salario minimo legal 1998"
...
* Modificacion MGD 06/23/2014: horas de la actividad principaln.
gen subemp_ci=0
...
replace aedu_ci=0         if (nivel==1 & ultano==9) | (nivel==1 & ultano==0)
replace aedu_ci=ultano    if nivel==1 & ultano<9
replace aedu_ci=ultano+6  if nivel==2 & ultano<9
replace aedu_ci=ultano+9  if nivel==3 & ultano<9
replace aedu_ci=ultano+9  if nivel==4 & ultano<9
replace aedu_ci=ultano+12 if nivel==5 | nivel==6 & ultano<9 | nivel==7
replace aedu_ci=0         if asistio==2 & (edad>=5 & edad!=.)
...
*** Correcciones Marcela Rubio Septiembre 2014: cambios en sintaxis

/*
8 preescolar o jardin
1 primaria
...
rename ramactv codindustria 
rename ocuprinc codocupa 
compress
